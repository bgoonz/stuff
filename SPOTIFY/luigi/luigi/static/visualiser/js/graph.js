Graph = (function () {
  ...
  var legendMaxY = (function () {
    return (
      Object.keys(statusColors).length * legendLineHeight + legendLineHeight / 2
    );
  })();
  ...
  function layoutNodes(nodes, rowSizes) {
    ...
    function rowStartPosition(depth) {
      ...
      return (
        rowStartPosition(depth - 1) + Math.max(rowHeight * nodeHeight + 100)
      );
    }
  ...
  DependencyGraph.prototype.renderGraph = function () {
    ...
    $.each(this.graph.nodes, function (i, node) {
      ...
      $(svgLink(node.trackingUrl))
        .append($(svgElement("text")).text(node.name).attr("y", 3))
      ...
      var content = $.map(node.params, function (value, name) {
        return name + ": " + value;
      }).join("<br>");
      g.attr("title", titleText).popover({
        trigger: "hover",
        container: "body",
        html: true,
        placement: "top",
    ...
    var legend = $(svgElement("g")).addClass("legend").appendTo(self.svg);
