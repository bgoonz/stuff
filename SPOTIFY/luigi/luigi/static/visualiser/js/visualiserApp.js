function visualiserApp(luigi) {
  ...
  function renderTemplate(templateName, dataObject) {
    return $("<div>").html(
      Mustache.render(templates[templateName], dataObject)
    );
  }
  ...
  function taskToDisplayTask(task) {
    ...
    var displayTime = new Date(
      Math.floor(task.last_updated * 1000)
    ).toLocaleString();
    var time_running = -1;
    if (task.status == "RUNNING" && "time_running" in task) {
      var current_time = new Date().getTime();
      var minutes_running = Math.round(
        (current_time - task.time_running * 1000) / 1000 / 60
      );
  ...
  function filterByCategory(dt, activeBoxes) {
    ...
    dt.column(0)
      .search(categoryQuery(activeBoxes), (regex = true))
      .draw();
  }
  ...
  function renderWarnings() {
    return renderTemplate("warningsTemplate", {
      missingCategories: $.map(missingCategories, function (v, k) {
        return v;
      }),
    });
  }
  ...
  function processWorker(worker) {
    ...
    worker.tasks.sort(function (task1, task2) {
      return task1.timeRunning - task2.timeRunning;
    });
  ...
  function renderWorkers(workers) {
    return renderTemplate("workerTemplate", {
      workerList: workers.map(processWorker),
    });
  }
  ...
  function renderResources(resources) {
    return renderTemplate("resourceTemplate", {
      resources: resources.map(processResource).sort(function (r1, r2) {
        if (r1.percent_used > r2.percent_used) return -1;
        else if (r1.percent_used < r2.percent_used) return 1;
        else if (r1.num_used > r2.num_used) return -1;
        else if (r1.num_used < r2.num_used) return 1;
  ...
  function showStatusMessage(data) {
    $("#statusMessageModal")
      .empty()
      .append(renderTemplate("statusMessageTemplate", data));
    ...
    var refreshInterval = setInterval(function () {
      if ($("#statusMessageModal").is(":hidden"))
        ...
        luigi.getTaskStatusMessage(data.taskId, function (data) {
          if (data.statusMessage === null) $("#statusMessageModal pre").hide();
          else {
        ...
        luigi.getTaskProgressPercentage(data.taskId, function (data) {
          ...
          $("#statusMessageModal .progress").toggle(
          ...
          var progressBars = $(
            "#statusMessageModal .progress-bar, " +
              '.worker-table tbody .taskProgressBar[data-task-id="' +
              data.taskId +
              '"]'
          );
          progressBars
    ...
    $send.on("click", function ($event) {
      ...
      if (content && data.worker) {
        if (awaitResponse) {
          ...
          luigi.sendSchedulerMessage(
            data.worker,
            data.taskId,
            content,
            function (messageId) {
              var interval = window.setInterval(function () {
                luigi.getSchedulerMessageResponse(
                  data.taskId,
                  messageId,
                  function (response) {
                    if (response != null) {
  ...
  function makeGraphCallback(visType, taskId, paint) {
    ...
    function depGraphCallback(dependencyGraph) {
      ...
      if (dependencyGraph.length > 0) {
        ...
        $("#graphPlaceholder")
          .get(0)
          .graph.updateData(dependencyGraph, hashBase);
        $("#graphContainer").show();
        bindGraphEvents();
      } else {
        $("#searchError").addClass("alert alert-error");
  ...
  function processHashChange(paint) {
    ...
    if (fragmentQuery.tab == "workers") {
      ...
      $("#invertCheckbox").prop(
        "checked",
        fragmentQuery.invert === "1" ? true : false
      );
      ...
      $("#serverSideCheckbox").prop(
        "checked",
        fragmentQuery.filterOnServer === "1" ? true : false
      );
      ...
      if (fragmentQuery.family) {
        family_item = $(
          '#familySidebar li[data-task="' + fragmentQuery.family + '"]'
        );
  ...
  function drawGraphETL(tasks, paint) {
    ...
    function draw(isUpdate) {
      for (var id in tasks) {
        ...
        html +=
          "<span class=queue><span class=counter>" +
          task.status +
          "</span></span>";
        html += "</div>";
      ...
      if (paint) {
        ...
        zoom.event(
          isUpdate ? svg.transition().duration(3000) : d3.select("#mysvg")
        );
      }
      ...
      inner
        ...
        .attr("title", function (v) {
          return styleTooltip(v, getParam(tasks, v));
        })
        .each(function (v) {
  ...
  function updateTaskCategory(dt, category, tasks) {
    ...
    $("#" + category + "_info i.fa")
      .removeClass()
      .addClass("fa fa-" + taskIcons[category]);
  }
  ...
  function initVisualisation(newVisType) {
    ...
    if (newVisType == "d3") {
      ...
      $("#graphPlaceholder").html(
        '<div class="live map"><svg width="100%" height="100%" id="mysvg"><g/></svg></div>'
      );
    } else {
      $("#graphPlaceholder").empty();
  ...
  function updateTasks() {
    ...
    var ajax6 = luigi.getUpstreamDisabledTaskList(function (
    ...
    $.when(ajax1, ajax2, ajax3, ajax4, ajax5, ajax6, ajax7, ajax8).done(
      function () {
        dt.draw();

        $(".sidebar").html(renderSidebar(dt.column(1).data()));
        var selectedFamily = $(".sidebar-menu").find(
          'li[data-task="' + currentFilter.taskFamily + '"]'
        )[0];
  ...
  function updateWorkerProcesses(worker, n) {
    ...
    var $label = $("#workerList").find(
      '#label-n-workers[data-worker="' + worker + '"]'
    );
  ...
  function expandedResources() {
    return $(".resource-box.in")
      .toArray()
      .map(function (val) {
        return val.dataset.resource;
      });
  }
  ...
  function createPauseToggle(checked) {
    ...
    var html = $(
      '<input id="pause" type="checkbox"' + check + ' data-toggle="toggle">'
    );
  ...
  $(document).ready(function () {
    ...
    luigi.getResourceList(function (resources) {
      ...
      $(".resources-collapse").click(function (e) {
        ...
        changeState(
          "resources",
          resourceList.length > 0 ? JSON.stringify(resourceList) : null
        );
    ...
    $("#serverSide").html(
      '<form class="form-inline"><label class="btn btn-default" for="serverSideCheckbox">' +
    ...
    $("#taskTable tbody").on(
      "click",
      "td.details-control .forgiveFailures",
      function (ev) {
        var that = $(this);
    ...
    $("#taskTable tbody").on(
      "click",
      "td.details-control .re-enable-button",
      function (ev) {
        var that = $(this);
    ...
    $("#taskTable tbody").on(
      "click",
      "td.details-control .statusMessage",
      function () {
        var data = $(this).data();
        showStatusMessage(data);
      }
    ...
    $("#taskTable tbody").on(
      "click",
      "td.details-control .schedulerMessage",
      function () {
        var data = $(this).data();
        showSchedulerMessageModal(data);
      }
    ...
    $("#workerList").on("show.bs.modal", "#setWorkersModal", function ($event) {
      $("#setWorkersButton").data(
        "worker",
        $($event.relatedTarget).data("worker")
      );
      var $input = $(this)
        .find("#setWorkersInput")
    ...
    $("#resourceList").on(
      ...
      "#setResourcesModal",
      function ($event) {
        $("#setResourcesButton").data(
          "resource",
          $($event.relatedTarget).data("resource")
        );
        var $input = $(this)
          .find("#setResourcesInput")
          .on("keypress", function ($event) {
            if (event.keyCode == 13) {
