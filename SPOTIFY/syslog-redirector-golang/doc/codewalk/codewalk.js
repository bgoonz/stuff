...
CodewalkViewer.prototype.installEventHandlers = function () {
  ...
  this.context.find(".comment").click(function (event) {
    if (jQuery(event.target).is("a[href]")) return true;
    self.changeSelectedComment(jQuery(this));
    return false;
  });

  this.context.find("#code-selector").change(function () {
    self.navigateToCode(jQuery(this).val());
  });

  this.context
    .find("#description-table .quote-feet.setting")
    .click(function () {
      self.toggleDescription(jQuery(this));
      return false;
    });

  this.sizer.mousedown(function (ev) {
    self.startSizerDrag(ev);
    return false;
  });
  this.overlay
    .mouseup(function (ev) {
      self.endSizerDrag(ev);
      return false;
    })
    .mousemove(function (ev) {
      self.handleSizerDrag(ev);
      return false;
    });

  this.context.find("#prev-comment").click(function () {
    self.changeSelectedComment(self.lastSelected.prev());
    return false;
  });

  this.context.find("#next-comment").click(function () {
  ...
  // keypresses.
  this.context.find("#code-display").load(function (ev) {
    self.shortcutInput.focus();
  });
...
CodewalkViewer.prototype.handleSizerDrag = function (ev) {
  ...
  this.options.codeWidth = parseInt(
    (this.codeColumn.width() /
      (this.codeColumn.width() + this.commentColumn.width())) *
      100
  );
...
CodewalkViewer.prototype.changeSelectedComment = function (target) {
  ...
  if (!(this.lastSelected && this.lastSelected.get(0) === target.get(0))) {
    ...
    if (
      targetTop + target.height() > parentTop + target.parent().height() ||
      targetTop < parentTop
    ) {
    ...
    this.context
      .find("#prev-comment")
      .toggleClass("disabled", !target.prev().length);
    this.context
      .find("#next-comment")
      .toggleClass("disabled", !target.next().length);
  }
...
CodewalkViewer.prototype.updateHeight = function () {
  ...
  this.commentArea.height(
    areaHeight -
      footerHeight -
      this.context.find("#comment-options").outerHeight(true)
  );
  var codeHeight = areaHeight - footerHeight - 15; // GOK
  this.codeArea.height(codeHeight);
  this.codeDisplay.height(
    codeHeight - this.codeDisplay.offset().top + this.codeArea.offset().top
  );
