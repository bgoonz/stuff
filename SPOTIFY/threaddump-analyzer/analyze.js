...
function ThreadStatus(thread) {
  this.isRunning = function () {
    return (
      this.thread.frames.length > 0 && this.thread.threadState === "RUNNABLE"
    );
  };
  ...
  this.toHtml = function () {
    ...
    if (this.thread.wantNotificationOn !== null) {
      ...
      html +=
        '&quot; without waiting for anything">inconsistent<sup>?</sup></span>';
    }
...
function StringCounter() {
  ...
  this.getStrings = function () {
    ...
    for (var string in this._stringsToCounts) {
      if (
        !Object.prototype.hasOwnProperty.call(this._stringsToCounts, string)
      ) {
        continue;
      }
...
function Synchronizer(id, className) {
  ...
  this.toHtmlTableRow = function () {
    ...
    if (this.lockHolder !== null) {
      html += '<div class="synchronizer">';
      html +=
        'Held by:<br><span class="raw">  ' +
        this.lockHolder.getLinkedName() +
        "</span>";
      html += "</div>";
    }

    html += createLockUsersHtml(
...
function Analyzer(text) {
  ...
  this.toHtml = function () {
    ...
    for (var i = 0; i < threadsAndStacks.length; i++) {
      ...
      var noOrThis =
        currentThreadsAndStack.stackFrames.length === 0 ? "no" : "this";
      if (threads.length > 4) {
        asHtml +=
          '<div class="threadcount">' +
          threads.length +
          " threads with " +
          noOrThis +
          " stack:</div>\n";
  ...
  this._createSynchronizerById = function () {
    ...
    for (var i = 0; i < this.threads.length; i++) {
      ...
      this._registerSynchronizer(
        returnMe,
        thread.wantNotificationOn,
        thread.synchronizerClasses
      );
      this._registerSynchronizer(
        returnMe,
        thread.wantToAcquire,
        thread.synchronizerClasses
      );

      for (var j = 0; j < thread.locksHeld.length; j++) {
        var lock = thread.locksHeld[j];
        this._registerSynchronizer(returnMe, lock, thread.synchronizerClasses);
      }
    }
