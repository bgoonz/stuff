...
QUnit.test("thread.getLinkedName()", function (assert) {
  ...
  assert.equal(
    new Thread(header).getLinkedName(),
...
QUnit.test("thread header 2", function (assert) {
  var header =
    '"ApplicationImpl pooled thread 1" prio=4 tid=11296d000 nid=0x118a84000 waiting on condition [118a83000]';
...
QUnit.test("thread header 3", function (assert) {
  var header =
    '"Gang worker#1 (Parallel GC Threads)" prio=9 tid=105002800 nid=0x10bc88000 runnable';
...
QUnit.test("thread header 4", function (assert) {
  var header =
    '"Attach Listener" #10 daemon prio=9 os_prio=31 tid=0x00007fddb280e000 nid=0x380b waiting on condition [0x0000000000000000]';
...
QUnit.test("thread header 5", function (assert) {
  var header =
    '"Attach Listener" #10 daemon prio=9 os_prio=31 tid=0x00007fddb280e000 nid=0x380b waiting on condition';
  assert.equal(new Thread(header).name, "Attach Listener");
  assert.equal(new Thread(header).tid, "0x00007fddb280e000");
  assert.equal(new Thread(header).group, undefined);
});
...
QUnit.test("thread header 6", function (assert) {
  var header =
    '"VM Thread" os_prio=31 tid=0x00007fddb2049800 nid=0x3103 runnable';
...
QUnit.test("thread header 7", function (assert) {
  var header =
    '"Queued build chains changes collector 8" daemon group="main" prio=5 tid=431,909 nid=431,909 waiting ';
  assert.equal(
    new Thread(header).name,
    "Queued build chains changes collector 8"
  );
...
QUnit.test("thread header 8", function (assert) {
  var header =
    '"Attach Listener" #10 prio=9 os_prio=31 tid=0x00007fddb280e000 nid=0x380b waiting on condition [0x0000000000000000]';
  assert.equal(new Thread(header).name, "Attach Listener");
  assert.equal(new Thread(header).tid, "0x00007fddb280e000");
  assert.equal(new Thread(header).group, undefined);
});
...
QUnit.test("thread header 9", function (assert) {
  var header =
    '"Connect thread foo.net session" prio=5 tid=8,057,104 nid=8,057,104';
...
QUnit.test("thread header 10", function (assert) {
  var header =
    '"thread name" daemon prio=10 tid=0x00007f16a118e000 nid=0x6e5a runnable [0x00007f18b91d0000]';
  assert.equal(new Thread(header).name, "thread name");
  assert.equal(new Thread(header).tid, "0x00007f16a118e000");
  assert.equal(new Thread(header).group, undefined);
});
...
QUnit.test("thread header 11", function (assert) {
  var header =
    '"VM Periodic Task Thread" prio=10 tid=0x00007f1af00c9800 nid=0x3c2c waiting on condition ';
...
QUnit.test("thread header 12", function (assert) {
  var header =
    '"Store spotify-uuid Spool Thread" daemon prio=10 tid=0x00007f1a16aa0800 nid=0x3f5b sleeping[0x00007f199997a000]';
...
QUnit.test("thread header 13", function (assert) {
  var header =
    '"git@github.com:caoliang2598/ta-zelda-test.git#master"}; 09:09:58 Task started; VCS Periodical executor 39" prio=10 tid=0x00007f1728056000 nid=0x1347 sleeping[0x00007f169cdcb000]';
  assert.equal(
    new Thread(header).name,
...
QUnit.test("non-multiline thread name", function (assert) {
  ...
  assert.equal(threads.length, 1);
  var threadLines = threads[0].toString().split("\n");
  assert.deepEqual(threadLines, ['"line 1": ', "	<empty stack>"]);
});
...
QUnit.test("analyze thread waiting for notification", function (assert) {
  ...
  var locksHeld = [
    /* Lock is released while synchronizing */
  ];
...
QUnit.test(
  "analyze thread waiting for java.util.concurrent lock",
  function (assert) {
    var threadDump = [
    ...
    assert.equal(
      thread.synchronizerClasses["7c2cd7dd0"],
...
QUnit.test("analyze thread waiting for traditional lock", function (assert) {
  ...
  assert.equal(
    thread.synchronizerClasses["0xe0375410"],
...
QUnit.test(" analyze thread waiting for locks 2", function (assert) {
  ...
  assert.equal(
    thread.synchronizerClasses["259a4a41"],
...
QUnit.test("analyze thread holding locks", function (assert) {
  ...
  assert.equal(
    thread.synchronizerClasses["7c37ef220"],
    "io.netty.channel.nio.SelectedSelectionKeySet"
  );
  assert.equal(
    thread.synchronizerClasses["7c392fac0"],
    "java.util.Collections$UnmodifiableSet"
  );
  assert.equal(
    thread.synchronizerClasses["7c37f5b88"],
    "sun.nio.ch.KQueueSelectorImpl"
  );
  assert.equal(
    thread.synchronizerClasses["7c393f190"],
...
QUnit.test("thread stack", function (assert) {
  var header =
    '"Thread name" prio=10 tid=0x00007f1728056000 nid=0x1347 sleeping[0x00007f169cdcb000]';
  ...
  assert.ok(
    thread.addStackLine(
      "	- eliminated <0x00000006b3ccb178> (a java.io.PipedInputStream)"
    )
  );
  assert.ok(
    thread.addStackLine(
  ...
  assert.ok(
    thread.addStackLine(
      "	- parking to wait for  <0x00000003138d65d0> (a java.util.concurrent.locks.AbstractQueuedSynchronizer$ConditionObject)"
    )
  );
...
QUnit.test("Analyzer.stackToHtml()", function (assert) {
  ...
  assert.deepEqual(
    analyzer._stackToHtml(["top_frame", "second_frame"]).split("\n"),
    [
...
QUnit.test("Synchronizer class name", function (assert) {
  assert.equal(
    new Synchronizer("x", "java.lang.Foo").getPrettyClassName(),
    "Foo"
  );
  assert.equal(
    new Synchronizer(
      "x",
      "java.lang.Class for java.lang.Foo"
    ).getPrettyClassName(),
    "Foo.class"
  );
  assert.equal(new Synchronizer("x", "Foo").getPrettyClassName(), "Foo");
  assert.equal(
    new Synchronizer("x", undefined).getPrettyClassName(),
    undefined
  );
});
...
QUnit.test("Synchronizer.toHtmlTableRow()", function (assert) {
  assert.equal(
    new Synchronizer("1234", "g.w.Bush").toHtmlTableRow(),
...
QUnit.test("thread status unset, locks held", function (assert) {
  ...
  assert.equal(
    threadStatus.toHtml(),
    'non-Java thread, holding [<a href="#synchronizer-aaa" class="internal">aaa</a>]'
  );
});
...
QUnit.test("thread status sleeping", function (assert) {
  ...
  assert.equal(
    threadStatus.toHtml(),
    'sleeping, holding [<a href="#synchronizer-aaa" class="internal">aaa</a>]'
  );
});
...
QUnit.test(
  "analyze thread waiting for unspecified notification 1",
  function (assert) {
    var threadDump = [
...
QUnit.test(
  "analyze thread waiting for unspecified notification 2",
  function (assert) {
    var threadDump = [
...
QUnit.test(
  "analyze thread waiting for unspecified notification 3",
  function (assert) {
    var threadDump = [
      '"Thread Name" daemon prio=10 tid=0x000000000219d000 nid=0x3fa3 in Object.wait() [0x00007f0fc985d000]',
...
QUnit.test(
  "analyze thread waiting for unspecified notification 4",
  function (assert) {
    var threadDump = [
...
QUnit.test("lock user html creator", function (assert) {
  ...
  assert.equal(
    createLockUsersHtml("Threads waiting to take lock", [thread1]),
  ...
  assert.equal(
    createLockUsersHtml("Threads waiting to take lock", [
      thread3,
      thread1,
      thread4,
      thread2,
...
QUnit.test("synchronizer sort function", function (assert) {
  ...
  assert.deepEqual([zebra, unused].sort(synchronizerComparator), [
    unused,
    zebra,
  ]);
  ...
  assert.deepEqual(
    [zebra, biggerId, unused, held].sort(synchronizerComparator),
