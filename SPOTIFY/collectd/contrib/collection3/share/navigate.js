function nav_init(time_begin, time_end) {
  ...
  for (i = 0; i < all_images.length; i++) {
    if (all_images[i].className != "graph_image") continue;

    all_images[i].navTimeBegin = new Number(time_begin);
    all_images[i].navTimeEnd = new Number(time_end);

    all_images[i].navBaseURL = all_images[i].src.replace(
      /;(begin|end)=[^;]*/g,
      ""
    );

    if (all_images[i].addEventListener) {
      /* Mozilla */
      all_images[i].addEventListener(
        "dblclick",
        nav_handle_dblclick,
        false /* == bubbling */
      );
...
function nav_image_repaint(img) {
  if (!img || !img.navBaseURL || !img.navTimeBegin || !img.navTimeEnd) return;

  img.src =
    img.navBaseURL +
    ";" +
    "begin=" +
    img.navTimeBegin.toFixed(0) +
    ";" +
    "end=" +
    img.navTimeEnd.toFixed(0);
} /* nav_image_repaint */
...
function nav_time_change_obj(img, factor_begin, factor_end) {
  ...
  if (!img || !img.navBaseURL || !img.navTimeBegin || !img.navTimeEnd)
    return false;

  diff = img.navTimeEnd - img.navTimeBegin;

  /* Prevent zooming in if diff is less than five minutes */
  if (diff <= 300 && factor_begin > 0.0 && factor_end < 0.0) return true;
...
function nav_set_reference(img_id) {
  ...
  for (i = 0; i < all_images.length; i++) {
    ...
    if (
      !tmp ||
      tmp.className != "graph_image" ||
      !tmp.navTimeBegin ||
      !tmp.navTimeEnd
    )
      continue;

    if (tmp.id == img_id) continue;
...
function nav_calculate_offset_x(obj) {
  ...
  if (!obj) return offset;

  offset = obj.offsetLeft;
  if (obj.offsetParent) offset += nav_calculate_offset_x(obj.offsetParent);
...
function nav_recenter(e) {
  ...
  x = e.layerX - 70;
  if (!x || x < 0 || x > width) return;

  y = e.layerY;
  if (!y || y < 35 || y > 135) return;
...
function nav_handle_wheel(e) {
  ...
  nav_recenter(e);
  if (delta > 0) nav_zoom_in(img.id);
  else nav_zoom_out(img.id);
