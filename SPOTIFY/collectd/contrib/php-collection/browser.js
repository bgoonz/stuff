...
function toggleDiv(divID) {
  ...
  if (label_txt && label) {
    ...
    while (childCnt > 0) label.removeChild(label.childNodes[--childCnt]);
...
function refillSelect(options, select) {
  ...
  while (childCnt > 0) select.removeChild(select.childNodes[--childCnt]);
  ...
  if (optCnt == 0) {
    ...
    if (optCnt == 1) {
      ...
      for (i = 0; i < optCnt && !keepSelection; i++)
        if (
          oldValue == (options[i].firstChild ? options[i].firstChild.data : "")
        )
          keepSelection = true;
    }
    newOption = document.createElement("option");
    newOption.value = "/";
    if (keepSelection) newOption.setAttribute("disabled", "disabled");
    else newOption.setAttribute("selected", "selected");
    ...
    for (i = 0; i < optCnt; i++) {
      ...
      if (newOption.value[0] == "@") {
        ...
        else newOption.appendChild(document.createTextNode(newOption.value));
      } else newOption.appendChild(document.createTextNode(newOption.value));
      select.appendChild(newOption);
    }
...
function ListOfHost(response) {
  ...
  var idx = refillSelect(
    response ? response.getElementsByTagName("option") : null,
    select
  );
  if (idx > 0) {
    ListRefreshPlugin();
  } else ListOfPlugin(null);
}
...
function ListRefreshPlugin() {
  ...
  var host =
    host_list.selectedIndex >= 0
      ? host_list.options[host_list.selectedIndex].value
      : "/";
  if (host != "/") {
...
function ListOfPlugin(response) {
  ...
  } else ListOfPluginInstance(null);
}
...
function ListRefreshPluginInstance() {
  ...
  var plugin =
    plugin_list.selectedIndex >= 0
      ? plugin_list.options[plugin_list.selectedIndex].value
      : "/";
  if (host != "/" && plugin != "/") {
    var query =
      "action=list_pinsts&host=" +
...
function ListRefreshType() {
  ...
  var pinst =
    pinst_list.selectedIndex >= 0
      ? pinst_list.options[pinst_list.selectedIndex].value
      : "/";
  if (host != "/" && plugin != "/" && pinst != "/") {
    var query =
      "action=list_types&host=" +
      encodeURIComponent(host) +
      "&plugin=" +
      encodeURIComponent(plugin) +
      "&plugin_instance=" +
      encodeURIComponent(pinst);
    loadXMLDoc(dhtml_url, query);
  } else {
    ListOfType(null);
  }
...
function ListRefreshTypeInstance() {
  ...
  var type =
    type_list.selectedIndex >= 0
      ? type_list.options[type_list.selectedIndex].value
      : "/";
  if (host != "/" && plugin != "/" && pinst != "/" && type != "/") {
    var query =
      "action=list_tinsts&host=" +
      encodeURIComponent(host) +
      "&plugin=" +
      encodeURIComponent(plugin) +
      "&plugin_instance=" +
...
function RefreshButtons() {
  ...
  var tinst =
    tinst_list.selectedIndex >= 0
      ? tinst_list.options[tinst_list.selectedIndex].value
      : "/";
  if (
    host != "/" &&
    plugin != "/" &&
...
function GraphAppend() {
  ...
  var timespan =
    time_list.selectedIndex >= 0
      ? time_list.options[time_list.selectedIndex].value
      : "";
  ...
  if (
    ...
    pinst[0] == "@" ||
    type[0] == "@" ||
    (tinst[0] == "@" && tinst.substr(0, 5) != "@merge")
  ) {
    var query =
      "action=list_graphs&host=" +
      encodeURIComponent(host) +
      "&plugin=" +
      encodeURIComponent(plugin) +
      "&plugin_instance=" +
    ...
    query =
      ...
      encodeURIComponent(type) +
      "&type_instance=" +
      encodeURIComponent(tinst) +
      "&timespan=" +
      encodeURIComponent(timespan);
    query =
      query +
      (logarithmic ? "&logarithmic=1" : "") +
      (tinyLegend ? "&tinylegend=1" : "");
    loadXMLDoc(dhtml_url, query);
  } else
    GraphDoAppend(
      host,
...
function ListOfGraph(response) {
  ...
  if (graphs && graphs.length > 0) {
    for (i = 0; i < graphs.length; i++)
      GraphDoAppend(
        graphs[i].getAttribute("host"),
        graphs[i].getAttribute("plugin"),
        graphs[i].getAttribute("plugin_instance"),
        graphs[i].getAttribute("type"),
        graphs[i].getAttribute("type_instance"),
        graphs[i].getAttribute("timespan"),
        graphs[i].getAttribute("tinyLegend") == "1",
        graphs[i].getAttribute("logarithmic") == "1"
      );
  } else alert("No graph found for adding");
}

function GraphDoAppend(
  host,
  plugin,
  pinst,
  type,
  ...
  if (host != "/" && plugin != "/" && pinst != "/" && type != "/") {
    ...
    if (logarithmic) graph_src += "&logarithmic=1";
    if (tinyLegend) graph_src += "&tinylegend=1";
    if (timespan) graph_src += "&timespan=" + encodeURIComponent(timespan);
    var now = new Date();
    graph_src += "&ts=" + now.getTime();
    graphList.push(
      graph_id +
        " " +
        encodeURIComponent(graph_alt) +
        (logarithmic ? "&logarithmic=1" : "") +
        (tinyLegend ? "&tinylegend=1" : "") +
        "&timespan=" +
        encodeURIComponent(timespan)
...
function GraphDropAll() {
  ...
  while (childCnt > 0)
    if (
      graphs.childNodes[--childCnt].id != "nograph" &&
      (graphs.childNodes[childCnt].nodeName == "div" ||
        graphs.childNodes[childCnt].nodeName == "DIV")
    )
...
function GraphToggleTools(graph) {
  ...
  if (ref_img) {
    ...
    document.getElementById("ge_tinylegend").checked =
      src_url.match(/&tinylegend=1/);
    document.getElementById("ge_logarithmic").checked =
      src_url.match(/&logarithmic=1/);
...
function GraphRefreshAll() {
  ...
  while (imgCnt > 0)
    if (imgs[--imgCnt].parentNode.getAttribute("class") == "graph") {
      ...
      if (newSrc == oldSrc) newSrc = newSrc + newTS;
...
function GraphRefresh(graph) {
  ...
  if (graph == null) {
    var graphId = document.getElementById("ge_graphid").value;
    if (graphId != "") graphElement = document.getElementById(graphId);
  } else graphElement = document.getElementById(graph);
...
function GraphAdjust(graph) {
  var graphId =
    graph == null ? document.getElementById("ge_graphid").value : graph;
  ...
  if (graphElement != null) {
    ...
    while (imgCnt > 0)
      if (imgs[--imgCnt].parentNode.getAttribute("class") == "graph") {
        ...
        if (logarithmic) newSrc += "&logarithmic=1";
        newSrc = newSrc.replace(/&tinylegend=[^&]*/, "");
        if (tinyLegend) newSrc += "&tinylegend=1";
        newSrc = newSrc.replace(/&timespan=[^&]*/, "");
        if (timespan) newSrc += "&timespan=" + encodeURIComponent(timespan);
        imgs[imgCnt].setAttribute("src", newSrc);

        var myList = Array();
        for (i = 0; i < graphList.length; i++)
          if (
            graphList[i].substring(0, graphId.length) == graphId &&
            graphList[i].charAt(graphId.length) == " "
          ) {
            newSrc = graphList[i];
            newSrc = newSrc.replace(/&logarithmic=[^&]*/, "");
            newSrc = newSrc.replace(/&tinylegend=[^&]*/, "");
...
function GraphMoveUp(graph) {
  ...
  for (i = 0; i < childCnt; i++)
    if (
      graphs.childNodes[i].nodeName == "div" ||
      graphs.childNodes[i].nodeName == "DIV"
    ) {
      if (graphs.childNodes[i].id == "nograph") {
        ...
        break;
      } else prevGraph = graphs.childNodes[i];
    }
  for (i = 0; i < graphList.length; i++)
    if (
      graphList[i].substring(0, graphId.length) == graphId &&
  ...
  if (document.cookie.length > 0) {
    ...
    for (i = 0; i < cookies.length; i++)
      if (cookies[i].substring(0, 8) == "graphLst") {
        ...
        options.push(
          new Array(
            cookies[i].substring(8, p),
            cookies[i].substring(p + 1).split("/").length
          )
        );
      }
  }
    ...
    for (i = 0; i < optCnt; i++) {
      ...
      if (options[i][1] == 1)
        newOption.appendChild(
          document.createTextNode(newOption.value + " (1 graph)")
        );
      else
        newOption.appendChild(
          document.createTextNode(
            newOption.value + " (" + options[i][1] + " graphs)"
          )
        );
      select.appendChild(newOption);
    }
  ...
  if (lname) {
    if (lname.value.match(/^[a-zA-Z0-9_-]+$/)) {
      ...
      if (doalert && lname.value.length == 0)
        alert(
          "Graph list name is empty.\n\n" +
            "Please fill in a name and try again."
        );
      else if (doalert)
        alert(
          "Graph list name contains non-permitted character.\n\n" +
...
function GraphSave() {
  ...
  if (graphList.length > 0) {
    ...
    else alert("Successfully saved current graph list.");
  } else {
    document.cookie =
      "graphLst" + lstName + "=; expires=" + new Date().toGMTString();
...
function GraphDrop() {
  ...
  if (cname && cname.selectedIndex >= 0) {
    ...
    document.cookie =
      "graphLst" + cname + "=; expires=" + new Date().toGMTString();
    GraphListRefresh();
  } else return;
}
...
function GraphLoad() {
  ...
  for (i = 0; i < grLst.length; i++) {
    ...
    for (j = 0; j < graph.length; j++)
      if (graph[j] == "logarithmic=1") logarithmic = true;
      else if (graph[j] == "tinylegend=1") tinyLegend = true;
  ...
  if (grLst.length == 0) alert("No list '" + cname + "' found for loading.");
