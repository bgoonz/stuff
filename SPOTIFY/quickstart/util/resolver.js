...
var Resolver = prime({
  constructor: function (options) {
    ...
    this.defaultPath = options.defaultPath
      ? pathogen.resolve(options.defaultPath)
      : null;
  },
  ...
  _findRoute: function (paths, resolved) {
    ...
    return sequence(paths, function (path, i, control) {
      transport.json(path + "package.json").then(
        function (json) {
          if (isPlainObject(json.browser)) {
            self
              ._findRouteInBrowserField(json.browser, path, resolved)
              .then(function (route) {
                if (route == null) control.save(resolved).continue();
                // no route found
  ...
  resolvePackage: function (path, packageName) {
    ...
    var nodePath =
      typeof process !== "undefined" && process.env && process.env.NODE_PATH;
