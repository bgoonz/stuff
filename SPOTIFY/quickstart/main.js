...
function compileSelf(options, appRuntimePath, appRuntimeData, messages) {
  ...
  if (options.defaultPath)
    config.defaultPath = pathogen.relative(root, options.defaultPath);

  var runtimePath = pathogen(__dirname + "/runtime/browser.js");

  return sequence
    .map(options.parsers, function (path) {
...
function compileApp(options, appRuntimePath, appRuntimeData, messages) {
  ...
  return sequence
    .map(options.parsers, function (parserPath) {
      return resolver
        .resolve(root, parserPath)
        .then(require)
    ...
    .then(function (transformModules) {
      ...
      return compiler
        .require(root, options.main ? pathogen(options.main) : "./")
        .then(function (main) {
          return {
            main: main,
            modules: compiler.modules,
...
function compile(options, messages) {
  ...
  var root = (options.root = options.root
    ? pathogen.resolve(options.root)
    : pathogen.cwd());
  if (options.defaultPath)
    options.defaultPath = pathogen.resolve(root, options.defaultPath + "/");

  var appRuntimePath;

  // Get the appropriate runtime path based on options.
  if (options.runtime) appRuntimePath = options.runtime;
  // let it resolve
  else if (options.node)
    appRuntimePath = pathogen.resolve(__dirname, "./runtime/node.js");
  else appRuntimePath = pathogen.resolve(__dirname, "./runtime/browser.js");

  return resolver
    .resolve(root, appRuntimePath)
    .then(function (resolved) {
      return (appRuntimePath = resolved);
    })
    .then(transport)
    .then(function (appRuntimeData) {
      // building with --self means compiling a compiler for a browser.
      if (options.self)
        return compileSelf(options, appRuntimePath, appRuntimeData, messages);
