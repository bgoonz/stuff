...
module.exports = function (argv) {
  ...
  if (argv.config == null)
    argv.config = pathogen.resolve(root, "./quickstart.json");
  ...
  help.write(
    clc.green("--self       "),
    "compile quickstart for browser compilation. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--root       "),
    "use this as the root of each path. defaults to",
    clc.blue(pathogen.cwd())
  );

  help.write(
    clc.green("--main       "),
    "override the default entry point. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--output, -o "),
    "output the compiled source to a file or STDOUT. defaults to",
    clc.blue("STDOUT")
  );

  help.write(
    clc.green("--source-map "),
    "output the source map to a file, STDOUT or inline. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--ast        "),
    "output the Esprima generated AST to a file or STDOUT. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--optimize   "),
    "feed transforms with an optimized ast. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--compress   "),
    "compress the resulting AST using esmangle. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--runtime    "),
    "specify a custom runtime. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--config     "),
    "specify a configuration json file to augment command line options. defaults to",
    clc.red("false")
  );

  help.write(
    clc.green("--warnings   "),
    "display warning messages. defaults to",
    clc.blue("true")
  );

  help.write(clc.green("--help, -h   "), "display this help screen");

  help.write(clc.green("--version, -v"), "display the current version");
    ...
    .then(function (compiled) {
      ...
      if (isString(options.sourceMap)) {
        ...
        compilation.log({
          ...
          source: pathogen.relative(root, sourceMapPath),
        });
      }
      ...
      if (isString(options.output)) {
        ...
        compilation.log({
          id: "source",
          message: "file written",
          source: pathogen.relative(root, sourcePath),
        });
      }
      ...
      if (isString(options.ast)) {
        ...
        compilation.log({
          id: "ast",
          message: "file written",
          source: pathogen.relative(root, astPath),
        });
      }
      ...
      compilation.timeEnd(
        "time",
        options.self ? "compiled itself in" : "compiled in",
        true,
        true
      );
