...
jsGrowlMsg.prototype = {
  ...
  html: function (jsg) {
    var icon_html = this.icon_src
      ? '<img src="' + this.icon_src + '" class="jsg_icon">'
      : "";
    var click_html = this.click_callback
      ? ' onclick="' + jsg + ".clickMsg(" + this.id + ');" '
      : "";
    var click_css = this.click_callback ? ' class="jsg_clickable" ' : "";
    var title_html = this.title
      ? '<div class="jsg_title">' + this.title + "</div>"
      : "";
    return (
      '<table id="growl_flash_msg_' +
      this.id +
      '" class="jsgrowl_msg_container"' +
      ' onmouseover="' +
      jsg +
      ".overMsg(" +
      this.id +
      ...
      '<div id="growl_flash_close_icon_' +
      this.id +
      '" title="Close Message" class="jsg_close" ' +
      ' onclick="' +
      jsg +
      ".closeMsg(" +
      this.id +
...
jsGrowl.prototype = {
  ...
  initialize: function (name, opts) {
    ...
    if (name) {
      ...
      alert(
        "The variable name is required.  I need this for when I write out the message, the message message can fire the correct events."
      );
    }
