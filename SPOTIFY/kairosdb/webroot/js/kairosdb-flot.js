...
function drawSingleSeriesChart(subTitle, data, flotOptions) {
  ...
  $chartContainer.bind("plothover", function (event, pos, item) {
    if (item) {
      if (previousPoint != item.dataIndex) {
        ...
        var formattedDate = $.plot.formatDate(
          timestamp,
          "%b %e, %Y %H:%M:%S.millis %p"
        );
  ...
  $chartContainer.bind("plotselected", function (event, ranges) {
    if (flotOptions.yaxes.length != Object.keys(ranges).length - 1) return;
...
function showTooltip(x, y, contents) {
  var tooltip = $(
    '<div id="tooltip" class="graphTooltip">' + contents + "</div>"
  );
  ...
  if (
    (x > left && x < left + tooltip.outerWidth()) ||
    (y < top && y > top + tooltip.outerHeight())
  ) {
