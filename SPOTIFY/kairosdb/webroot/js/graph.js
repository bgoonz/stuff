...
function displayQuery() {
  ...
  if ($("#query-type-json").is(":checked")) $("#query-text").val(queryString);
  else
    $("#query-text").val(
      "var query = " + queryString.replace(/\"(\w*)\":/g, "$1:") + ";"
    );
}
...
function updateChart() {
  ...
  $graphLink.attr(
    "href",
    "view.html?q=" +
      encodeURI(JSON.stringify(query, null, 0)) +
      "&d=" +
      encodeURI(JSON.stringify(metricData, null, 0))
  );
  $graphLink.show();
...
function buildKairosDBQuery() {
  ...
  $(".metricContainer").each(function (index, element) {
    ...
    $metricContainer.find(".groupBy").each(function (index, groupBy) {
      ...
      if (name == "tags") {
        ...
        if (value < 1) {
          showErrorMessage(
            "Missing Time Group By size must be greater than 0."
          );
          hasError = true;
          return true;
        }
        ...
        if (count < 1) {
          showErrorMessage(
            "Missing Time Group By count must be greater than 0."
          );
          hasError = true;
          return true;
        }
        ...
        if (size < 1) {
          showErrorMessage(
            "Missing Value Group By size must be greater than 0."
          );
          hasError = true;
          return true;
        }
    ...
    function isValidScalingFactor(value) {
      ...
      if (!intRegex.test(value)) {
        showErrorMessage(
          "scaling factor must be a floating point number >= 0."
        );
        hasError = true;
        return false;
      } else {
    ...
    $metricContainer.find("[name='tags']").each(function (index, tagContainer) {
      ...
      if (name && value) metric.addTag(name, value);
    });
  ...
  if (startTimeAbsolute != null) {
    ...
    query.setStartRelative(
      startTimeRelativeValue,
      $("#startRelativeUnit").val()
    );
  } else {
...
function addMetric() {
  ...
  var $newMetric = $(
    '<li id="metricTab' +
      metricCount +
      '">' +
      '<a class="metricTab" style="padding-right:2px;" href="#metricContainer' +
      metricCount +
      '">Metric</a>' +
      '<button id="removeMetric' +
      metricCount +
      '" style="background:none; border: none; width:15px;"></button></li>'
  );
  ...
  $metricContainer
    .find(".metricName")
    .bind("comboboxselect comboboxchange comboboxfocus", function (event) {
  ...
  $metricContainer
    .find("#aggregatorContainer")
    .attr("id", "metric-" + metricCount + "AggregatorContainer");
  var $aggregatorContainer = $(
    "#metric-" + metricCount + "AggregatorContainer"
  );
  ...
  $metricContainer
    .find("#groupByContainer")
    .attr("id", "metric-" + metricCount + "GroupByContainer");
  ...
  searchButton
    ...
    .click(function () {
      ...
      $groupByTagDialog.dialog({
        position: { my: "left bottom", at: "right bottom", of: searchButton },
      });
...
function addAggregator(container) {
  ...
  if (aggregators.length > 0) {
    // Add arrow
    $(
      '<span class="ui-icon ui-icon-arrowthick-1-s aggregatorArrow" style="margin-left: 45px;"></span>'
    ).appendTo(container);
  }
...
function addTag(tagContainer) {
  ...
  // add auto complete
  var $tagNameElement = newDiv
    .find("[name='tagName']")
    .combobox()
    .combobox({
      source: function (request, response) {
        var metricCount = tagContainer.attr("metricCount");
        var metricName = $("#metricContainer" + metricCount)
          .find(".metricName")
          .combobox("value");
        if (metricName) {
          var tags = [];
...
function showChart(subTitle, queries, metricData) {
  ...
  if (dataPointCount > 20000) {
    var response = confirm(
      "You are attempting to plot more than 20,000 data points.\nThis may take a long time." +
  ...
  if (isHighChartsLoaded()) showHighChartsChart(subTitle, yaxis, data);
  else showFlotChart(subTitle, yaxis, data);
  $status.html("");
}
...
function deleteDataPoints() {
  if (
    confirm(
      "Are you sure you want to delete all data points returned from the last query?"
    )
  ) {
    ...
    kairosdb.deleteDataPoints(query, function () {
      if (
        confirm(
          "Data was deleted. It may take up 30 seconds are more to update. Do you want to refresh the graph?"
        )
      ) {
        updateChart();
      }
