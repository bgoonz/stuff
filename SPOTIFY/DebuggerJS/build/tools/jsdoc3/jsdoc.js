...
/** The absolute path to the base directory of the jsdoc application.
    ...
    @global
 */
(__dirname = "."), (args = Array.prototype.slice.call(arguments, 0));
...
function main() {
  ...
  if (env.opts._.length > 0) {
    // are there any files to scan and parse?

    var includeMatch =
        env.conf.source && env.conf.source.includePattern
          ? new RegExp(env.conf.source.includePattern)
          : null,
      excludeMatch =
        env.conf.source && env.conf.source.excludePattern
          ? new RegExp(env.conf.source.excludePattern)
          : null;

    sourceFiles = app.jsdoc.scanner.scan(
      env.opts._,
      env.opts.recurse ? 10 : undefined,
      includeMatch,
      excludeMatch
    );
    ...
    if (typeof publish === "function") {
      publish(new (require("typicaljoe/taffy"))(docs), env.opts, resolver.root);
    } else {
