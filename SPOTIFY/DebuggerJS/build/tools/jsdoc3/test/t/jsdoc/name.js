...
test("The module:jsdoc/name.shorten function breaks a longname up into the correct memberof, name and scope parts.", function () {
  ...
  assert.equal(
    parts.name,
    "open",
    "The name should be the bit after the last scoping character."
  );
  assert.equal(
    parts.memberof,
    "lib.Panel",
    "The memberof should be the bit before the last scoping character."
  );
  assert.equal(
    parts.scope,
    "#",
    "The scope should be the scoping character itself."
  );
});
...
test("The module:jsdoc/name.applyNamespace function inserts the namespace in the correct place.", function () {
  ...
  assert.equal(
    endName,
    "lib.Panel#event:open",
    "The namespace should be inserted only before the name part of the longname."
  );
  ...
  assert.equal(
    endName,
    "module:maths/bigint",
    "The namespace should be inserted before a global name."
  );
});
...
test("The module:jsdoc/name.applyNamespace function treats quoted parts of the name as atomic.", function () {
  ...
  assert.equal(
    ...
    "The namespace should be inserted before a quoted shortname."
  );
});
...
test("The module:jsdoc/name.applyNamespace function will not add another namespace if one already exists.", function () {
  ...
  assert.equal(
    endName,
    "lib.Panel#event:open",
    "The namespace should not be inserted twice."
  );
});
