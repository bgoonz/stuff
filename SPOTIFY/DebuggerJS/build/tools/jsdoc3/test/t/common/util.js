...
test("There is a common/util module.", function () {
  assert.equal(
    typeof common.util,
    "object",
    "The common/util module should be an object."
  );
});

test('The common/util module exports a "mixin" function.', function () {
  assert.equal(
    typeof common.util.mixin,
    "function",
    "The module:common/util.mixin member should be a function."
  );
});
...
test("The module:common/util.mixin function overwrites properties in the target if they exist in the source.", function () {
  ...
  assert.equal(
    target.a,
    1,
    "Existing properties in the target with unique keys are left alone."
  );
  assert.equal(
    target.b,
    source.b,
    "Existing properties in the target with same-named keys are overwritten."
  );
  assert.equal(
    target.c,
    source.c,
    "Properties in the source with unique keys are added to the target."
  );
});
...
test("The module:common/util.mixin function can mix several source objects into the target.", function () {
  ...
  assert.deepEqual(
    source1,
    { a: 1, b: 2 },
    "The source objects themselves are not modified by being mixed in."
  );
  assert.deepEqual(
    returned,
    { a: 1, b: 3, c: 4, d: 5 },
    "The returned object has all the properties of the sources."
  );
});
