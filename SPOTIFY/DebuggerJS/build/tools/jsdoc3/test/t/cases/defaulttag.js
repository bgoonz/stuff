(function () {
  ...
  test("When symbol set to null has a @default tag with no text.", function () {
    assert.equal(
      ...
      "null",
      "The doclet's defaultValue property should be: undefined."
    );
  });
  ...
  test("When symbol set to a string has a @default tag with no text.", function () {
    assert.equal(
      ...
      '"ok"',
      "The doclet's defaultValue property should be that quoted string."
    );
  });
  ...
  test("When symbol set to a number has a @default tag with no text.", function () {
    assert.equal(
      rcode.defaultvalue,
      "200",
      "The doclet's defaultValue property should be that number."
    );
  });
  ...
  test("When symbol has a @default tag with text.", function () {
    assert.equal(
      win.defaultvalue,
      "the parent window",
      "The doclet's defaultValue property should be that text."
    );
  });
