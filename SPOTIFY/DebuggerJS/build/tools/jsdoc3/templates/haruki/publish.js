...
(function () {
  ...
  publish = function (data, opts) {
    ...
    if (opts.destination === "console") {
      ...
      console.log(
        'The only -d destination option currently supported is "console"!'
      );
    }
  };
        ...
        if (element.kind === "namespace") {
          ...
          if (element.returns) {
            parentNode.functions[element.name].returns = {
              type: element.returns.type
                ? element.returns.type.names.length === 1
                  ? element.returns.type.names[0]
          ...
          if (element.params) {
            for (var i = 0, len = element.params.length; i < len; i++) {
              thisFunction.parameters.push({
                ...
                type: element.params[i].type
                  ? element.params[i].type.names.length === 1
                    ? element.params[i].type.names[0]
                ...
                optional:
                  typeof element.params[i].optional === "boolean"
                    ...
                    : "",
                nullable:
                  typeof element.params[i].nullable === "boolean"
                    ? element.params[i].nullable
                    : "",
              });
            }
          }
