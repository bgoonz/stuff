...
exports.resolve = function (doclet) {
  ...
  if (memberof || doclet.forceMemberof) {
    ...
    if (name && name.indexOf(memberof) === 0) {
      ...
      // like @memberof foo# or @memberof foo~
      about = exports.shorten(
        memberof + (scopeToPunc[doclet.scope] || "") + name,
        doclet.forceMemberof ? memberof : undefined
      );
    }
  } else {
  ...
  if (doclet.scope === "global") {
    ...
    if (doclet.name && doclet.memberof && !doclet.longname) {
      ...
      doclet.setLongname(
        doclet.memberof + scopeToPunc[doclet.scope] + doclet.name
      );
    }
  }
 ...
 */
function quoteUnsafe(name, kind) {
  // docspaced names may have unsafe characters which need to be quoted by us
  if (
    jsdoc.tagDictionary.lookUp(kind).setsDocletDocspace &&
