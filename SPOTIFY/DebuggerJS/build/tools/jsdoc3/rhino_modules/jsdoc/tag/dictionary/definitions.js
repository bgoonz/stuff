      ...
      onTagged: function (doclet, tag) {
        if (tag.value) {
          ...
          if (
            ...
            /STRING|NUMBER|NAME|TRUE|FALSE/.test(doclet.meta.code.type)
          ) {
            doclet.defaultvalue = doclet.meta.code.value;
            if (doclet.meta.code.type === "STRING") {
              // TODO: handle escaped quotes in values
              doclet.defaultvalue =
                '"' + doclet.defaultvalue.replace(/"/g, '\\"') + '"';
            }

            if (
              doclet.defaultvalue === "TRUE" ||
              doclet.defaultvalue == "FALSE"
            ) {
...
function applyNamespace(docletOrNs, tag) {
  if (typeof docletOrNs === "string") {
    ...
    docletOrNs.longname = app.jsdoc.name.applyNamespace(
      docletOrNs.name,
      tag.title
    );
  }
...
function setDocletNameToFilename(doclet, tag) {
  var name =
    (doclet.meta.path
      ? doclet.meta.path + java.lang.System.getProperty("file.separator")
