...
Spotify.MessageRateSelector = {
  ...
  getButtonRate: function (buttonId) {
    switch (buttonId) {
      case "btn-stop":
        return this.Rates.STOP;
      case "btn-slow-rate":
        return this.Rates.SLOW;
      case "btn-medium-rate":
        return this.Rates.MEDIUM;
      case "btn-fast-rate":
        return this.Rates.FAST;
      default:
        return this.Rates.STOP;
    }
  },
...
Spotify.TrackNotifier = function () {
  ...
  this.setupMessageInterval = function (messageRate) {
    ...
    if (messageRate < Spotify.MessageRateSelector.Rates.STOP) {
      this.messageInterval = window.setInterval(
        this.notifyRandomTrack.bind(this),
        messageRate
      );
    }
  };
  ...
  this.getRandomTrackUri = function () {
    return Spotify.TracksData[
      Math.floor(Math.random() * Spotify.TracksData.length)
    ];
  };
  ...
  this.notifyRandomTrack = function () {
    ...
    request.open(
      "GET",
      "http://ws.spotify.com/lookup/1/.json?uri=" + this.getRandomTrackUri()
    );
    request.send();
  };
  ...
  this.setMessageRate = function (buttonId) {
    if (buttonId != this.lastRateButtonSelected) {
      document
        ...
        .classList.add(Spotify.MessageRateSelector.getButtonClass(buttonId));
      if (this.lastRateButtonSelected) {
        document
          .getElementById(this.lastRateButtonSelected)
          .classList.remove(
            Spotify.MessageRateSelector.getButtonClass(
              this.lastRateButtonSelected
            )
          );
      }
      this.lastRateButtonSelected = buttonId;
      this.setupMessageInterval(
        Spotify.MessageRateSelector.getButtonRate(buttonId)
      );
    }
  };

  this.listenForRateChanges = function () {
    document.getElementById("rate-selector").addEventListener(
      "click",
      function (event) {
