...
type RecursivePartial<T> = {
  [P in keyof T]?: T[P] extends Set<infer U>
    ? RecursivePartial<U>[]
    : T[P] extends (infer U)[]
    ? RecursivePartial<U>[]
    : T[P] extends object
...
) => Promise<null | RecursivePartial<GitpodConfig>> = require("probot-config");
const createScheduler: (
  app: Application
) => Scheduler = require("probot-scheduler");
...
class Gitpod {
  ...
  protected async findIssues(q: string): Promise<Issue[][]> {
    const { github } = this.context;
    const pages: Issue[][] = [];
    let response: IssueSearchResponse = await github.search.issues({
      q,
      sort: "updated",
      per_page: 100,
    });
...
export = (app: Application) => {
  ...
  app.on("issues.opened", async (context) => {
    ...
    const { number } = issue;
    issue.labels = (
      await context.github.issues.getIssueLabels(context.repo({ number }))
    ).data;
