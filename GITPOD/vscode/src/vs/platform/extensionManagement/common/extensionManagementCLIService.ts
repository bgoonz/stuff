	...
	public async installExtensions(extensions: (string | URI)[], builtinExtensionIds: string[], isMachineScoped: boolean, force: boolean, output: CLIOutput = console): Promise<void> {
		...
		for (const extension of extensions) {
			if (extension instanceof URI) {
				vsixs.push({ uri: extension, installOptions: { isBuiltin: false, isMachineScoped } });
			} else {
		...
		if (vsixs.length) {
			await Promise.all(vsixs.map(async vsix => {
				try {
					const manifest = await this.installVSIX(vsix.uri, vsix.installOptions, force, output);
					if (manifest) {
