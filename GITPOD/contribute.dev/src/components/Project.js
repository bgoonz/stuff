...
const icons = importAll(
  require.context("../resources/projects/", false, /\.(png|jpe?g|svg)$/)
)

const Project = (props) => {
  const url = props.repo
  const platform = url.includes("github")
    ? "github"
    : url.includes("gitlab")
    ? "gitlab"
    : null
    ...
    <Styled>
      <div className="img-container">
        <a
          ...
          target="_blank"
          rel="noopener"
          aria-label={`${props.title} repository`}
        >
          {props.logo ? (
            <img src={icons[props.logo]} alt={props.title} className="img" />
          ) : null}
        </a>
      </div>
      <div className="text">
      ...
      <div className="tags">
        ...
        {(
          ...
          <a href={tag.href} key={i} aria-label={tag.alt}>
            <img className="tag__img" src={tag.src} alt={tag.alt} />
          </a>
        ))}
      </div>
      <div className="call-to-action">
          ...
          <div className="stars">
            <img
              src={
                ...
                (platform === "gitlab" &&
                  `https://img.shields.io/badge/dynamic/json?color=green&label=gitlab%20stars&query=star_count&url=https://gitlab.com/api/v4/projects/${props.gitLabId}`)
              }
              alt={props.title}
              style={{ height: 20 }}
            />
        ...
        <a
          href={`https://gitpod.io/#${props.link ? props.link : url}`}
          target="_blank"
          rel="noopener"
        >
