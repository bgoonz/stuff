...
const GITHUB_PROJECTS_JSON_PATH = path.join(
  __dirname,
  "..",
...
;(async () => {
  ...
  const { projects } = JSON.parse(
    await readFile(GITHUB_PROJECTS_JSON_PATH, "utf-8")
  )
  ...
  const existing = projects.find(
    (project) =>
      ...
      `https://github.com/${project.repoName}`.toLowerCase()
  )
  if (existing) {
    console.log(
      `[SKIPPED] Repository already in ${GITHUB_PROJECTS_JSON}: ${JSON.stringify(
        existing,
        null,
        4
      )}`
  ...
  try {
    ...
    const githubData = await fetchGitHubAPI(
    ...
    for (const p of projects) {
      const data = await fetchGitHubAPI(
        `https://api.github.com/repos/${p.repoName}`
      )
      if (typeof data.stargazers_count !== "number") {
        throw new Error(
          `Could not fetch stars! repoName=${
            p.repoName
          }, stars=${JSON.stringify(data.stargazers_count)}`
        )
      }
    ...
    await writeFile(
      GITHUB_PROJECTS_JSON_PATH,
      JSON.stringify({ projects }, null, 4),
      "utf-8"
    )
    console.log(
      `[OK] Succesfully added new project: ${JSON.stringify(project, null, 4)}`
    )
  } catch (error) {
    ...
    https
      .get(url, { headers }, (res) => {
        ...
        res.on("end", () => {
          ...
          const nextUrl = (res.headers.link || "")
            .split(",")
