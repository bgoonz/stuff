...
dazzle is a rather experimental Docker/OCI image builder. Its goal is to build independent layers where a change to one layer does _not_ invalidate the ones sitting "above" it.
...
While of limited practical use, it is _possible_ to run the test runner standalone using a base64-encoded JSON blob as a parameter:
