module.exports = ({
  ...
  function parseSelector(selector) {
    ...
    return [selectorList.filter(s => s.includes(':hover')), selectorList.filter(s => !s.includes(':hover'))];
  }
  ...
  return {
    ...
    Rule(rule, {
      ...
      let [hoverSelectorList, nonHoverSelectorList] = parseSelector(rule.selector);
      let mediaQuery = createMediaQuery(rule.clone({
        selectors: hoverSelectorList
      }), {
        AtRule
      });
      ...
      if (nonHoverSelectorList.length) {
        rule.replaceWith(rule.clone({
          selectors: nonHoverSelectorList
        }));
        return;
      }
