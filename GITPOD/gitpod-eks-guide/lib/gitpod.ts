...
export class GitpodStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props: GitpodProps) {
    ...
    if (process.env.IMAGE_PULL_SECRET_FILE) {
      createNestedObject(
        values,
        ["components", "imageBuilderMk3", "registry"],
        {}
      );
      values.components.imageBuilderMk3.registry.secretName =
        "gitpod-image-pull-secret";
    }

    if (process.env.IMAGE_REGISTRY_WHITELIST) {
      createNestedObject(
        values,
        ["components", "server", "defaultBaseImageRegistryWhitelist"],
        []
      );
      process.env.IMAGE_REGISTRY_WHITELIST.split(",").forEach(
        (registry: string) => {
          values.components.server.defaultBaseImageRegistryWhitelist.push(
            registry
          );
        }
      );
    ...
    if (props.certificateArn) {
      ...
      manifest.metadata.annotations["alb.ingress.kubernetes.io/ssl-policy"] =
        "ELBSecurityPolicy-FS-1-2-Res-2020-10";
    }

    manifest.metadata.annotations[
      "alb.ingress.kubernetes.io/load-balancer-name"
    ] = `${process.env.CLUSTER_NAME}-${props.env?.region}`;

    // if we have a route53 zone ID, enable external-dns.
    if (process.env.ROUTE53_ZONEID) {
      manifest.metadata.annotations[
        "external-dns.alpha.kubernetes.io/hostname"
      ] = `${props.domain}, *.${props.domain}, *.ws.${props.domain}`;
    }

    if (
      process.env.USE_INTERNAL_ALB &&
      process.env.USE_INTERNAL_ALB.toLowerCase() === "true"
    ) {
      manifest.metadata.annotations["alb.ingress.kubernetes.io/scheme"] =
        "internal";
    } else {
      manifest.metadata.annotations["alb.ingress.kubernetes.io/scheme"] =
        "internet-facing";
