...
build(context, version).catch((err) => {
...
export async function build(context, version) {
  ...
  try {
    ...
    if (k3sWsCluster) {
      ...
      shell
        .exec(
          "kubectl get secret k3sdev -n werft -o=go-template='{{index .data \"k3s-external.yaml\"}}' | base64 -d > k3s-external.yaml"
        )
        .trim();
    }
    werft.done("prep");
  ...
  const workspaceFeatureFlags = (buildConfig["ws-feature-flags"] || "")
    .split(",")
    .map((e) => e.trim());
  ...
  exec(
    `leeway build --werft=true -c ${cacheLevel} ${
      dontTest ? "--dont-test" : ""
  ...
  if (withInstaller || publishRelease) {
    exec(
      `leeway build --werft=true -c ${cacheLevel} ${
        dontTest ? "--dont-test" : ""
      } -Dversion=${version} -DimageRepoBase=${imageRepo} install:all`
    );
  }
  exec(
    `leeway build --werft=true -c ${cacheLevel} ${retag} --coverage-output-path=${coverageOutput} -Dversion=${version} -DremoveSources=false -DimageRepoBase=${imageRepo} -DlocalAppVersion=${localAppVersion} -DnpmPublishTrigger=${
  ...
  if (publishRelease) {
    try {
      ...
      if (!semver.valid(version)) {
        ...
        throw new Error(
          `'${version}' is not semver compliant and thus cannot used for Self-Hosted releases!`
        );
      }
      ...
      exec(
        `leeway build --werft=true chart:release-tars -Dversion=${version} -DimageRepoBase=${imageRepo} --save ${releaseFilesTmpDir}/${releaseTarName}`
      );
      exec(
  ...
  try {
    ...
    for (let index = 0; index < coverageFiles.length; index++) {
      ...
      exec(
        `codecov -N "${parent_commit}" --flags=${flag} --file "${coverageOutput}/${file}"`
      );
    }
...
export async function deployToDev(
  ...
  const { version, destname, namespace, domain, url, k3sWsCluster } =
    deploymentConfig;
  ...
  const [wsdaemonPortK3sWs, registryNodePortK3sWs] = !k3sWsCluster
    ? [0, 0]
    : findFreeHostPorts(
        getK3sWsKubeConfigPath(),
        [
          { start: 10000, end: 11000 },
          { start: 30000, end: 31000 },
        ],
  ...
  if (k3sWsCluster) {
    try {
      ...
      const existingIP = exec(
        `gcloud compute addresses describe ${namespace} --region europe-west1 | grep 'address:' | cut -c 10-`,
        { silent: true }
      ).trim();
      werft.log(
        "certificate",
        `The gcloud compute address describe returned: ${existingIP}`
      );
      if (existingIP.indexOf("ERROR:") == -1 && existingIP != "") {
        k3sWsProxyIP = existingIP;
        werft.log(
          "certificate",
          `found existing ip named ${namespace} and value: ${k3sWsProxyIP}`
        );
      } else {
        exec(
          `gcloud compute addresses create ${namespace} --region europe-west1`
        );
        const newIP = exec(
          `gcloud compute addresses describe ${namespace} --region europe-west1 | grep 'address:' | cut -c 10-`
        ).trim();
        ...
        werft.log(
          "certificate",
          `created new ip named ${namespace} and value: ${k3sWsProxyIP}`
        );
      }
    } catch (err) {
  ...
  try {
    ...
    if (k3sWsCluster) {
      setKubectlContextNamespace(getK3sWsKubeConfigPath(), namespace, {
        slice: "prep",
      });
    }
  ...
  try {
    const auth = exec(
      `echo -n "_json_key:$(kubectl get secret gcp-sa-registry-auth --namespace=keys -o yaml \
  ...
  function installGitpod(commonFlags: string) {
    ...
    if (storage === "gcp") {
      exec(
        "kubectl get secret gcp-sa-cloud-storage-dev-sync-key -n werft -o yaml | yq d - metadata | yq w - metadata.name remote-storage-gcloud | kubectl apply -f -"
      );
    ...
    exec(
      `/usr/local/bin/helm3 upgrade --install --timeout 10m -f ../.werft/values.dev.yaml ${flags} ${helmInstallName} .`
    );
    ...
    exec(
      `/usr/local/bin/helm3 upgrade --install --set image.version=${sweeperVersion} --set command="werft run github -a namespace=${namespace} --remote-job-path .werft/wipe-devstaging.yaml github.com/gitpod-io/gitpod:main" sweeper ../dev/charts/sweeper`
    );
  }

  function installGitpodOnK3sWsCluster(
    commonFlags: string,
    ...
    if (storage === "gcp") {
      ...
      exec(
        "kubectl get secret gcp-sa-cloud-storage-dev-sync-key -n werft -o yaml | yq d - metadata | yq w - metadata.name remote-storage-gcloud > remote-storage-gcloud.yaml"
      );
      // After storing the yaml we apply it to the k3s cluster
      exec(
        `export KUBECONFIG=${pathToKubeConfig} && kubectl apply -f remote-storage-gcloud.yaml`
      );
      flags += ` -f ../.werft/values.dev.gcp-storage.yaml`;
    }
    ...
    exec(
      `export KUBECONFIG=${pathToKubeConfig} && /usr/local/bin/helm3 upgrade --install --timeout 10m -f ../.werft/values.k3sWsCluster.yaml ${flags} ${helmInstallName} .`
    );
  }
  ...
  function addDeploymentFlags() {
    ...
    if (deploymentConfig.withPayment) {
      ...
      exec(
        `cp /mnt/secrets/payment-webhook-config/webhook payment-core-dev-webhook.json`
      );
  ...
  async function cleanStateEnv(pathToKubeConfig: string) {
    await wipeAndRecreateNamespace(
      pathToKubeConfig,
      helmInstallName,
      namespace,
      { slice: "prep" }
    ...
    try {
      deleteNonNamespaceObjects(pathToKubeConfig, namespace, destname, {
...
export async function triggerIntegrationTests(
  ...
  try {
    const imageVersion = exec(
      `docker run --rm eu.gcr.io/gitpod-core-dev/build/versions:${version} cat /versions.yaml | yq r - 'components.integrationTest.version'`,
      { silent: true }
    ).stdout.trim();
...
async function registerK3sWsCluster(
  namespace: string,
  domain: string,
  pathToKubeConfigMeta: string,
  pathToKubeConfigK3s: string
) {
  ...
  shell.exec(
    `export KUBECONFIG=${pathToKubeConfigK3s} && kubectl rollout restart deploy ws-proxy -n ${namespace}`
  );
  waitForDeploymentToSucceed(
    pathToKubeConfigK3s,
    "ws-proxy",
  ...
  returnedValue = gpctl.registerCluster(
    pathToKubeConfigMeta,
    "k3s",
    wsProxyUrl
  );
  werft.log(phases.REGISTER_K3S_WS_CLUSTER, returnedValue);
