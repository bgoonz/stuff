...
export async function wipePreviewEnvironment(
  pathToKubeConfig: string,
  helmInstallName: string,
  namespace: string,
  shellOpts: ExecOptions
...
function uninstallHelm(
  ...
  const installations = exec(
    `export KUBECONFIG=${pathToKubeConfig} && helm --namespace ${namespace} list -q`,
    { silent: true, dontCheckRc: true }
  )
    .stdout.split("\n")
    .map((o) => o.trim())
    .filter((o) => o.length > 0);
  ...
  exec(
    `export KUBECONFIG=${pathToKubeConfig} && helm --namespace ${namespace} delete ${installationName}`,
    shellOpts
  );
}
...
function deleteAllWorkspaces(
  ...
  const objs = exec(
    `export KUBECONFIG=${pathToKubeConfig} && kubectl get pod -l component=workspace --namespace ${namespace} --no-headers -o=custom-columns=:metadata.name`
  )
    .split("\n")
    .map((o) => o.trim())
    .filter((o) => o.length > 0);
  ...
  objs.forEach((o) => {
    try {
      ...
      exec(
        `export KUBECONFIG=${pathToKubeConfig} && kubectl patch pod --namespace ${namespace} ${o} -p '{"metadata":{"finalizers":null}}'`,
        { ...shellOpts }
      );
      const result = exec(
        `export KUBECONFIG=${pathToKubeConfig} && kubectl delete pod --namespace ${namespace} ${o} --ignore-not-found=true --timeout=10s`,
      ...
      if (result.code !== 0) {
        ...
        exec(
          `export KUBECONFIG=${pathToKubeConfig} && kubectl delete pod --namespace ${namespace} ${o} --ignore-not-found=true --force`,
          { ...shellOpts }
        );
      }
    } catch (err) {
      const result = exec(
        `export KUBECONFIG=${pathToKubeConfig} && kubectl get pod --namespace ${namespace} ${o}`,
        { dontCheckRc: true }
      );
      if (result.code === 0) {
        console.error(
...
async function deleteAllUnnamespacedObjects(
  ...
  for (const resType of [
    ...
    const objs = exec(
      `export KUBECONFIG=${pathToKubeConfig} && kubectl get ${resType} --no-headers -o=custom-columns=:metadata.name`
    )
      .split("\n")
      .map((o) => o.trim())
      .filter((o) => o.length > 0)
    ...
    for (const obj of objs) {
      promisedDeletes.push(
        exec(
          `export KUBECONFIG=${pathToKubeConfig} && kubectl delete ${resType} ${obj}`,
          { slice, async: true }
        )
      );
    }
...
export function createNamespace(
  ...
  const result = exec(
    `export KUBECONFIG=${pathToKubeConfig} && kubectl get namespace ${namespace}`,
...
export function listAllPreviewNamespaces(pathToKubeConfig: string): string[] {
  return exec(
    `export KUBECONFIG=${pathToKubeConfig} && kubectl get namespaces -l ${IS_PREVIEW_APP_LABEL}=true -o=custom-columns=:metadata.name`,
    { silent: true }
  )
    .stdout.split("\n")
    .map((o) => o.trim())
...
export function deleteNonNamespaceObjects(
  pathToKubeConfig: string,
  namespace,
  destname,
  shellOpts
  ...
  exec(
    `export KUBECONFIG=${pathToKubeConfig} && /usr/local/bin/helm3 delete gitpod-${destname} || echo gitpod-${destname} was not installed yet`,
  ...
  [
    ...
    ["ClusterRole", "ClusterRoleBinding", "PodSecurityPolicy"].forEach((kind) =>
      exec(
        `export KUBECONFIG=${pathToKubeConfig} && kubectl get ${kind} -l component=${comp} --no-headers -o=custom-columns=:metadata.name | grep ${namespace}-ns`,
        { dontCheckRc: true }
      )
        .split("\n")
        .map((o) => o.trim())
  ...
  objs.forEach((o) => {
    exec(
      `export KUBECONFIG=${pathToKubeConfig} && kubectl delete ${o.kind} ${o.obj}`,
      shellOpts
    );
  });
}
...
export function findFreeHostPorts(
  ...
  const hostPorts: number[] = exec(
    `export KUBECONFIG=${pathToKubeConfig} && kubectl get pods --all-namespaces -o yaml | yq r - 'items.*.spec.containers.*.ports.*.hostPort'`,
    { silent: true }
  )
    .stdout.split("\n")
    .map((l) => l.trim())
...
export function waitForDeploymentToSucceed(
  ...
  exec(
    `export KUBECONFIG=${pathToKubeConfig} && kubectl rollout status ${type} ${name} -n ${namespace}`
  );
}
