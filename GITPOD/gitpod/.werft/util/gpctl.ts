...
export function printClustersList(pathToKubeConfig: string): string {
  let cmd =
    `/workspace/dev/gpctl/gpctl clusters list` +
    buildRequiredFlags(pathToKubeConfig);
  const result = shell.exec(cmd).trim();
  return result;
}

export function uncordonCluster(
  pathToKubeConfig: string,
  name: string
): string {
  let cmd =
    `/workspace/dev/gpctl/gpctl clusters uncordon --name=${name}` +
    buildRequiredFlags(pathToKubeConfig);
  const result = shell.exec(cmd).trim();
  return result;
}
...
export function registerCluster(
  ...
  let cmd =
    `/workspace/dev/gpctl/gpctl clusters register \
	--name ${name} \
	--hint-cordoned \
	--hint-govern \
	--tls-path ./wsman-tls \
...
export function getClusterTLS(pathToKubeConfig: string): string {
  let cmd =
    `/workspace/dev/gpctl/gpctl clusters get-tls-config` +
    buildRequiredFlags(pathToKubeConfig);
  const result = shell.exec(cmd).trim();
  return result;
}
