...
export class SupervisorServiceClient {
  ...
  private async checkReady(
    ...
    try {
      ...
      const wsSupervisurStatusUrl = GitpodHostUrl.fromWorkspaceUrl(
      ...
      const response = await fetch(wsSupervisurStatusUrl.toString(), {
        credentials: "include",
      });
      let result;
      if (response.ok) {
        ...
        if (
          kind === "supervisor" &&
          (result as SupervisorStatusResponse.AsObject).ok
        ) {
          return;
        }
        if (
          kind === "content" &&
          (result as ContentStatusResponse.AsObject).available
        ) {
          return;
        }
