...
export const track = (w: Window) => {
  w.document.addEventListener("mousemove", updateLastActivitiy, {
    capture: true,
  });
  w.document.addEventListener("keydown", updateLastActivitiy, {
    capture: true,
  });
};
...
export function schedule(instanceId: string): void {
  ...
  const sendHeartBeat = async (wasClosed?: true) => {
    try {
      await window.gitpod.service.server.sendHeartBeat({
        instanceId,
        wasClosed,
      });
    } catch (err) {
