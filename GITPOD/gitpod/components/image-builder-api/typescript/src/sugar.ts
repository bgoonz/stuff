...
@injectable()
export class CachingImageBuilderClientProvider
  implements ImageBuilderClientProvider
{
  @inject(ImageBuilderClientConfig)
  protected readonly clientConfig: ImageBuilderClientConfig;
...
export class PromisifiedImageBuilderClient {
  ...
  public resolveBaseImage(
    ctx: TraceContext,
    request: ResolveBaseImageRequest
  ): Promise<ResolveBaseImageResponse> {
    return new Promise<ResolveBaseImageResponse>((resolve, reject) => {
      const span = TraceContext.startSpan(
        `/image-builder/resolveBaseImage`,
        ctx
      );
      this.client.resolveBaseImage(
        request,
        withTracing({ span }),
        (err, resp) => {
          if (err) {
  ...
  public resolveWorkspaceImage(
    ...
    return new Promise<ResolveWorkspaceImageResponse>((resolve, reject) => {
      const span = TraceContext.startSpan(
        `/image-builder/resolveWorkspaceImage`,
        ctx
      );
  ...
  public build(
    ...
    try {
      ...
      stream.on("end", () => {
        ...
        let spanFinished =
          (result.isResolved && !resultResp.actuallyNeedsBuild) ||
          buildResult.isResolved;
        if (!result.isResolved) {
          result.reject(err);
  ...
  public logs(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      stream.on("data", (resp: LogsResponse) => {
        if (
          cb(new TextDecoder("utf-8").decode(resp.getContent_asU8())) === "stop"
        ) {
          stream.cancel();
        }
      });
