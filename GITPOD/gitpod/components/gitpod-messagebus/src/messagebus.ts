...
export class MessageBusHelperImpl implements MessageBusHelper {
  ...
  static async assertPrebuildWorkspaceUpdatableQueue(
    ch: Channel
  ): Promise<void> {
...
export abstract class AbstractMessageBusIntegration {
  ...
  @inject(MessagebusConfiguration)
  protected readonly config: MessagebusConfiguration;
  @inject(MessageBusHelper)
  ...
  async connect(
    ...
    if (!this.connectionAttempt) {
      ...
      this.connectionAttempt = this.doConnectWithRetries(
        retries,
    ...
    for (var retry = 0; retry < retries; retry++) {
      try {
        ...
        await new Promise((resolve, _) =>
          setTimeout(resolve, reconnectTimeoutInMs)
        );
      }
    }
  ...
  protected async doConnect(
    ...
    if (!this.connection) {
      const connection = await connect(this.connectOptions, {
  ...
  protected scheduleReconnect(reconnectTimeoutInMs: number) {
    ...
    const timeout = Math.min(
      reconnectTimeoutInMs +
        Math.floor((Math.random() * reconnectTimeoutInMs) / 10),
...
export abstract class AbstractTopicListener<T> implements MessagebusListener {
  ...
  constructor(
    ...
    protected readonly listener: TopicListener<T>
  ) {}
  ...
  protected handleMessage(message: Message | null) {
    ...
    if (msg) {
      const spanCtx = globalTracer().extract(
        FORMAT_HTTP_HEADERS,
        message.properties.headers
      );
      const span = !!spanCtx
        ? globalTracer().startSpan(`/messagebus/${this.exchangeName}`, {
