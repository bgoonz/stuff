...
export class WorkspaceManagerBridgeEE extends WorkspaceManagerBridge {
  ...
  protected async updatePrebuiltWorkspace(
    ...
    try {
      const prebuild = await this.workspaceDB
        .trace({ span })
        .findPrebuildByWorkspaceID(status.metadata!.metaId!);
      if (!prebuild) {
        log.warn(logCtx, "headless workspace without prebuild");
        TraceContext.logError(
          { span },
          new Error("headless workspace without prebuild")
        );
        return;
      }
      ...
      if (status.phase === WorkspacePhase.STOPPING) {
        let headlessUpdateType: HeadlessWorkspaceEventType =
          HeadlessWorkspaceEventType.Aborted;
  ...
  protected async controlPrebuildInstance(
    ...
    const prebuild = await this.workspaceDB
      .trace({})
      .findPrebuildByWorkspaceID(instance.workspaceId);
