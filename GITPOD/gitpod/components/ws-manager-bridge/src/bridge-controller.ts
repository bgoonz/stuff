...
export class BridgeController {
    ...
    return this.reconcileQueue.enqueue(async () => {
      ...
      try {
        for (const [name, bridge] of this.bridges) {
          ...
          if (!cluster) {
            log.info("reconcile: cluster not present anymore, stopping", {
              name,
            });
            bridge.stop();
            toDelete.push(name);
          } else {
            log.info("reconcile: cluster already present, doing nothing", {
              name,
            });
  ...
  protected async createAndStartBridge(
    cluster: WorkspaceClusterInfo
  ): Promise<WorkspaceManagerBridge> {
  ...
  protected async getAllWorkspaceClusters(): Promise<
    Map<string, WorkspaceClusterWoTLS>
  > {
...
@injectable()
export class WorkspaceManagerClientProviderConfigSource
  implements WorkspaceManagerClientProviderSource
{
  @inject(Configuration)
  protected readonly config: Configuration;

  public async getWorkspaceCluster(
    name: string
  ): Promise<WorkspaceCluster | undefined> {
