...
export class MessageBusIntegration extends AbstractMessageBusIntegration {
  ...
  async connect(): Promise<void> {
    ...
    await this.channel.assertExchange(
      MessageBusIntegration.WORKSPACE_EXCHANGE,
      "topic",
      { durable: false }
    );
    await this.channel.assertExchange(
      MessageBusIntegration.LOCAL_WORKSPACE_EXCHANGE,
      "topic",
      { durable: false }
    );
  ...
  listenForWorkspaceStatusUpdates(
    ...
    const listener = new WorkspaceStatusUpdateListener(
      callback,
      MessageBusIntegration.WORKSPACE_EXCHANGE,
      topic
    );
