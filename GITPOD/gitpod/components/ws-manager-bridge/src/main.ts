...
export const start = async (container: Container) => {
  try {
    ...
    const clusterServiceServer =
      container.get<ClusterServiceServer>(ClusterServiceServer);
    await clusterServiceServer.start();

    const metaInstanceController = container.get<MetaInstanceController>(
      MetaInstanceController
    );
