...
export class MetaInstanceController {
    ...
    await Promise.all(
      instances.map(async (instance: RunningWorkspaceInfo) => {
        ...
        try {
          log.debug(
            logContext,
            "MetaInstanceController: Checking for workspaces to stop"
          );

          const now = Date.now();
          const creationTime = new Date(
            instance.latestInstance.creationTime
          ).getTime();
          const timedOutInPreparing =
            now >=
            creationTime + this.config.timeouts.preparingPhaseSeconds * 1000;
          const timedOutInUnknown =
            now >=
            creationTime + this.config.timeouts.unknownPhaseSeconds * 1000;
          ...
          if (
            ...
            log.info(
              logContext,
              "MetaInstanceController: Setting workspace instance to stopped",
              {
                creationTime,
                currentPhase,
              }
          ...
          log.error(
            logContext,
            "MetaInstanceController: Error whilst stopping workspace instance",
            err
          );
        }
      })
  ...
  public start() {
    log.debug(
      "MetaInstanceController: Starting interval to check for workspaces to stop",
      {
