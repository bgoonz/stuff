...
import {
  ...
  TracedUserDB,
  DBWithTracing,
} from "@gitpod/gitpod-db/lib/traced-db";
...
export const WorkspaceManagerBridgeFactory = Symbol(
  "WorkspaceManagerBridgeFactory"
);
...
export type WorkspaceClusterInfo = Pick<
  WorkspaceCluster,
  "name" | "url" | "govern"
>;
...
export class WorkspaceManagerBridge implements Disposable {
  ...
  protected async startDatabaseUpdater(
  ...
  protected serializeMessagesByInstanceId<M>(
    ...
    q.enqueue(() => handler(ctx, msg)).catch((e) =>
      log.error({ instanceId }, e)
    );
  ...
  protected async handleStatusUpdate(
    ...
    try {
      ...
      if (!instance) {
        log.warn(logCtx, "Received a status update for an unknown instance", {
          status,
        });
        return;
      }
      ...
      if (
        !instance.status.conditions.firstUserActivity &&
        status.conditions.firstUserActivity
      ) {
        // Only report this when it's observed the first time
        const firstUserActivity = mapFirstUserActivity(
          rawStatus.getConditions()!.getFirstUserActivity()
        )!;
        this.prometheusExporter.observeFirstUserActivity(
          instance,
          firstUserActivity
        );
      }
      ...
      instance.status.conditions.pullingImages = toBool(
        status.conditions.pullingImages!
      );
      instance.status.conditions.serviceExists = toBool(
        status.conditions.serviceExists!
      );
      ...
      instance.status.nodeName =
        instance.status.nodeName || status.runtime?.nodeName;
      instance.status.podName =
        instance.status.podName || status.runtime?.podName;
      ...
      switch (status.phase) {
        ...
        case WorkspacePhase.STOPPED:
          ...
          lifecycleHandler = () =>
            this.onInstanceStopped({ span }, userId, instance);
          break;
      }
  ...
  protected startController(
    ...
    const timer = setInterval(async () => {
      try {
        ...
        await this.controlInstallationInstances(
          client,
          installation,
        ...
        if (
          durationLongerThanSeconds(
            disconnectStarted,
            controllerMaxDisconnectSeconds
          )
        ) {
  ...
  protected async controlInstallationInstances(
    ...
    const runningInstances = await this.workspaceDB
      .trace(ctx)
      .findRunningInstancesWithWorkspaces(installation);
    const runningInstancesIdx = new Map<string, RunningWorkspaceInfo>();
    runningInstances.forEach((i) =>
      runningInstancesIdx.set(i.latestInstance.id, i)
    );

    const actuallyRunningInstances = await client.getWorkspaces(
      ctx,
      new GetWorkspacesRequest()
    );
    actuallyRunningInstances
      .getStatusList()
      .forEach((s) => runningInstancesIdx.delete(s.getId()));
  ...
  protected async cleanupProbeWorkspace(
    ctx: TraceContext,
    status: WorkspaceStatus.AsObject | undefined
  ) {
  ...
  protected async controlPrebuildInstance(
  ...
  protected async onInstanceStopped(
    ...
    try {
      await this.userDB
        .trace({ span })
        .deleteGitpodTokensNamedLike(ownerUserID, `${instance.id}-%`);
...
const mapFirstUserActivity = (
  firstUserActivity: Timestamp | undefined
): string | undefined => {
...
const mapPortVisibility = (
  visibility: WsManPortVisibility | undefined
): PortVisibility | undefined => {
  switch (visibility) {
    case undefined:
      return undefined;
...
const durationLongerThanSeconds = (
  time: number,
