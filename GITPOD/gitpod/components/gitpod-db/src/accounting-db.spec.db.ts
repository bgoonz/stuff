...
export class AccountingDBSpec {
  ...
  @test public async subscriptionFixedPeriod() {
    ...
    const inBetween = new Date(
      0.5 * (new Date(now).getTime() + new Date(later).getTime())
    ).toISOString();
    ...
    // date inclusion
    expectExactlyOne(
      await this.db.findActiveSubscriptions(now, now),
      subscription
    );
    expectExactlyOne(
      await this.db.findActiveSubscriptions(now, inBetween),
      subscription
    );
    expectExactlyOne(
      await this.db.findActiveSubscriptions(inBetween, later),
      subscription
    );
    expectExactlyOne(
      await this.db.findActiveSubscriptions(now, later),
      subscription
    );
    expectExactlyOne(
      await this.db.findActiveSubscriptions(
        rightBefore(later),
        rightBefore(later)
      ),
      subscription
    ...
    expect(
      await this.db.findActiveSubscriptions(rightBefore(now), rightBefore(now))
    ).to.be.empty;
  }
  ...
  @test public async subscriptionOpenEnd() {
    ...
    expectExactlyOne(
      await this.db.findActiveSubscriptions(later, later),
      subscription
    );
    expectExactlyOne(
      await this.db.findActiveSubscriptions(now, inBetween),
  ...
  @test public async subscriptionUpdate() {
    ...
    expectExactlyOne(
      await this.db.findActiveSubscriptions(now, rightAfter(later)),
      subscription
    );
    expect(
      await this.db.findActiveSubscriptions(rightBefore(now), rightBefore(now))
  ...
  @test public async subscriptionsForUser() {
    ...
    expectExactlyOne(
      await this.db.findActiveSubscriptionsForUser(subscription.userId, now),
      subscription
    );
    expect(
      await this.db.findActiveSubscriptionsForUser(
        subscription.userId,
        rightBefore(now)
      )
    ).to.be.an("array").and.empty;
    expectExactlyOne(
      await this.db.findActiveSubscriptionsForUser(subscription.userId, later),
      subscription
    );
    Subscription.cancelSubscription(dbSubscription, later);
    await this.db.storeSubscription(dbSubscription);
