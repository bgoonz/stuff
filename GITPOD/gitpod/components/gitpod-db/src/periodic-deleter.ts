...
export class PeriodicDbDeleter {
  ...
  start() {
    ...
    this.sync().catch((err) =>
      log.error("[PeriodicDbDeleter] sync failed", err)
    );
  }
  ...
  protected async collectRowsToBeDeleted(
    table: TableDescription
  ): Promise<{ table: string; deletions: string[] }> {
    try {
    ...
    const whereClauseFn = (row: any) =>
      primaryKeys.map((pk) => `${pk}='${row[pk]}'`).join(" AND ");
    for (const i in rows) {
      const row = rows[i];
