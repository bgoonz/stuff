...
import {
  ...
  DBPaymentSourceInfo,
} from "./typeorm/entity/db-subscription";
...
export interface AccountingDB {
  ...
  findSubscriptionsForUserInPeriod(
    userId: string,
    fromDate: string,
    toDate: string
  ): Promise<Subscription[]>;
  findNotYetCancelledSubscriptions(
    userId: string,
    date: string
  ): Promise<Subscription[]>;
  findSubscriptionForUserByPaymentRef(
    userId: string,
    paymentReference: string
  ): Promise<Subscription[]>;
  findSubscriptionsByTeamSubscriptionSlotId(
  ...
  findSubscriptionAdditionalData(
    paymentReference: string
  ...
  storePaymentSourceInfo(
    cardInfo: DBPaymentSourceInfo
  ): Promise<DBPaymentSourceInfo>;
}

export type DBPaymentSourceInfoPartial = DeepPartial<DBPaymentSourceInfo> &
  Pick<DBPaymentSourceInfo, "id">;
