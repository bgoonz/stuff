...
export interface UserDB extends OAuthUserRepository, OAuthTokenRepository {
  ...
  findIdentitiesByName(
    identity: Pick<Identity, "authProviderId" | "authName">
  ): Promise<Identity[]>;
   ...
   */
  storeSingleToken(
    identity: Pick<Identity, "authProviderId" | "authId">,
    token: Token
  ): Promise<TokenEntry>;
  ...
  deleteGitpodTokensNamedLike(
    userId: string,
    namePattern: string
  ): Promise<void>;
}
export type PartialUserUpdate = Partial<Omit<User, "identities">> &
  Pick<User, "id">;
...
export type UserEmailContact = Pick<
  User,
  "id" | "name" | "allowsMarketingCommunication"
> & {
  primaryEmail: string;
};
