...
import {
  TypeORMWorkspaceDBImpl,
  TransactionalWorkspaceDbImpl,
} from "./typeorm/workspace-db-impl";
...
import {
  KeyProviderImpl,
  KeyProviderConfig,
} from "@gitpod/gitpod-protocol/lib/encryption/key-provider";
import {
  DBWithTracing,
  bindDbWithTracing,
  TracedWorkspaceDB,
...
export const dbContainerModule = new ContainerModule(
  (bind, unbind, isBound, rebind) => {
    ...
    bind(KeyProviderConfig)
      .toDynamicValue((ctx) => {
        ...
        return {
          keys: KeyProviderImpl.loadKeyConfigFromJsonString(
            config.dbEncryptionKeys
          ),
        };
      })
      .inSingletonScope();
