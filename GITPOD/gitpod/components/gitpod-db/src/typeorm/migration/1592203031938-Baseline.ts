  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    // features repositories
    {
      const count = (
        await queryRunner.query(
          `SELECT COUNT(1) AS cnt FROM d_b_repository_white_list`
        )
      )[0].cnt;
      if (count < 1) {
        const entries = [
          ...
          },
          {
            ...
            description:
              "**JavaScript** - Simple PWA boilerplate with Next.js and Redux",
            priority: 8,
          },
          {
            url: "https://github.com/gitpod-io/django-locallibrary-tutorial",
            description:
              '**Python** - Tutorial "Local Library" website written in Django',
            priority: 10,
          },
          {
          ...
          },
          {
            ...
            description:
              "**Typescript** - Deep dive into Gitpod\\'s open-source IDE, Theia.",
            priority: 4,
          },
        ];
        await Promise.all(
    ...
    // domain filters
    {
      ...
      const values = entries
        .map((e) => `('${e.domain}', '${e.negative ? 1 : 0}')`)
        .join(",");
      await queryRunner.query(
        `INSERT IGNORE INTO d_b_email_domain_filter (domain, negative) VALUES ${values}`
      );
    }
    ...
    // probe user
    {
      ...
      if (!exists) {
        await queryRunner.query(
          `INSERT IGNORE INTO d_b_user (id, creationDate, avatarUrl, name, fullName) VALUES ('builtin-user-workspace-probe-0000000', '${new Date().toISOString()}', '', '${BUILTIN_WORKSPACE_PROBE_USER_NAME}', '')`
        );
      }
    }
  }
