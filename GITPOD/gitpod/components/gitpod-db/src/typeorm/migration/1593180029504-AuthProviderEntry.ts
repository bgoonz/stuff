...
export class AuthProviderEntry1593180029504 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<any> {
    if (!(await tableExists(queryRunner, "d_b_auth_provider_entry"))) {
      ...
      await queryRunner.query(
        "CREATE INDEX `ind_lastModified` ON `d_b_auth_provider_entry` (_lastModified)"
      );
      await queryRunner.query(
        "CREATE INDEX `ind_ownerId` ON `d_b_auth_provider_entry` (ownerId)"
      );
      await queryRunner.query(
        "CREATE INDEX `ind_host` ON `d_b_auth_provider_entry` (host)"
      );
      await queryRunner.query(
        "CREATE INDEX `ind_dbsync` ON `d_b_auth_provider_entry` (_lastModified)"
      );
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    if (await tableExists(queryRunner, "d_b_auth_provider_entry")) {
      await queryRunner.query(
        "ALTER TABLE `d_b_auth_provider_entry` DROP INDEX `ind_dbsync`"
      );
      await queryRunner.query(
        "ALTER TABLE `d_b_auth_provider_entry` DROP INDEX `ind_host`"
      );
      await queryRunner.query(
        "ALTER TABLE `d_b_auth_provider_entry` DROP INDEX `ind_ownerId`"
      );
      await queryRunner.query(
        "ALTER TABLE `d_b_auth_provider_entry` DROP INDEX `ind_lastModified`"
      );
