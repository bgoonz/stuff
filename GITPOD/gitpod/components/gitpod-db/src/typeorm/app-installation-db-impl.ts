...
export class TypeORMAppInstallationDBImpl implements AppInstallationDB {
  ...
  protected async findAndFinishInstallation(
    ...
    const repo = await this.getRepo();
    const installationRecords = await repo.find({
      where: { platform, installationID },
    });

    // maybe we're already done and have a finished installation
    const finishedInstallation = installationRecords.find(
      (r) => r.state == "installed"
    );
    ...
    const platformClaim = installationRecords.find(
      (r) => r.state == "claimed.platform"
    );
    const userClaim = installationRecords.find(
      (r) => r.state == "claimed.user"
    );
    if (!!platformClaim && !!userClaim) {
      ...
      obj.state = "installed";
      obj.ownerUserID =
        userClaim.ownerUserID || platformClaim.ownerUserID || undefined;
