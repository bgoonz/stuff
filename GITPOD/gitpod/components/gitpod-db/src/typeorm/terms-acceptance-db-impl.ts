...
export class TermsAcceptanceDBImpl implements TermsAcceptanceDB {
  ...
  protected async getTermsAcceptanceRepo(): Promise<
    Repository<DBTermsAcceptanceEntry>
  > {
    return (await this.getEntityManager()).getRepository(
      DBTermsAcceptanceEntry
    );
  }

  async getAcceptedRevision(
    userId: string
  ): Promise<TermsAcceptanceEntry | undefined> {
