...
export class TeamSubscriptionDBImpl implements TeamSubscriptionDB {
  ...
  async transaction<T>(
    code: (db: TeamSubscriptionDB) => Promise<T>
  ): Promise<T> {
  ...
  async storeTeamSubscriptionEntry(
    ts: TeamSubscription
  ): Promise<TeamSubscription> {
    return (await this.getRepo()).save(ts);
  }

  async findTeamSubscriptionById(
    id: string
  ...
  async findTeamSubscriptionsForUser(
    userId: string,
    date: string
  ): Promise<TeamSubscription[]> {
    const repo = await this.getRepo();
  ...
  async findTeamSubscriptions(
    partial: DeepPartial<TeamSubscription>
  ): Promise<TeamSubscription[]> {
    const repo = await this.getRepo();
  ...
  async findSlotsByTeamSubscriptionId(
