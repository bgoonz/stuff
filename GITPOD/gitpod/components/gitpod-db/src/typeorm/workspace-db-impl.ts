...
export abstract class AbstractTypeORMWorkspaceDBImpl implements WorkspaceDB {
  ...
  protected async getWorkspaceRepo(): Promise<Repository<DBWorkspace>> {
    ...
    ).getRepository<DBWorkspace>(DBWorkspace);
  }

  protected async getWorkspaceInstanceRepo(): Promise<
    Repository<DBWorkspaceInstance>
  > {
    return await (
      await this.getManager()
    ).getRepository<DBWorkspaceInstance>(DBWorkspaceInstance);
  }

  protected async getWorkspaceInstanceUserRepo(): Promise<
    Repository<DBWorkspaceInstanceUser>
  > {
    return await (
      await this.getManager()
    ).getRepository<DBWorkspaceInstanceUser>(DBWorkspaceInstanceUser);
  }

  protected async getRepositoryWhitelist(): Promise<
    Repository<DBRepositoryWhiteList>
  > {
    return await (
      await this.getManager()
    ).getRepository<DBRepositoryWhiteList>(DBRepositoryWhiteList);
  }

  protected async getSnapshotRepo(): Promise<Repository<DBSnapshot>> {
    return await (
      await this.getManager()
    ).getRepository<DBSnapshot>(DBSnapshot);
  }

  protected async getPrebuiltWorkspaceRepo(): Promise<
    Repository<DBPrebuiltWorkspace>
  > {
    return await (
      await this.getManager()
    ).getRepository<DBPrebuiltWorkspace>(DBPrebuiltWorkspace);
  }

  protected async getPrebuiltWorkspaceUpdatableRepo(): Promise<
    Repository<DBPrebuiltWorkspaceUpdatable>
  > {
    return await (
      await this.getManager()
  ...
  protected async getLayoutDataRepo(): Promise<Repository<DBLayoutData>> {
    return await (
      await this.getManager()
    ).getRepository<DBLayoutData>(DBLayoutData);
  }
  ...
  public async findRunningInstance(
    workspaceId: string
  ): Promise<MaybeWorkspaceInstance> {
  ...
  public async find(options: FindWorkspacesOptions): Promise<WorkspaceInfo[]> {
    ...
    const qb = repo
      ...
      .addOrderBy(
        "GREATEST(ws.creationTime, wsi.creationTime, wsi.startedTime, wsi.stoppedTime)",
        "DESC"
      )
      .limit(options.limit || 10);
    if (options.searchString) {
      qb.andWhere("ws.description LIKE :searchString", {
    ...
    if (options.projectId) {
      qb.andWhere("ws.projectId = :projectId", {
        projectId: options.projectId,
      });
    }

    const rawResults = (await qb.getMany()) as any as (Workspace & {
      latestInstance?: WorkspaceInstance;
    })[]; // see leftJoinAndMapOne above
  ...
  protected async augmentWithCurrentInstance(
  ...
  public async updateLastHeartbeat(
    ...
    workspaceInstanceUserRepo.query(query, [
      instanceId,
      userId,
      lastSeen,
      lastSeen,
  ...
  public async getLastOwnerHeartbeatFor(
    ...
    const workspaceInstanceUserRepo = await this.getWorkspaceInstanceUserRepo();
    const result = await workspaceInstanceUserRepo.query(query, [
      instance.id,
  ...
  public async getWorkspaceUsers(
    ...
    const minLastSeenString = this.toTimestampString(
      new Date(new Date().getTime() - minLastSeen)
    );
    const query =
  ...
  public async internalStoreInstance(
    instance: WorkspaceInstance
  ...
  public async findCurrentInstance(
    workspaceId: string
  ): Promise<MaybeWorkspaceInstance> {
    const workspaceInstanceRepo = await this.getWorkspaceInstanceRepo();
    workspaceInstanceRepo.findOneById(workspaceId, {});
    const qb = await workspaceInstanceRepo
      .createQueryBuilder("wsi")
      .where(`wsi.workspaceId = :workspaceId`, { workspaceId })
  ...
  public async findAllWorkspaceInstances(
    ...
    if (onlyRunning) {
      queryBuilder
        .andWhere("wsi.phasePersisted != 'stopped'")
        .andWhere("wsi.deleted != TRUE");
    }
    const [rows, total] = await queryBuilder.getManyAndCount();
    return { total, rows };
  }

  public async findRegularRunningInstances(
    userId?: string
  ): Promise<WorkspaceInstance[]> {
    const infos = await this.findRunningInstancesWithWorkspaces(
      undefined,
      userId
    );
    return infos
      .filter((info) => info.workspace.type === "regular")
  ...
  public async findWorkspacePortsAuthDataById(
    workspaceId: string
  ): Promise<WorkspacePortsAuthData | undefined> {
    const workspaceInstanceRepo = await this.getWorkspaceInstanceRepo();
  ...
  public async findSessionsInPeriod(
    ...
    for (const session of sessions) {
      resultSessions.push({
        ...
        instance: {
          id: session.wsi_id,
          startedTime: !session.wsi_startedTime
            ? undefined
            : session.wsi_startedTime, // Copy the TypeORM behavior according to column config
          stoppedTime: !session.wsi_stoppedTime
            ? undefined
            : session.wsi_stoppedTime, // Copy the TypeORM behavior according to column config
          stoppingTime: !session.wsi_stoppingTime
            ? undefined
            : session.wsi_stoppingTime, // Copy the TypeORM behavior according to column config
        },
      });
    }
  ...
  public async findWorkspacesForGarbageCollection(
    minAgeInDays: number,
    limit: number
  ...
  public async findPrebuiltWorkspacesForGC(
    daysUnused: number,
    limit: number
  ): Promise<WorkspaceAndOwner[]> {
    const workspaceRepo = await this.getWorkspaceRepo();
  ...
  protected async doJoinInstanceWithWorkspace<T>(
    ...
    const rawResult: InstanceJoinResult[] =
      (await qb.getMany()) as InstanceJoinResult[];
  ...
  public async getFeaturedRepositories(): Promise<
    Partial<WhitelistedRepository>[]
  > {
    const whitelist = await this.getRepositoryWhitelist();
    const allRepos = await whitelist
      .createQueryBuilder("rwl")
      .where("rwl.priority >= :minPrio", {
        minPrio: DBRepositoryWhiteList.MIN_FEATURED_REPOSITORY_PRIO,
      })
  ...
  public async findSnapshotsByWorkspaceId(
    workspaceId: string
  ): Promise<Snapshot[]> {
    const snapshots = await this.getSnapshotRepo();
  ...
  public async storePrebuiltWorkspace(
  ...
  public async findPrebuiltWorkspaceByCommit(
    ...
    return await repo
      ...
      .where("pws.cloneURL = :cloneURL AND pws.commit LIKE :commit", {
        cloneURL,
        commit: commit + "%",
      })
  ...
  public async getTotalPrebuildUseSeconds(
    forDays: number
  ...
  public async findPrebuildsWithWorkpace(
    cloneURL: string
  ): Promise<PrebuildWithWorkspace[]> {
    const repo = await this.getPrebuiltWorkspaceRepo();
    ...
    query = query.innerJoinAndMapOne(
      ...
      DBWorkspace,
      "ws",
      "pws.buildWorkspaceId = ws.id"
    );

    const res = await query.getMany();
    return res.map((r) => {
      const withWorkspace: PrebuiltWorkspace & { workspace: Workspace } =
        r as any;
      return {
        prebuild: r,
  ...
  public async findQueuedPrebuilds(
    cloneURL?: string
  ): Promise<PrebuildWithWorkspace[]> {
    const repo = await this.getPrebuiltWorkspaceRepo();
  ...
  public async attachUpdatableToPrebuild(
    pwsid: string,
    update: PrebuiltWorkspaceUpdatable
  ): Promise<void> {
    const repo = await this.getPrebuiltWorkspaceUpdatableRepo();
    await repo.save(update);
  }
  public async findUpdatablesForPrebuild(
    pwsid: string
  ): Promise<PrebuiltWorkspaceUpdatable[]> {
    const repo = await this.getPrebuiltWorkspaceUpdatableRepo();
    return await repo
  ...
  public async getUnresolvedUpdatables(): Promise<
    PrebuiltUpdatableAndWorkspace[]
  > {
    ...
    return (await pwsuRepo
      ...
      .innerJoinAndMapOne(
        ...
        DBPrebuiltWorkspace,
        "pws",
        "pwsu.prebuiltWorkspaceId = pws.id"
      )
      .innerJoinAndMapOne(
        "pwsu.workspace",
        DBWorkspace,
      ...
      .innerJoinAndMapOne(
        ...
        DBWorkspaceInstance,
        "wsi",
        "ws.id = wsi.workspaceId"
      )
      .where("pwsu.isResolved = 0")
      .getMany()) as any;
  }

  public async findLayoutDataByWorkspaceId(
    workspaceId: string
  ): Promise<LayoutData | undefined> {
  ...
  public async hardDeleteWorkspace(workspaceId: string): Promise<void> {
    await (
      await this.getWorkspaceRepo()
    ).updateById(workspaceId, { deleted: true });
    await (
      await this.getWorkspaceInstanceRepo()
    ).update({ workspaceId }, { deleted: true });
  }
  ...
  public async findAllWorkspaces(
    ...
    if (searchTerm) {
      queryBuilder.andWhere(
        "(contextURL LIKE :searchTerm OR description LIKE :searchTerm)",
        { searchTerm }
      );
    }
    if (minCreationTime) {
  ...
  public async findAllWorkspaceAndInstances(
    ...
    let qb = workspaceRepo
      ...
      .leftJoinAndMapOne(
        ...
        DBWorkspaceInstance,
        "wsi",
        ["wsi.workspaceId = ws.id"].join(" AND ")
      )
      .leftJoinAndMapOne(
        "ignore",
        DBWorkspaceInstance,
  ...
  async findWorkspaceAndInstance(
    id: string
  ): Promise<WorkspaceAndInstance | undefined> {
    const workspaceRepo = await this.getWorkspaceRepo();
  ...
  async findPrebuiltWorkspacesById(
    id: string
  ): Promise<PrebuiltWorkspace | undefined> {
    const repo = await this.getPrebuiltWorkspaceRepo();
