...
export class TypeORMPendingGithubEventDBImpl implements PendingGithubEventDB {
  ...
  protected async getRepo(): Promise<Repository<DBPendingGithubEvent>> {
    ...
    ).getRepository<DBPendingGithubEvent>(DBPendingGithubEvent);
  }
  ...
  public async findByGithubUserID(
    type: string,
    accountId: number
  ): Promise<PendingGithubEvent[]> {
  ...
  public async findWithUser(
    type: string
  ): Promise<PendingGithubEventWithUser[]> {
    const repo = await this.getRepo();
    const res = await repo
      ...
      .innerJoinAndMapOne(
        ...
        DBIdentity,
        "ident",
        "pghe.githubUserId = ident.authId"
      )
