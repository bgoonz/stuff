...
import {
  BUILTIN_WORKSPACE_PROBE_USER_NAME,
  MaybeUser,
  PartialUserUpdate,
  UserDB,
} from "../user-db";
...
export class TypeORMUserDBImpl implements UserDB {
  ...
  protected async getWorkspaceRepo(): Promise<Repository<DBWorkspace>> {
    return (await this.getEntityManager()).getRepository<DBWorkspace>(
      DBWorkspace
    );
  }
  ...
  protected async getTokenRepo(): Promise<Repository<DBTokenEntry>> {
    return (await this.getEntityManager()).getRepository<DBTokenEntry>(
      DBTokenEntry
    );
  }
  ...
  protected async getGitpodTokenRepo(): Promise<Repository<DBGitpodToken>> {
    return (await this.getEntityManager()).getRepository<DBGitpodToken>(
      DBGitpodToken
    );
  }
  ...
  public async findUsersByEmail(email: string): Promise<User[]> {
    ...
    let order = (u1: User, u2: User) =>
      u1.creationDate.localeCompare(u2.creationDate);
    if (result.length > 1) {
      ...
      const lastActivity = (u: User) =>
        lastActivities
          .filter(({ userId }) => userId === u.id)
          .map(({ lastActivity }) => lastActivity)[0];
  ...
  public async findUserByGitpodToken(
    ...
    const qBuilder = repo
      .createQueryBuilder("gitpodToken")
      .leftJoinAndSelect("gitpodToken.user", "user");
    if (!!tokenType) {
      qBuilder.where(
        "gitpodToken.tokenHash = :tokenHash AND gitpodToken.type = :tokenType",
        { tokenHash, tokenType }
      );
    } else {
    ...
    qBuilder.andWhere(
      "gitpodToken.deleted <> TRUE AND user.markedDeleted <> TRUE AND user.blocked <> TRUE"
    );
  ...
  public async storeGitpodToken(
    token: GitpodToken & { user: DBUser }
  ): Promise<void> {
    const repo = await this.getGitpodTokenRepo();
  ...
  public async deleteGitpodTokensNamedLike(
    userId: string,
    namePattern: string
  ): Promise<void> {
    const repo = await this.getGitpodTokenRepo();
  ...
  public async findTokenEntryById(
    uid: string
  ...
  public async updateTokenEntry(
    tokenEntry: Partial<TokenEntry> & Pick<TokenEntry, "uid">
  ): Promise<void> {
    const repo = await this.getTokenRepo();
  ...
  public async deleteTokens(
    identity: Identity,
    shouldDelete?: (entry: TokenEntry) => boolean
  ): Promise<void> {
  ...
  public async findTokenForIdentity(
    identity: Identity
  ): Promise<Token | undefined> {
    const tokenEntries = await this.findTokensForIdentity(identity);
    if (tokenEntries.length > 1) {
      throw new Error(
        `Found more than one active token for ${identity.authProviderId} and user ${identity.authName}`
      );
    }
  ...
  public async findTokensForIdentity(
    identity: Identity,
  ...
  public async getUserCount(
    excludeBuiltinUsers: boolean = true
  ): Promise<number> {
    const userRepo = await this.getUserRepo();
  ...
  public async findAllUsers(
    ...
    if (excludeBuiltinUsers) {
      qBuilder.andWhere("user.name <> :username", {
        username: BUILTIN_WORKSPACE_PROBE_USER_NAME,
      });
    }
  ...
  public async extraAccessTokenFields?(
    user: OAuthUser
  ...
  async persist(accessToken: OAuthToken): Promise<void> {
    ...
    const tokenHash = crypto
      .createHash("sha256")
      .update(accessToken.accessToken, "utf8")
      .digest("hex");
  ...
  async revoke(accessTokenToken: OAuthToken): Promise<void> {
    const tokenHash = crypto
      .createHash("sha256")
      .update(accessTokenToken.accessToken, "utf8")
      .digest("hex");
