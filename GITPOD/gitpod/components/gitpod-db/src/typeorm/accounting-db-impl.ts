...
import {
  DBSubscription,
  DBSubscriptionAdditionalData,
  DBPaymentSourceInfo,
} from "./entity/db-subscription";
...
export class TypeORMAccountingDBImpl implements AccountingDB {
  ...
  async newAccountEntry(
    accountEntry: Omit<AccountEntry, "uid">
  ...
  protected async getSubscriptionAdditionalDataRepo(): Promise<
    Repository<DBSubscriptionAdditionalData>
  > {
    return (await this.getEntityManager()).getRepository(
      DBSubscriptionAdditionalData
    );
  }

  protected async getPaymentSourceRepo(): Promise<
    Repository<DBPaymentSourceInfo>
  > {
    return (await this.getEntityManager()).getRepository(DBPaymentSourceInfo);
  }

  async newSubscription(
    subscription: Omit<Subscription, "uid">
  ...
  async deleteSubscription(subscription: Subscription): Promise<void> {
    return await (
      await this.getSubscriptionRepo()
    ).delete(subscription as DBSubscription);
  }

  async findActiveSubscriptionByPlanID(
    planID: string
  ): Promise<Subscription[]> {
  ...
  async findActiveSubscriptions(
    ...
    return (await this.getSubscriptionRepo())
      .createQueryBuilder("subscription")
      .where(
        'subscription.startDate <= :to AND (subscription.endDate = "" OR subscription.endDate > :from)',
  ...
  async findNotYetCancelledSubscriptions(
    userId: string,
    date: string
  ): Promise<Subscription[]> {
    return (await this.getSubscriptionRepo())
      ...
      .where("subscription.userId  = :userId ", { userId: userId })
      .andWhere(
        '(subscription.cancellationDate = "" OR subscription.cancellationDate > :date)',
        { date: date }
      )
      .andWhere("subscription.deleted != true")
      .andWhere('subscription.planId != "free"') // TODO DEL FREE-SUBS
  ...
  async findSubscriptionForUserByPaymentRef(
    userId: string,
  ...
  async findSubscriptionsByTeamSubscriptionSlotId(
  ...
  async findActiveSubscriptionsByIdentity(
    ...
    const query = repo
      ...
      .where(
        'sub.startDate <= :date AND (:date < sub.endDate OR sub.endDate = "")',
  ...
  async hadSubscriptionCreatedWithCoupon(
    userId: string,
    couponId: string
  ): Promise<boolean> {
  ...
  async findSubscriptionAdditionalData(
    paymentReference: string
  ): Promise<DBSubscriptionAdditionalData | undefined> {
    const repo = await this.getSubscriptionAdditionalDataRepo();
    return repo
      .createQueryBuilder("sad")
  ...
  async storePaymentSourceInfo(
    info: DBPaymentSourceInfo
  ): Promise<DBPaymentSourceInfo> {
