...
export const createServiceMock = function <
  C extends GitpodClient,
  S extends GitpodServer
>(methods: Partial<JsonRpcProxy<S>>): GitpodServiceImpl<C, S> {
  return new GitpodServiceImpl<C, S>(createServerMock(methods));
};

export const createServerMock = function <
  C extends GitpodClient,
  S extends GitpodServer
>(methods: Partial<JsonRpcProxy<S>>): JsonRpcProxy<S> {
...
export class GitpodCompositeClient<Client extends GitpodClient>
...
export class WorkspaceInstanceUpdateListener {
  ...
  private sync(): void {
    ...
    this.syncQueue = this.syncQueue.then(async () => {
      ...
      try {
        const info = await this.service.server.getWorkspace(
          this._info.workspace.id
        );
        if (token.isCancellationRequested) {
          return;
        }
...
export class GitpodServiceImpl<
...
> {
  ...
  constructor(
    public readonly server: JsonRpcProxy<Server>,
    private options?: GitpodServiceOptions
  ) {
    server.setClient(this.compositeClient);
    server.onDidOpenConnection(() =>
      this.compositeClient.notifyDidOpenConnection()
    );
    server.onDidCloseConnection(() =>
      this.compositeClient.notifyDidCloseConnection()
    );
  }
  ...
  private readonly instanceListeners = new Map<
    string,
...
export function createGitpodService<
  C extends GitpodClient,
  S extends GitpodServer
