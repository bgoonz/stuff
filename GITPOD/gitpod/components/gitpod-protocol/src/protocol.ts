...
export namespace UserEnvVar {
  ...
  export function score(value: UserEnvVarValue): number {
    ...
    const [ownerPattern, repoPattern] = splitRepositoryPattern(
  ...
  export function filter<T extends UserEnvVarValue>(
    vars: T[],
    owner: string,
    repo: string
  ): T[] {
...
export namespace Workspace {
  ...
  export function getFullRepositoryUrl(ws: Workspace): string | undefined {
    if (CommitContext.is(ws.context)) {
      return `https://${ws.context.repository.host}/${getFullRepositoryName(
        ws
      )}`;
    }
    return undefined;
...
export namespace GithubAppPrebuildConfig {
  export function is(
    obj: boolean | GithubAppPrebuildConfig
  ): obj is GithubAppPrebuildConfig {
    return !(typeof obj === "boolean");
  }
}

export type WorkspaceImageSource =
...
export namespace ExternalImageConfigFile {
  ...
  ): config is ExternalImageConfigFile {
    return (
...
export namespace WithDefaultConfig {
  ...
  ): WorkspaceContext & WithDefaultConfig {
    return {
      ...ctx,
      withDefaultConfig: true,
    };
...
export type RunningWorkspaceInfo = WorkspaceInfo & {
  latestInstance: WorkspaceInstance;
};
