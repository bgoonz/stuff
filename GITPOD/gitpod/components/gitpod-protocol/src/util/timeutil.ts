...
export const durationInHours = (d1: string, d2: string) =>
  liftDate(d1, d2, (d1, d2) =>
    millisecondsToHours(d1.getTime() - d2.getTime())
  );
export const durationInMillis = (d1: string, d2: string) =>
...
export const isDateSmallerOrEqual = (d1: string, d2: string | undefined) =>
  !d2 || d1 <= d2;
export const isDateSmaller = (d1: string, d2: string | undefined) =>
  !d2 || d1 < d2;
export const oldest = (d1: string, d2: string): string => (d1 > d2 ? d1 : d2);
export const earliest = (d1: string, d2: string): string => (d1 < d2 ? d1 : d2);
export const orderAsc = (d1: string, d2: string): number =>
  liftDate(d1, d2, (d1, d2) => d1.getTime() - d2.getTime());
export const liftDate1 = <T>(d1: string, f: (d1: Date) => T): T =>
  f(new Date(d1));
export const liftDate = <T>(
  d1: string,
  d2: string,
  f: (d1: Date, d2: Date) => T
): T => f(new Date(d1), new Date(d2));
