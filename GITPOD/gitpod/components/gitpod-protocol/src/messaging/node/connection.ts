...
export function createServerWebSocketConnection(
...
export function openJsonRpcSocket(
  options: IServerOptions,
  ...
  return <IWebSocket>{
    send: (content) => {
      if (webSocket.readyState !== ws.OPEN) {
        if (sendsAfterOpen++ > 3) {
          log.error(
            `Repeated try to send on closed web socket (readyState was ${webSocket.readyState})`,
            { ws }
          );
        }
        return;
