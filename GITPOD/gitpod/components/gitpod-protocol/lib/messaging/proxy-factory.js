...
var JsonRpcConnectionHandler = /** @class */ (function () {
  ...
  JsonRpcConnectionHandler.prototype.onConnection = function (
    connection,
    request
  ) {
...
var JsonRpcProxyFactory = /** @class */ (function () {
  ...
  JsonRpcProxyFactory.prototype.listen = function (connection) {
    ...
    connection.onDispose(function () {
      return _this.waitForConnection();
    });
    connection.listen();
        ...
        switch (_b.label) {
          ...
          case 2:
            ...
            if (e_1 instanceof vscode_jsonrpc_1.ResponseError) {
              ...
              logging_1.log.error(
                "Request " + method + " failed with internal server error",
                e_1,
                { method: method, args: args }
              );
            }
    ...
    return function () {
      ...
      return _this.connectionPromise.then(function (connection) {
        return new Promise(function (resolve, reject) {
          try {
            if (isNotify) {
              connection.sendNotification.apply(
                connection,
                __spread([p.toString()], args)
              );
              resolve(undefined);
            } else {
              var resultPromise = connection.sendRequest.apply(
                connection,
                __spread([p.toString()], args)
              );
              resultPromise
