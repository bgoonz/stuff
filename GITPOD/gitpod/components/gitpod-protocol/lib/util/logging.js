...
function doLog(calledViaConsole, consoleLog, severity, args) {
    ...
    var logItem = makeLogItem(severity, context, message, error, payloadArgs, calledViaConsole);
    if (logItem !== undefined) {
        consoleLog(logItem);
    }
}
function makeLogItem(severity, context, message, error, payloadArgs, calledViaConsole) {
    ...
    var payload = payloadArgs.length == 0 ? undefined : payloadArgs.length == 1 ? payloadArgs[0] : payloadArgs;
    var logItem = __assign(__assign({}, reportedErrorEvent), { component: component,
        severity: severity, time: new Date().toISOString(), environment: process.env.KUBE_STAGE, region: process.env.GITPOD_REGION, context: context,
        message: message,
        error: error,
        payload: payload, loggedViaConsole: calledViaConsole ? true : undefined });
...
function jsonStringifyWithErrors(value) {
    return JSON.stringify(value, function (key, value) { return value instanceof Error ? value.stack : value; });
}
