...
var basewoWkspaceIDRegex =
  "(([a-f][0-9a-f]{7}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})|([0-9a-z]{2,16}-[0-9a-z]{2,16}-[0-9a-z]{8}))";
// this pattern matches v4 UUIDs as well as the new generated workspace ids (e.g. pink-panda-ns35kd21)
var workspaceIDRegex = RegExp("^" + basewoWkspaceIDRegex + "$");
// this pattern matches URL prefixes of workspaces
var workspaceUrlPrefixRegex = RegExp(
  "^([0-9]{4,6}-)?" + basewoWkspaceIDRegex + "\\."
);
var GitpodHostUrl = /** @class */ (function () {
  ...
  GitpodHostUrl.prototype.withDomainPrefix = function (prefix) {
    return this.with(function (url) {
      return { host: prefix + url.host };
    });
  };
  ...
  GitpodHostUrl.prototype.withoutDomainPrefix = function (removeSegmentsCount) {
    return this.with(function (url) {
      return {
        host: url.host.split(".").splice(removeSegmentsCount).join("."),
      };
    });
  };
  GitpodHostUrl.prototype.with = function (urlUpdate) {
    var update =
      typeof urlUpdate === "function" ? urlUpdate(this.url) : urlUpdate;
    var addSlashToPath =
      update.pathname &&
      update.pathname.length > 0 &&
      !update.pathname.startsWith("/");
    if (addSlashToPath) {
  ...
  GitpodHostUrl.prototype.withApi = function (urlUpdate) {
    ...
    return updated.with(function (url) {
      return { pathname: "/api" + url.pathname };
    });
  };
  GitpodHostUrl.prototype.withContext = function (contextUrl) {
    return this.with(function (url) {
      return { hash: contextUrl };
    });
  };
  GitpodHostUrl.prototype.asWebsocket = function () {
    return this.with(function (url) {
      return { protocol: url.protocol === "https:" ? "wss:" : "ws:" };
    });
  };
  ...
  GitpodHostUrl.prototype.asAccessControl = function () {
    return this.with(function (url) {
      return { pathname: "/access-control/" };
    });
  };
  ...
  GitpodHostUrl.prototype.asApiLogout = function () {
    return this.withApi(function (url) {
