...
export class TheiaPluginService {
   ...
   */
  protected toObjectPath(
    pluginEntryId: string,
    userId: string,
    fullPluginName: string
  ) {
    return `${userId}.plugin.${pluginEntryId}.${fullPluginName}`.replace(
      /[^A-Za-z0-9-_.~]/g,
      "~"
    );
  }
  ...
  async preflight(
    ...
    try {
      if (state == TheiaPlugin.State.Uploading) {
        const signedUrl = await this.storageClient.createPluginUploadUrl(
          bucketName,
          path
        );
        return signedUrl;
      } else {
        const signedUrl = await this.storageClient.createPluginDownloadUrl(
          bucketName,
          path
        );
        return signedUrl;
      ...
      log.warn(
        `Failed to create a signed URL for plugin with DB id ${pluginEntryId}!`,
        error,
        { bucketName, path, state }
      );
      throw error;
    }
  }
  ...
  async checkin(pluginEntryId: string): Promise<string> {
    ...
    try {
      ...
      log.error("Failed to mark a failed plugin checkin.", err, {
        pluginEntryId,
      });
    }
    throw error;
   ...
   */
  async preparePluginUpload(
    params: PreparePluginUploadParams,
    userId: string
  ): Promise<string> {
    const { fullPluginName } = params;
    const pathFn = (pluginEntryId: string) =>
      this.toObjectPath(pluginEntryId, userId, fullPluginName);
    const pluginEntry = await this.pluginDB.newPlugin(
      userId,
      fullPluginName,
      this.bucketName,
      pathFn
  ...
  private parseFullPluginName(fullPluginName: string): {
    name: string;
    version?: string;
  } {
  ...
  protected toFullPluginName(pluginId: string): string {
    return (
      pluginId.substring(0, pluginId.lastIndexOf(":")) || pluginId
    ).toLowerCase();
  }
  ...
  async resolvePlugins(
          ...
          resolving.push(
            (async () => {
              try {
                ...
                resolved[pluginId] =
                  (resolvedPlugin && Object.assign(resolvedPlugin, { kind })) ||
                  undefined;
              } catch (e) {
    ...
    const workspaceExtensions =
      (config && config.vscode && config.vscode.extensions) || [];
  ...
  > {
    ...
    const uploadedPlugins = pluginEntries.filter(
      (e) => e.state == TheiaPlugin.State.Uploaded
    );
    ...
    if (uploadedPlugins.length > 1) {
      log.debug(`Many plugins with same ID" found. Taking first!`, {
        count: uploadedPlugins.length,
        pluginId,
      });
    }
  ...
  private async resovleFromOpenVSX(
  ...
  > {
    try {
      ...
      if (response.status !== 200) {
        log.error(
          `Failed to find extension '${name}@${
            version || "latest"
          }' with '${queryHref}': ${response.status} (${response.statusText}).`
        );
        return undefined;
      }
      const result: {
      ...
      if (!extension) {
        log.debug(
          `Extension '${name}@${
            version || "latest"
          }' not found in '${vsxRegistryUrl}' registry.`
        );
        return undefined;
      }
      return {
        fullPluginName:
          `${extension.namespace}.${extension.name}@${extension.version}`.toLowerCase(),
  ...
  async installUserPlugins(
    userId: string,
    params: InstallPluginsParams
  ): Promise<boolean> {
  ...
  protected async updateUserPlugins(
    ...
    await this.userStorageResourcesDB.update(
      userId,
      userPluginsUri,
  ...
  protected async getUserPlugins(userId: string): Promise<Set<string>> {
    const content = await this.userStorageResourcesDB.get(
      userId,
      userPluginsUri
    );
