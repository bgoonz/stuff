...
export class Server<C extends GitpodClient, S extends GitpodServer> {
  ...
  protected sessionHandlerProvider: SessionHandlerProvider;
  ...
  @inject(EnforcementController)
  protected readonly enforcementController: EnforcementController;
  @inject(TheiaPluginController)
  protected readonly pluginController: TheiaPluginController;
  @inject(WebsocketConnectionManager)
  protected websocketConnectionHandler: WebsocketConnectionManager<C, S>;
  @inject(MessageBusIntegration)
  protected readonly messagebus: MessageBusIntegration;
  @inject(WorkspaceDownloadService)
  protected readonly workspaceDownloadService: WorkspaceDownloadService;
  @inject(MonitoringEndpointsApp)
  protected readonly monitoringEndpointsApp: MonitoringEndpointsApp;
  @inject(CodeSyncService) private readonly codeSyncService: CodeSyncService;
  @inject(HeadlessLogController)
  protected readonly headlessLogController: HeadlessLogController;

  @inject(RabbitMQConsensusLeaderMessenger)
  protected readonly consensusMessenger: RabbitMQConsensusLeaderMessenger;
  @inject(ConsensusLeaderQorum) protected readonly qorum: ConsensusLeaderQorum;
  @inject(WorkspaceGarbageCollector)
  protected readonly workspaceGC: WorkspaceGarbageCollector;
  @inject(DeletedEntryGC) protected readonly deletedEntryGC: DeletedEntryGC;
  @inject(OneTimeSecretServer)
  protected readonly oneTimeSecretServer: OneTimeSecretServer;

  @inject(PeriodicDbDeleter)
  protected readonly periodicDbDeleter: PeriodicDbDeleter;

  @inject(BearerAuth) protected readonly bearerAuth: BearerAuth;

  @inject(HostContextProvider)
  protected readonly hostCtxProvider: HostContextProvider;
  ...
  public async init(app: express.Application) {
    ...
    this.eventEmitter.on(Server.EVENT_ON_START, (httpServer) => {
      ...
      const csrfGuard: ws.VerifyClientCallbackAsync = (
        info: { origin: string; secure: boolean; req: http.IncomingMessage },
        callback: (res: boolean, code?: number, message?: string) => void
      ) => {
        let allowedRequest = isAllowedWebsocketDomain(
          info.origin,
          this.env.hostUrl.url.hostname
        );
        if (
          this.env.kubeStage === "prodcopy" ||
          this.env.kubeStage === "staging"
        ) {
      ...
      const handleSession: WsRequestHandler = (ws, req, next) => {
        ...
        this.sessionHandlerProvider.sessionHandler(
          req,
          {} as express.Response,
          next
        );
      ...
      const initSessionHandlers =
        this.authenticator.initHandlers.map<WsRequestHandler>(
          (handler) => (ws, req, next) => {
            // The fake response needs to be create in a per-request context to avoid memory leaks
      ...
      wsHandler.ws(
        ...
        (ws, request) => {
          ...
          (request as any).wsConnection = createWebSocketConnection(
            websocket,
            console
          );
        },
        ...
        (ws: ws, req: express.Request) => {
          websocketConnectionHandler.onConnection(
            (req as any).wsConnection,
            req
          );
        }
      );
      wsHandler.ws(
        ...
        this.bearerAuth.websocketHandler,
        handleError,
        pingPong,
        (ws: ws, req: express.Request) => {
          websocketConnectionHandler.onConnection(
    ...
    app.use(
      (
        ...
        if (
          ...
          res.redirect(
            this.env.hostUrl.with({ pathname: req.originalUrl }).toString()
          );
          return;
        }
        return next(
          new Error("Unhandled request: " + req.method + " " + req.originalUrl)
        );
      }
    );
    ...
    this.workspaceGC
      .start()
      .catch((err) => log.error("wsgc: error during startup", err));
  ...
  public async start(port: number) {
    ...
    const httpServer = this.app.listen(port, () => {
      ...
      log.info(
        `Server listening on port: ${(<AddressInfo>httpServer.address()).port}`
      );
    });
    ...
    if (this.monApp) {
      this.monHttpServer = this.monApp.listen(9500, "localhost", () => {
        log.info(
          `Monitoring server listening on port: ${
            (<AddressInfo>this.monHttpServer!.address()).port
          }`
        );
      });
    }
