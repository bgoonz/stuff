...
@injectable()
export class GitlabContextParser
  extends AbstractContextParser
  implements IContextParser
{
  ...
  public async handle(
    ...
    try {
      const { host, owner, repoName, moreSegments } = await this.parseURL(
        user,
        contextUrl
      );
      if (moreSegments.length > 0) {
        switch (moreSegments[0]) {
          ...
          case "issues": {
            return await this.handleIssueContext(
              user,
              host,
              owner,
              repoName,
  ...
  public async parseURL(user: User, contextUrl: string): Promise<URLParts> {
    var { host, owner, repoName, moreSegments, searchParams } =
      await super.parseURL(user, contextUrl);
    // TODO: we remove the /-/ in the path in the next line as quick fix for #3809 -- improve this in the long term
    const segments = [
      owner,
      repoName,
      ...moreSegments.filter((s) => s !== "-"),
    ];
  ...
  protected async handleDefaultContext(
    ...
    try {
      ...
      try {
        const branchOrTag = await this.getBranchOrTag(user, owner, repoName, [
        ...
        if (
          ...
          (error.message as string).startsWith(
            "Cannot find tag/branch for context"
          )
        ) {
  ...
  protected async handleTreeContext(
    ...
    try {
      const branchOrTagPromise =
        segments.length > 0
          ? this.getBranchOrTag(user, owner, repoName, segments)
          : undefined;
      const repository = await this.fetchRepo(user, `${owner}/${repoName}`);
      ...
      const result = await this.gitlabApi.run<GitLab.TreeObject[]>(
        user,
        async (g) => {
      ...
      if (GitLab.ApiError.is(result)) {
        throw new Error(
          `Error reading TREE ${owner}/${repoName}/tree/${segments.join(
            "/"
          )}: ${result}`
        );
      } else {
  ...
  protected async getBranchOrTag(
    ...
    let branchOrTagObject:
      | { type: RefType; name: string; revision: string }
      | undefined = undefined;
    ...
    for (const candidate of branchOrTagCandidates) {
      ...
      const possibleBranch = await this.gitlabApi.run<GitLab.Branch>(
        user,
        async (g) => {
      ...
      const isNotFoundBranch =
        ...
        (GitLab.ApiError.isNotFound(possibleBranch) ||
          GitLab.ApiError.isInternalServerError(possibleBranch));
      if (!isNotFoundBranch) {
        ...
        branchOrTagObject = {
          type: "branch",
          name: possibleBranch.name,
          revision: possibleBranch.commit.id,
        };
        break;
      }

      // Check if there is a TAG with name `candidate`:
      const possibleTag = await this.gitlabApi.run<GitLab.Tag>(
        user,
        async (g) => {
      ...
      const isNotFoundTag =
        ...
        (GitLab.ApiError.isNotFound(possibleTag) ||
          GitLab.ApiError.isInternalServerError(possibleTag));
      if (!isNotFoundTag) {
        ...
        branchOrTagObject = {
          ...
          revision: possibleTag.commit.id,
        };
        break;
      }
    }
    ...
    if (branchOrTagObject === undefined) {
      log.debug(
        `Cannot find tag/branch for context: ${owner}/${repoName}/tree/${segments.join(
          "/"
        )}.`,
  ...
  protected async handlePullRequestContext(
    ...
    const result = await this.gitlabApi.run<GitLab.MergeRequest>(
      user,
      async (g) => {
    ...
    const tagetRepo =
      sourceProjectId === targetProjectId
        ? sourceRepo
        : await this.fetchRepo(user, targetProjectId);
  ...
  protected async fetchRepo(
    ...
    if (forked_from_project) {
      ...
      const url = new URL(
        forked_from_project.http_url_to_repo.split(
          forked_from_project.namespace.full_path
        )[0]
      );
      const relativePath = url.pathname.slice(1); // hint: pathname always starts with `/`
      const host = relativePath
  ...
  protected async fetchCommit(
    ...
    if (GitLab.ApiError.is(result)) {
      if (result.message === "GitLab responded with code 404") {
        throw new Error(
          `Couldn't find commit #${sha} in repository ${projectId}.`
        );
      }
      throw result;
    }
  ...
  protected async handleCommitContext(
    ...
    if (GitLab.ApiError.is(commit)) {
      throw new Error(
        `Couldn't find commit #${sha} in repository ${owner}/${repoName}.`
      );
    }
    return <NavigatorContext>{
      path: "",
  ...
  public async fetchCommitHistory(
    ...
    const result = await this.gitlabApi.run<GitLab.Commit[]>(
      user,
      async (g) => {
