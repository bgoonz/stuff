...
export class GitlabFileProvider implements FileProvider {
  ...
  public async getGitpodFileContent(
    commit: Commit,
    user: User
  ...
  public async getLastChangeRevision(
    ...
    const result = await this.gitlabApi.run<GitLab.Commit[]>(
      user,
      async (g) => {
        return g.Commits.all(`${repository.owner}/${repository.name}`, {
          path,
          ref_name: revisionOrBranch,
        });
      }
    );
  ...
  public async getFileContent(
    ...
    try {
      const result = await this.gitlabApi.getRawContents(
        user,
        org,
        name,
        commitish,
