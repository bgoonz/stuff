...
export type GitpodServiceFactory<
  C extends GitpodClient,
  S extends GitpodServer
> = () => GitpodServerImpl<C, S>;
...
> implements ConnectionHandler
{
  ...
  protected createAccessGuard(request?: object): FunctionAccessGuard {
    return (
      (request && (request as WithFunctionAccessGuard).functionGuard) ||
      new AllAccessFunctionGuard()
    );
  }

  protected createProxyTarget(
    client: JsonRpcProxy<C>,
    request?: object
  ): GitpodServerImpl<C, S> {
  ...
  protected createRateLimiter(request?: object): RateLimiter {
    ...
    return {
      user: id,
      consume: (method) =>
        UserRateLimiter.instance(this.rateLimiterConfig).consume(id, method),
    };
  }
  ...
  public onConnectionCreated(
    l: (server: GitpodServerImpl<C, S>) => void
  ): Disposable {
...
class GitpodJsonRpcConnectionHandler<
  T extends object
> extends JsonRpcConnectionHandler<T> {
  constructor(
...
class GitpodJsonRpcProxyFactory<
  T extends object
> extends JsonRpcProxyFactory<T> {
  constructor(
  ...
  protected async onRequest(method: string, ...args: any[]): Promise<any> {
    ...
    try {
      ...
      log.warn(
        `Rate limiter prevents accessing method '${method}' of user '${this.rateLimiter.user} due to too many requests.`,
        rlRejected
      );
    ...
    try {
      ...
      if (e instanceof ResponseError) {
        ...
        log.info(`Request ${method} unsuccessful: ${e.code}/"${e.message}"`, {
          method,
          args,
        });
      } else {
        increaseApiCallCounter(method, 500);
        log.error(`Request ${method} failed with internal server error`, e, {
          method,
          args,
        });
      }
