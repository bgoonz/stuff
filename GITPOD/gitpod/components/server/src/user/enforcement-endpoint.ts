...
export const EnforcementControllerServerFactory = Symbol(
...
export class EnforcementController {
  ...
  @inject(EnforcementControllerServerFactory)
  private readonly serverFactory: EnforcementControllerServerFactory;
  ...
  protected getAuthorizedUser(
    ...
    | { callingUser: User; resourceAccessGuard: ResourceAccessGuard }
    | undefined {
    ...
    if (this.authService.hasPermission(user, Permission.ENFORCEMENT)) {
      return {
        callingUser: user,
        resourceAccessGuard: new OwnerResourceGuard(user.id),
      };
    } else {
      return undefined;
    }
  ...
  protected addRouteToBlockUser(router: express.Router) {
    router.get("/block-user/:userid", async (req, res, next) => {
      ...
      if (!auth) {
        log.warn(
          "Unauthorized user attempted to access enforcement endpoint",
          req
        );
      ...
      try {
        ...
        if (e instanceof ResponseError && e.code === ErrorCodes.NOT_FOUND) {
          log.info(
            ...
            `Tried to block non-existent user id=${targetUserID}`
          );
          res.status(404);
          ...
          log.error(
            { userId: callingUser.id },
            `Blocking failed user id=${targetUserID}`,
            e
          );
          res.status(500);
          res.send(e);
  ...
  protected addRouteToKillWorkspace(router: express.Router) {
    router.get("/kill-workspace/:wsid", async (req, res, next) => {
      ...
      const actionUrl = this.env.hostUrl
    ...
    router.post("/kill-workspace/:wsid", async (req, res, next) => {
      ...
      try {
        ...
        log.info(
          { userId: callingUser.id },
          `Stopped workspace ${target.id} through enforcement endpoint`
        );
        res.status(200);
        res.send(
        ...
        if (e instanceof ResponseError && e.code === ErrorCodes.NOT_FOUND) {
          log.info(
            { userId: callingUser.id },
            `Tried to kill non-existent workspace id=${targetWsID}`
          );
          res.status(404);
          ...
          log.error(
            { userId: callingUser.id },
            `Stopping workspace failed workspace id=${targetWsID}`,
            e
          );
          res.status(500);
          res.send(e);
  ...
  protected addRouteToDeleteUser(router: express.Router) {
    ...
    router.post("/delete-user/:userid", async (req, res, next) => {
      ...
      try {
        ...
        log.info(logCtx, `Deleted user id=${targetUserID}.`, {
          action: "delete-user",
          status: "success",
        });
        res.sendStatus(200);
        ...
        res.send(
          `An error occurred while deleting ${targetUserID}: ${message}`
        );
      }
    });
  }
  ...
  protected blockUserUrl(userId: string): string {
    return this.env.hostUrl
      .withApi({ pathname: `/enforcement/block-user/${userId}` })
      .toString();
  }

  protected deleteUserUrl(userId: string): string {
    return this.env.hostUrl
      .withApi({ pathname: `/enforcement/delete-user/${userId}` })
      .toString();
  }
}
