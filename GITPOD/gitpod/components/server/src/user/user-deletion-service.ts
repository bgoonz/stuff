...
export class UserDeletionService {
  ...
  async deleteUser(id: string): Promise<void> {
    ...
    if (user.markedDeleted === true) {
      log.debug(
        { userId: id },
        "Is deleted but markDeleted already set. Continuing."
      );
    }
    ...
    // Auth Providers
    const authProviders = await this.authProviderService.getAuthProvidersOfUser(
      user
    );
    for (const provider of authProviders) {
      try {
        ...
        log.error(
          { userId: id },
          "Failed to delete user's auth provider.",
          error
        );
      }
    }
  ...
  protected async deleteTokens(db: UserDB, user: User) {
    const tokenDeletions = user.identities.map((identity) =>
      db.deleteTokens(identity)
    );
