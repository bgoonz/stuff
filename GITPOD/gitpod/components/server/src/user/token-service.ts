...
export class TokenService implements TokenProvider {
  ...
  @inject(TokenGarbageCollector)
  protected readonly tokenGC: TokenGarbageCollector;
  ...
  async createGitpodToken(
    ...
    await this.userDB.deleteTokens(
      ...
      (tokenEntry) =>
        tokenEntry.token.scopes.every((s) => scopes.indexOf(s) !== -1)
    );
    const token: Token = {
      value: uuid(),
      scopes: scopes || [],
   ...
   * @param workspaceId
   */
  async getFreshPortAuthenticationToken(
    user: User,
    workspaceId: string
  ): Promise<Token> {
    const newPortAuthToken = (): Token => {
      return {
        ...
        updateDate: new Date().toISOString(),
        expiryDate: new Date(
          Date.now() + TokenService.GITPOD_PORT_AUTH_TOKEN_EXPIRY_MILLIS
        ).toISOString(),
      };
    };
  ...
  protected getIdentityForHost(user: User, host: string): Identity {
    ...
    const hostIdentity =
      authProviderId && User.getIdentity(user, authProviderId);
    if (!hostIdentity) {
