...
export class UserController {
  ...
  get apiRouter(): express.Router {
      ...
      async (
        ...
        if (req.isAuthenticated()) {
          log.info(
            ...
            "(Auth) User is already authenticated.",
            { "login-flow": true }
          );
          // redirect immediately
          const redirectTo =
            this.getSafeReturnToParam(req) ||
            this.env.hostUrl.asDashboard().toString();
        ...
        log.info(
          { sessionId: req.sessionID },
          "(Auth) User started the login process",
          { "login-flow": true, clientInfo }
        );
        ...
        if (redirectToLoginPage) {
          ...
          const loginPageUrl = this.env.hostUrl
            .asLogin()
            .with({ search })
            .toString();
        ...
        try {
          ...
          log.error(
            `Login failed due to session save error; redirecting to /sorry`,
            { req, error, clientInfo }
          );
    ...
    router.get(
      ...
      async (
        ...
        let redirectToUrl =
          this.getSafeReturnToParam(req) ||
          branding.redirectUrlAfterLogout ||
          this.env.hostUrl.toString();
        ...
        try {
          ...
          log.warn(logContext, "(Logout) Error on Logout.", {
            error,
            req,
            ...logPayload,
          });
        ...
        // then redirect
        log.info(logContext, "(Logout) Redirecting...", {
          redirectToUrl,
          ...logPayload,
        });
    ...
    router.get(
      ...
      async (
        ...
        if (!req.isAuthenticated() || !User.is(req.user)) {
          res.sendStatus(401);
          log.warn("unauthenticated workspace cookie fetch", {
        ...
        if (!workspace || !instance) {
          res.sendStatus(404);
          log.warn(
            "attempted to fetch workspace cookie for non-existent workspace instance",
            { instanceId: req.params.instanceID, userId: user.id }
          );
          return;
        }
        ...
        if (!token) {
          ...
          res.sendStatus(200);
          log.debug(
            "attempted to fetch workspace cookie, but instance has no owner token",
            { instanceId: req.params.instanceID, userId: user.id }
          );
          return;
        }
        ...
        [" ", "-", "."].forEach(
          (c) => (cookiePrefix = cookiePrefix.split(c).join("_"))
        );
        ...
        if (workspaceId) {
          ...
          if (
            ...
            log.info(
              ...
              "User does not own private workspace. Denied"
            );
            res.sendStatus(403);
            return;
          }
    ...
    router.get("/tos", async (req: express.Request, res: express.Response) => {
      ...
      const redirectOnInvalidRequest = async () => {
        ...
        res.redirect(
          this.getSorryUrl("Oops! Something went wrong. (invalid request)")
        );
      };
      ...
      if (mode === "login") {
        ...
        if (
          TosFlow.WithIdentity.is(tosFlowInfo) &&
          tosFlowInfo.termsAcceptanceRequired === false
        ) {
          log.info(logContext, "(TOS) Not required.", logPayload);
          await this.handleTosProceedForNewUser(
            req,
            res,
            authFlow,
            tosFlowInfo
      ...
      log.info(logContext, "(TOS) Redirecting to /tos.", {
        tosHints,
        ...logPayload,
      });
      res.redirect(
        this.env.hostUrl.with(() => ({ pathname: "/tos/" })).toString()
      );
    });
    ...
    router.post(
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        const redirectOnInvalidSession = async () => {
          ...
          log.info(
            logContext,
            "(TOS) Invalid session. (/tos/proceed)",
            logPayload
          );
          res.redirect(
            this.getSorryUrl("Oops! Something went wrong. (invalid session)")
          );
        };
        ...
        if (!agreeTOS) {
          ...
          log.info(
            logContext,
            "(TOS) User did NOT agree. Redirecting to /logout.",
            logPayload
          );

          res.redirect(
            this.env.hostUrl.withApi({ pathname: "/logout" }).toString()
          );
        ...
        if (TosFlow.WithIdentity.is(tosFlowInfo)) {
          ...
          const user = await this.userService.findUserForLogin({
            candidate: tosFlowInfo.candidate,
          });
          if (user) {
            log.info(
              `(TOS) User was created in a parallel browser session, let's login...`,
              { logPayload }
            );
        ...
        if (TosFlow.WithUser.is(tosFlowInfo)) {
          ...
          if (isInLoginProcess) {
            ...
            let returnTo =
              returnToUrl || this.env.hostUrl.asDashboard().toString();
  ...
  protected async handleTosProceedForNewUser(
    ...
    const user = await this.userService.createUser({
      ...
      token,
      userUpdate: (user) =>
        this.updateNewUserAfterTos(user, tosFlowInfo, tosProceedParams),
    });

    const { additionalIdentity, additionalToken, envVars } = tosFlowInfo;
    if (additionalIdentity && additionalToken) {
      await this.userService.updateUserIdentity(
        user,
        additionalIdentity,
        additionalToken
      );
  ...
  protected updateNewUserAfterTos(
    newUser: User,
    tosFlowInfo: TosFlow.WithIdentity,
    tosProceedParams?: any
  ) {
  ...
  protected async augmentLoginRequest(req: express.Request) {
    ...
    const authProviderConfigs = this.hostContextProvider
      .getAll()
      .map((hc) => hc.authProvider.config);
    ...
    if (returnToURL) {
      ...
      if (authProviderForSpecialContext) {
        ...
        log.debug(
          { sessionId: req.sessionID },
          `Using "${authProviderForSpecialContext.type}" for login ...`,
          {
            "login-flow": true,
            query: req.query,
    ...
    if (returnToURL) {
      ...
      const hash = decodeURIComponent(
        new URL(decodeURIComponent(returnToURL)).hash
      );
      ...
      if (
        !!contextUrlHost &&
        authProvidersOnDashboard.find((a) => a === contextUrlHost)
      ) {
        req.query.host = contextUrlHost;
        log.debug(
          { sessionId: req.sessionID },
          "Guessed auth provider from returnTo URL: " + contextUrlHost,
          { "login-flow": true, query: req.query }
        );
        return;
      }
    }
  ...
  protected getSafeReturnToParam(req: express.Request) {
    const returnToURL: string | undefined =
    ...
    if (
      this.urlStartsWith(returnToURL, this.env.hostUrl.toString()) ||
      this.urlStartsWith(returnToURL, this.env.brandingConfig.homepage)
    ) {
      return returnToURL;
    }

    log.debug({ sessionId: req.sessionID }, "The redirect URL does not match", {
      query: req.query,
    });
    return;
  }
