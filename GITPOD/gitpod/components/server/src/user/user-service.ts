...
export class UserService {
  @inject(BlockedUserFilter)
  protected readonly blockedUserFilter: BlockedUserFilter;
  @inject(UserDB) protected readonly userDb: UserDB;
  @inject(HostContextProvider)
  protected readonly hostContextProvider: HostContextProvider;
  @inject(Env) protected readonly env: Env;
  @inject(TermsAcceptanceDB)
  protected readonly termsAcceptanceDb: TermsAcceptanceDB;
   ...
   */
  async findUserByIdentityStr(
    identityStr: string
  ): Promise<FindUserByIdentityStrResult | undefined> {
    ...
    const identities = await this.userDb.findIdentitiesByName({
      authProviderId,
      authName,
    });
  ...
  protected handleNewUser(newUser: User, isFirstUser: boolean) {
    if (this.env.blockNewUsers) {
      const emailDomainInPasslist = (mail: string) =>
        this.env.blockNewUsersPassList.some((e) => mail.endsWith(`@${e}`));
      const canPass = newUser.identities.some(
        (i) => !!i.primaryEmail && emailDomainInPasslist(i.primaryEmail)
      );
  ...
  async acceptCurrentTerms(user: User) {
    ...
    return await this.termsAcceptanceDb.updateAcceptedRevision(
      user.id,
      terms.revision
    );
  }
  ...
  async updateUserIdentity(user: User, candidate: Identity, token: Token) {
    ...
    user.identities = user.identities.filter(
      (i) => i.authProviderId !== candidate.authProviderId
    );
  ...
  async updateUserEnvVarsOnLogin(user: User, envVars?: UserEnvVarValue[]) {
    ...
    const findEnvVar = (name: string, repositoryPattern: string) =>
      currentEnvVars.find(
        (env) =>
          env.repositoryPattern === repositoryPattern && env.name === name
      );
  ...
  async deauthorize(user: User, authProviderId: string) {
    ...
    const identity = externalIdentities.find(
      (i) => i.authProviderId === authProviderId
    );
    if (!identity) {
      log.debug("Cannot deauthorize. Authorization not found.", {
        userId: user.id,
        authProviderId,
      });
      return;
    ...
    if (externalIdentities.length === 1) {
      throw new Error(
        "Cannot remove last provider authorization. Please delete account instead."
      );
    }
  ...
  async asserNoTwinAccount(
    ...
    const loginIdentityOfCurrentUser =
      externalIdentities[externalIdentities.length - 1];
    const authProviderConfigOfCurrentUser = this.hostContextProvider
      .getAll()
      .find(
        (c) =>
          c.authProvider.authProviderId ===
          loginIdentityOfCurrentUser.authProviderId
      )?.authProvider?.config;
    ...
    const authProviderTypeOfOtherUser = this.hostContextProvider
      .getAll()
      .find((c) => c.authProvider.authProviderId === candidate.authProviderId)
    ...
    throw SelectAccountException.create(
      `User is trying to connect a provider identity twice.`,
      payload
    );
  }
  ...
  async asserNoAccountWithEmail(email: string) {
    ...
    throw EmailAddressAlreadyTakenException.create(
