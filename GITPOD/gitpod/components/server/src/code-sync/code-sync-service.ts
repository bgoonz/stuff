...
export class CodeSyncService {
  ...
  get apiRouter(): express.Router {
    ...
    router.use(async (req, res, next) => {
      ...
      try {
        await UserRateLimiter.instance(this.env.rateLimiter).consume(
          id,
      ...
      if (
        !isWithFunctionAccessGuard(req) ||
        !req.functionGuard?.canAccess(accessCodeSyncStorage)
      ) {
        res.sendStatus(403);
        return;
      }
    ...
    router.get("/v1/resource/:resource", async (req, res) => {
      ...
      const resourceKey = ALL_SERVER_RESOURCES.find(
        (key) => key === req.params.resource
      );
      const revs =
        resourceKey && (await this.db.getResources(req.user.id, resourceKey));
      if (!revs || !revs.length) {
        res.sendStatus(204);
        return;
      }
    ...
    router.get("/v1/resource/:resource/:ref", async (req, res) => {
      ...
      if (resourceRev !== fromTheiaRev) {
        resourceRev = (
          await this.db.getResource(req.user.id, resourceKey, resourceRev)
        )?.rev;
      }
      if (
        !resourceRev &&
        (resourceKey === SyncResource.Extensions ||
          resourceKey === SyncResource.Settings)
      ) {
      ...
      if (resourceRev === fromTheiaRev) {
        ...
        if (resourceKey === SyncResource.Extensions) {
          value = await this.theiaPluginService.getCodeSyncResource(
            req.user.id
          );
          version = 5;
        } else if (resourceKey === SyncResource.Settings) {
          const settings = await this.userStorageResourcesDB.get(
            req.user.id,
            userSettingsUri
          );
        ...
        try {
          const urlResponse = await util.promisify<
            DownloadUrlRequest,
            DownloadUrlResponse
          >(this.blobs.downloadUrl.bind(this.blobs))(request);
      ...
      async (req, res) => {
        ...
        const revLimit =
          resourceKey === "machines"
            ? 1
            : config.resources?.[resourceKey]?.revLimit ||
              config?.revLimit ||
              defautltRevLimit;
        ...
        const rev = await this.db.insert(
          ...
          async (rev, oldRev) => {
            ...
            const urlResponse = await util.promisify<
              UploadUrlRequest,
              UploadUrlResponse
            >(this.blobs.uploadUrl.bind(this.blobs))(request);
