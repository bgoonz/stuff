...
export class OAuthController {
  ...
  @inject(AuthCodeRepositoryDB)
  protected readonly authCodeRepositoryDb: AuthCodeRepositoryDB;
  ...
  private getValidUser(
    ...
    if (!req.isAuthenticated() || !User.is(req.user)) {
      const redirectTarget = encodeURIComponent(
        `${this.env.hostUrl}api${req.originalUrl}`
      );
    ...
    router.get(
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        try {
          ...
          const authRequest =
            await authorizationServer.validateAuthorizationRequest(request);
