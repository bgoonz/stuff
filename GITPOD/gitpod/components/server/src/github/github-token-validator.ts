...
export class GitHubTokenValidator implements IGitTokenValidator {
  ...
  async checkWriteAccess(
    ...
    let repo;
    try {
      repo = await this.githubRestApi.run(token, (api) =>
        api.repos.get(parsedRepoName)
      );
    } catch (error) {
      if (GitHubApiError.is(error) && error.response?.status === 404) {
        return { found: false };
      }
      log.error("Error getting repo information from GitHub", error, {
        repoFullName,
        parsedRepoName,
      });
    ...
    if (inOrg) {
      ...
      try {
        ...
        if (errors && errors[0] && (errors[0] as any)["type"] === "FORBIDDEN") {
          ...
          log.error(
            "Error getting organization information from GitHub",
            error,
