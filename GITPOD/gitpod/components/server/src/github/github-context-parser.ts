...
import {
  ...
  LogContext,
  LogPayload,
} from "@gitpod/gitpod-protocol/lib/util/logging";
import {
  IContextParser,
  IssueContexts,
  ...
  implements IContextParser
{
  ...
  public async handle(
    ...
    try {
      ...
      return await this.handleDefaultContext(
        { span },
        user,
        host,
        owner,
  ...
  protected async handleDefaultContext(
    ...
    const span = TraceContext.startSpan(
      "GithubContextParser.handleDefaultContext",
      ctx
    );
  ...
  protected async handleTreeContext(
    ...
    try {
      ...
      for (let i = 1; i <= segments.length; i++) {
        const branchNameOrCommitHash = decodeURIComponent(
        ...
        const pathExpression = JSON.stringify(
          `${branchNameOrCommitHash}:${path}`
        );
        const result: any = await this.githubQueryApi.runQuery(
          user,
          `
      ...
      throw new Error(
        `Couldn't find branch and path for ${segments.join(
          "/"
        )} in repo ${owner}/${repoName}.`
      );
    } catch (e) {
      span.log({ error: e });
      throw e;
  ...
  protected async handlePullRequestContext(
    ...
    try {
      ...
      if (pr === null) {
        log.info(
          `PR ${owner}/${repoName}/pull/${pullRequestNr} not found. Trying issue context.`
        );
        if (tryIssueContext) {
          return this.handleIssueContext(
            { span },
            user,
            host,
            owner,
    ...
    try {
      ...
      if (issue === null) {
        if (tryPullrequestContext) {
          log.info(
            `Issue ${owner}/${repoName}/issues/${issueNr} not found. Trying issue context.`
          );
          return this.handlePullRequestContext(
            { span },
            user,
            host,
            owner,
  ...
  public async fetchCommitHistory(
    ...
    const span = TraceContext.startSpan(
      "GithubContextParser.fetchCommitHistory",
      ctx
    );
