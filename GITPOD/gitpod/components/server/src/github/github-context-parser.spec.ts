...
@suite(
  timeout(10000),
  retries(2),
  skipIfEnvVarNotSet("GITPOD_TEST_TOKEN_GITHUB")
)
class TestGithubContextParser {
    ...
    container.load(
      new ContainerModule((bind, unbind, isBound, rebind) => {
        ...
        bind(AuthProviderParams).toConstantValue(
          TestGithubContextParser.AUTH_HOST_CONFIG
        );
        ...
        bind(HostContextProvider).toConstantValue(
          DevData.createDummyHostContextProvider()
        );
      })
    );
  ...
  protected getTestBranches(): BranchRef[] {
    return [
      TestGithubContextParser.BRANCH_TEST,
      TestGithubContextParser.BRANCH_ISSUE_974,
    ];
  }
  ...
  @test public async testErrorContext_02() {
    try {
      await this.parser.handle(
        {},
        this.user,
        "https://github.com/blo/bla/pull/42"
      );
    } catch (e) {
      expect(NotFoundError.is(e));
      expect(e.data).to.deep.equal(this.bloBlaErrorData);
  ...
  @test public async testErrorContext_03() {
    try {
      await this.parser.handle(
        {},
        this.user,
        "https://github.com/blo/bla/issues/42"
      );
    } catch (e) {
      expect(NotFoundError.is(e));
      expect(e.data).to.deep.equal(this.bloBlaErrorData);
  ...
  @test public async testErrorContext_04() {
    try {
      await this.parser.handle(
        {},
        this.user,
        "https://github.com/blo/bla/tree/my/branch/path/foo.ts"
      );
    } catch (e) {
      expect(NotFoundError.is(e));
      expect(e.data).to.deep.equal(this.bloBlaErrorData);
  ...
  @test public async testTreeContext_02() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/eclipse-theia/theia/tree/master"
    );
    expect(result).to.deep.include({
      ref: "master",
      refType: "branch",
  ...
  @test public async testTreeContext_06() {
    ...
    expect(result).to.deep.include({
      title:
        "Snailclimb/JavaGuide - 940982eb:数据结构与算法/常见安全算法（MD5、SHA1、Base64等等）总结.md",
      repository: {
        host: "github.com",
        owner: "Snailclimb",
        name: "JavaGuide",
  ...
  @test public async testTreeContext_07() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/eclipse-theia/theia#license"
    );
    expect(result).to.deep.include({
      ref: "master",
      refType: "branch",
  ...
  @test public async testTreeContext_tag_01() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/eclipse-theia/theia/tree/v0.1.0"
    );
    expect(result).to.deep.include({
  ...
  @test public async testCommitContext_02_invalidSha() {
    try {
      await this.parser.handle(
        {},
        this.user,
        "https://github.com/gitpod-io/gitpod-test-repo/commit/invalid"
      );
      // ensure that an error has been thrown
      chai.assert.fail();
    } catch (e) {
  ...
  @test public async testPullRequestContext_01() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/TypeFox/theia/pull/1"
    );
    expect(result).to.deep.include({
      title: "Merge master",
      repository: {
  ...
  @test public async testPullRequestthroughIssueContext_04() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/TypeFox/theia/issues/1"
    );
    expect(result).to.deep.include({
      title: "Merge master",
      repository: {
  ...
  @test public async testIssuePageContext() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/gitpod-io/gitpod-test-repo/issues"
    );
    expect(result).to.deep.include({
  ...
  @test public async testIssueThroughPullRequestContext() {
    const result = await this.parser.handle(
      {},
      this.user,
      "https://github.com/gitpod-io/gitpod-test-repo/pull/42"
    );
    expect(result).to.deep.include({
      title: "Test issue web-extension",
      repository: {
