...
export class GitHubRestApi {
  ...
  public async getRepository(
    ...
    const response = await this.runWithCache(key, user, (api) =>
      api.repos.get(params)
    );
...
export namespace GitHubResult {
  ...
  export function mayReadOrgs(result: OctokitResponse<any>): boolean {
    return actualScopes(result).some(
      (scope) => scope === "read:org" || scope === "user"
    );
  }
...
export namespace FileContent {
  export function is(content: any): content is FileContent {
    return (
      FileMetadata.is(content) && "encoding" in content && "content" in content
    );
  }
}
