...
import {
  AuthProviderParams,
  parseAuthProviderParamsFromEnv,
...
export class Env extends AbstractComponentEnv {
  ...
  readonly hostUrl = new GitpodHostUrl(
    process.env.HOST_URL || "https://gitpod.io"
  );
  ...
  readonly workspaceDefaultImage =
    process.env.WORKSPACE_DEFAULT_IMAGE || "gitpod/workspace-full:latest";
  ...
  readonly sessionMaxAgeMs: number = Number.parseInt(
    process.env.SESSION_MAX_AGE_MS || "259200000" /* 3 days */,
    10
  );

  readonly githubAppEnabled: boolean =
    process.env.GITPOD_GITHUB_APP_ENABLED == "true";
  readonly githubAppAppID: number = process.env.GITPOD_GITHUB_APP_ID
    ? parseInt(process.env.GITPOD_GITHUB_APP_ID, 10)
    : 0;
  readonly githubAppWebhookSecret: string =
    process.env.GITPOD_GITHUB_APP_WEBHOOK_SECRET || "unknown";
  readonly githubAppAuthProviderId: string =
    process.env.GITPOD_GITHUB_APP_AUTH_PROVIDER_ID || "Public-GitHub";
  readonly githubAppCertPath: string =
    process.env.GITPOD_GITHUB_APP_CERT_PATH || "unknown";
  readonly githubAppMarketplaceName: string =
    process.env.GITPOD_GITHUB_APP_MKT_NAME || "unknown";
  readonly githubAppLogLevel?: string = process.env.LOG_LEVEL;
  readonly githubAppGHEHost?: string = process.env.GHE_HOST;

  readonly definitelyGpDisabled: boolean =
    process.env.GITPOD_DEFINITELY_GP_DISABLED == "true";

  readonly daysBeforeGarbageCollection: number = parseInt(
    process.env.GITPOD_DAYS_BEFORE_GARBAGE_COLLECTION || "14",
    10
  );
  readonly daysBeforeGarbageCollectingPrebuilds: number = parseInt(
    process.env.GITPOD_DAYS_BEFORE_GARBAGE_COLLECTING_PREBUILDS || "7",
    10
  );
  readonly garbageCollectionStartDate: number = process.env
    .GITPOD_GARBAGE_COLLECTION_START_DATE
    ? new Date(process.env.GITPOD_GARBAGE_COLLECTION_START_DATE).getTime()
    : Date.now();
  readonly garbageCollectionLimit: number = parseInt(
    process.env.GITPOD_GARBAGE_COLLECTION_LIMIT || "1000",
    10
  );

  readonly garbageCollectionDisabled: boolean =
    process.env.GITPOD_GARBAGE_COLLECTION_DISABLED === "true";

  readonly workspaceDeletionRetentionPeriodDays: number = parseInt(
    process.env.GITPOD_WORKSPACE_DELETION_RETENTION_PERIOD_DAYS || "21",
    10
  );
  readonly workspaceDeletionLimit: number = parseInt(
    process.env.GITPOD_WORKSPACE_DELETION_LIMIT || "1000",
    10
  );
  ...
  readonly trialLicensePrivateKey: string | undefined =
    process.env.GITPOD_TRIAL_LICENSE_PVK;

  // this limit should be so high that no regular user ever reaches it
  readonly maxUserEnvvarCount =
    Number.parseInt(process.env.MAX_USER_ENVVAR_COUNT || "4048", 10) || 4048;

  // maxConcurrentPrebuildsPerRef is the maximum number of prebuilds we allow per ref type at any given time
  readonly maxConcurrentPrebuildsPerRef =
    Number.parseInt(process.env.MAX_CONCUR_PREBUILDS_PER_REF || "10", 10) || 10;
  ...
  readonly incrementalPrebuildsCommitHistory: number =
    Number.parseInt(
      process.env.INCREMENTAL_PREBUILDS_COMMIT_HISTORY || "100",
      10
    ) || 100;
  ...
  readonly disableDynamicAuthProviderLogin: boolean = this.parseBool(
    "DISABLE_DYNAMIC_AUTH_PROVIDER_LOGIN"
  );

  /** This value - if present - overrides the default naming scheme for the GCloud bucket that Theia Plugins are stored in */
  readonly theiaPluginsBucketNameOverride: string | undefined =
    process.env["THEIA_PLUGINS_BUCKET_NAME_OVERRIDE"];
  ...
  readonly defaultFeatureFlags: NamedWorkspaceFeatureFlag[] = (() => {
    try {
      const r = this.parseStringArray(
        "GITPOD_DEFAULT_FEATURE_FLAGS"
      ) as NamedWorkspaceFeatureFlag[];
  ...
  readonly insecureNoDomain: boolean =
    getEnvVar("SERVE_INSECURE_NO_DOMAIN", "false") === "true";
  ...
  readonly runDbDeleter: boolean =
    getEnvVar("RUN_DB_DELETER", "false") === "true";

  readonly oauthServerJWTSecret = getEnvVar("OAUTH_SERVER_JWT_SECRET");

  readonly imageBuilderAddress = getEnvVar(
    "IMAGE_BUILDER_SERVICE",
    "image-builder-mk3:8080"
  );

  readonly rateLimiter: RateLimiterConfig = JSON.parse(
    process.env.RATE_LIMITER_CONFIG || "{}"
  );

  readonly contentServiceAddress =
    process.env.CONTENT_SERVICE_ADDRESS || "content-service:8080";
