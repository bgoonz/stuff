...
export class WorkspaceStarter {
  ...
  protected readonly imagebuilderClientProvider: ImageBuilderClientProvider;
  @inject(ImageSourceProvider)
  protected readonly imageSourceProvider: ImageSourceProvider;
  @inject(UserService) protected readonly userService: UserService;
  @inject(IAnalyticsWriter) protected readonly analytics: IAnalyticsWriter;
  @inject(TheiaPluginService)
  protected readonly theiaService: TheiaPluginService;
  ...
  public async startWorkspace(
    ...
    try {
      ...
      if (!workspace.imageSource) {
        ...
        log.debug("Found workspace without imageSource, generated one", {
          imageSource,
        });
      ...
      const pastInstances = await this.workspaceDb
        .trace({ span })
        .findInstances(workspace.id);
      const mustHaveBackup = pastInstances.some(
        (i) =>
          !!i.status && !!i.status.conditions && !i.status.conditions.failed
      );
      ...
      try {
        ...
        needsImageBuild =
          forceRebuild ||
          (await this.needsImageBuild({ span }, user, workspace, instance));
        if (needsImageBuild) {
  ...
  protected async actuallyStartWorkspace(
    ...
    try {
      ...
      const { manager, installation } =
        await this.clientProvider.getStartManager(user, workspace, instance);
      ...
      try {
        ...
        log.warn(
          "cannot send instance update - this should be mostly inconsequential",
          err
        );
      }
      ...
      // start that thing
      const resp = (
        await manager.startWorkspace({ span }, startRequest)
      ).toObject();
      span.log({ resp: resp });
      ...
      if (workspace.type === "prebuild") {
        ...
        if (prebuild && prebuild.state !== "aborted") {
          ...
          await this.workspaceDb
            .trace({ span })
            .storePrebuiltWorkspace(prebuild);
          await this.messageBus.notifyHeadlessUpdate(
            { span },
            workspace.ownerId,
            workspace.id,
            <HeadlessWorkspaceEvent>{
  ...
  protected async newInstance(
    ...
    let featureFlags: NamedWorkspaceFeatureFlag[] =
      workspace.config._featureFlags || [];
    ...
    if (user.featureFlags && user.featureFlags.permanentWSFeatureFlags) {
      featureFlags = featureFlags.concat(
        featureFlags,
        user.featureFlags.permanentWSFeatureFlags
      );
    }
    ...
    if (!!user.additionalData?.featurePreview) {
      featureFlags = featureFlags.concat(
        this.env.previewFeatureFlags.filter((f) => !featureFlags.includes(f))
      );
    }
  ...
  protected async prepareBuildRequest(
    ...
    try {
      ...
      if (
        workspace.baseImageNameResolved &&
      ...
      const auth = new BuildRegistryAuth();
      const userHasRegistryAccess = this.authService.hasPermission(
        user,
      ...
      if (WorkspaceImageSourceDocker.is(imgsrc)) {
        ...
        if (
          !AdditionalContentContext.hasDockerConfig(
            workspace.context,
            workspace.config
          ) &&
        ...
        const contextPath = !!context
          ? path.join(checkoutLocation, context)
          : checkoutLocation;
        const dockerFilePath = path.join(
          checkoutLocation,
          imgsrc.dockerFilePath
        );
  ...
  protected async buildWorkspaceImage(
    ...
    try {
      ...
      if (
        !!buildResult.getBaseRef() &&
        buildResult.getBaseRef() != workspace.baseImageNameResolved
      ) {
        span.log({
          oldBaseRef: workspace.baseImageNameResolved,
          newBaseRef: buildResult.getBaseRef(),
        });
  ...
  protected async createSpec(
    ...
    const createGitpodTokenPromise = (async () => {
      const scopes = this.createDefaultGitpodAPITokenScopes(
        workspace,
        instance
      );
      ...
      tokenExpirationTime.setMinutes(
        tokenExpirationTime.getMinutes() + 24 * 60
      );
      const ots = await this.otsServer.serve(
        traceCtx,
        token,
        otsExpirationTime
      );
    ...
    const userTimeoutPromise =
      this.userService.getDefaultWorkspaceTimeout(user);
    ...
    spec.setWorkspaceLocation(
      workspace.config.workspaceLocation || spec.getCheckoutLocation()
    );
  ...
  protected createDefaultGitpodAPITokenScopes(
    workspace: Workspace,
    instance: WorkspaceInstance
  ): string[] {
    const scopes = [
      ...
      "resource:" +
        ScopedResourceGuard.marshalResourceScope({
          kind: "snapshot",
          subjectID:
            ScopedResourceGuard.SNAPSHOT_WORKSPACE_SUBJECT_ID_PREFIX +
            workspace.id,
          operations: ["create"],
        }),
      "resource:" +
    ...
    if (CommitContext.is(workspace.context)) {
      const subjectID =
        workspace.context.repository.owner +
        "/" +
        workspace.context.repository.name;
      scopes.push(
        "resource:" +
          ScopedResourceGuard.marshalResourceScope({
            kind: "envVar",
  ...
  protected async createInitializer(
    ...
    if (mustHaveBackup) {
      ...
      const snapshot = new SnapshotInitializer();
      snapshot.setSnapshot(context.snapshotBucketId);
      const { git } = await this.createGitInitializer(
        traceCtx,
        workspace,
        context,
        user
    ...
    if (AdditionalContentContext.is(context)) {
      ...
      const fileInfos = await Promise.all(
        Object.entries(context.additionalFiles).map(
          async ([filePath, content]) => {
            const url = await this.otsServer.serve(
              traceCtx,
              content,
              tokenExpirationTime
            );
  ...
  protected async createGitInitializer(
    ...
    const identity = user.identities.find(
      (i) => i.authProviderId === authProviderId
    );
    if (!identity) {
      throw new Error("User is unauthorized!");
    }
    ...
    try {
      ...
      const res = await this.otsServer.serve(
        traceCtx,
    ...
    const localBranchName = IssueContext.is(context)
      ? context.localBranch
      : undefined;
    if (localBranchName) {
    ...
    if (!!userGitConfig) {
      Object.keys(userGitConfig)
        ...
        .forEach((k) =>
          gitConfig.getCustomConfigMap().set(k, userGitConfig[k])
        );
    }
  ...
  protected getCheckoutLocation(workspace: Workspace) {
    return (
      ...
      (CommitContext.is(workspace.context) &&
        workspace.context.repository.name) ||
      "."
    );
  }
  ...
  protected toWorkspaceFeatureFlags(
