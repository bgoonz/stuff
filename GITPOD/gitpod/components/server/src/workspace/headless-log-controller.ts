...
export class HeadlessLogController {
  ...
  get apiRouter(): express.Router {
      ...
      asyncHandler(async (req: express.Request, res: express.Response) => {
        ...
        if (
          !(
            ...
            req.functionGuard?.canAccess(accesHeadlessLogs)
          ) &&
        ...
        try {
          ...
          if (
            !(await resourceGuard.canAccess(
              { kind: "workspaceLog", subject: workspace },
              "get"
            ))
          ) {
            res.sendStatus(403);
          ...
          await this.headlessLogService.streamWorkspaceLog(
            instance,
            params.terminalId,
            writeToResponse,
            aborted
    ...
    router.get("/", (req: express.Request, res: express.Response) => {
      ...
      log.error({ userId: user.id }, "/headless-logs: malformed request", {
        path: req.path,
      });
      res.status(400);
    });
