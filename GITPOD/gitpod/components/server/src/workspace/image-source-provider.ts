    ...
    try {
      ...
      if (ExternalImageConfigFile.is(imgcfg)) {
        ...
        if (!hostContext || !hostContext.services) {
          throw new Error(
            `Cannot fetch workspace image source for host: ${repository.host}`
          );
        }
        const lastDockerFileSha =
          await hostContext.services.fileProvider.getLastChangeRevision(
            repository,
        ...
        if (
          ...
          return {
            ...
            dockerFileHash: this.getContentSHA(
              context.additionalFiles[config.image.file]
            ),
        ...
        const hostContext = this.hostContextProvider.get(
          context.repository.host
        );
        if (!hostContext || !hostContext.services) {
          throw new Error(
            `Cannot fetch workspace image source for host: ${context.repository.host}`
          );
        }
        const lastDockerFileSha =
          await hostContext.services.fileProvider.getLastChangeRevision(
