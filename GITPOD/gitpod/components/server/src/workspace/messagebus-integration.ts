...
export class WorkspaceInstanceUpdateListener extends AbstractTopicListener<WorkspaceInstance> {
  constructor(
    ...
    listener: TopicListener<WorkspaceInstance>,
  ...
  topic() {
    return this.messageBusHelper.getWsTopicForListening(
      this.userId,
      undefined,
      "updates"
    );
...
export class PrebuildUpdatableQueueListener implements MessagebusListener {
  ...
  async establish(channel: Channel): Promise<void> {
    ...
    await MessageBusHelperImpl.assertPrebuildWorkspaceUpdatableQueue(
      this.channel
    );
  ...
  protected handleMessage(message: Message | null) {
    ...
    const spanCtx = opentracing
      .globalTracer()
      .extract(opentracing.FORMAT_HTTP_HEADERS, message.properties.headers);
    const span = !!spanCtx
      ? opentracing
          .globalTracer()
          .startSpan(
            `/messagebus/${MessageBusHelperImpl.PREBUILD_UPDATABLE_QUEUE}`,
    ...
    try {
      ...
      log.warn("Caught message without or with invalid JSON content", e, {
        message,
      });
    }
...
export class MessageBusIntegration extends AbstractMessageBusIntegration {
  ...
  listenForPrebuildUpdatableQueue(
    callback: (ctx: TraceContext, evt: HeadlessWorkspaceEvent) => void
  ): Disposable {
  ...
  listenForWorkspaceInstanceUpdates(
    ...
    callback: (ctx: TraceContext, workspaceInstance: WorkspaceInstance) => void
  ): Disposable {
    const listener = new WorkspaceInstanceUpdateListener(
  ...
  async notifyOnInstanceUpdate(userId: string, instance: WorkspaceInstance) {
    ...
    const topic = this.messageBusHelper.getWsTopicForPublishing(
      userId,
