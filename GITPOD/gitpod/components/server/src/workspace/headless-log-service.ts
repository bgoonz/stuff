...
import {
  Status,
  StatusServiceClient,
} from "@gitpod/supervisor-api-grpcweb/lib/status_pb_service";
import {
  TasksStatusRequest,
  TasksStatusResponse,
  TaskStatus,
...
} from "@gitpod/supervisor-api-grpcweb/lib/terminal_pb_service";
import {
...
export class HeadlessLogService {
  ...
  @inject(HeadlessLogServiceClient)
  protected readonly headlessLogClient: HeadlessLogServiceClient;
  ...
  protected async contentServiceListLogs(
    ...
    const response = await new Promise<ListLogsResponse>((resolve, reject) => {
      this.headlessLogClient.listLogs(
        req,
        (err: grpc.ServiceError | null, response: ListLogsResponse) => {
          if (err) {
            reject(err);
          } else {
            resolve(response);
    ...
    log.info(
      `got ${response.getTaskIdList().length} taskIds from content-service`
    );

    // fetch a download URL for each task
    const responses: Promise<[string, string]>[] = response.getTaskIdList().map(
      (taskId) =>
        new Promise<[string, string]>((resolve, reject) => {
          ...
          this.headlessLogClient.logDownloadURL(
            req,
            (
              err: grpc.ServiceError | null,
              response: LogDownloadURLResponse
            ) => {
              if (err) {
                reject(err);
              } else {
  ...
  protected async supervisorListHeadlessLogs(
    ...
    const tasks = await new Promise<TaskStatus[]>((resolve, reject) => {
      ...
      stream.on("end", (status?: Status) => {
        if (status && status.code !== grpc.status.OK) {
          const err = new Error(
            `upstream ended with status code: ${status.code}`
          );
    ...
    for (const task of tasks) {
      ...
      if (terminalId === "") {
        ...
        throw new Error(
          `instance's ${wsi.id} task ${task.getId} has no terminal yet`
        );
      }
      streams[taskId] = this.env.hostUrl
        .with({
  ...
  async streamWorkspaceLog(
    ...
    await this.retryWhileInstanceIsRunning(
      wsi,
      doStream,
