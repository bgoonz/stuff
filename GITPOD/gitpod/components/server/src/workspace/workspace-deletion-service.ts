...
export class WorkspaceDeletionService {
   ...
   * @param ws
   */
  public async garbageCollectWorkspace(
    ctx: TraceContext,
    ws: WorkspaceOwnerAndSoftDeleted
  ): Promise<boolean> {
    const span = TraceContext.startSpan("garbageCollectWorkspace", ctx);

    try {
      const deleteSnapshots = ws.softDeleted === "user";
      const successfulDeleted = await this.deleteWorkspaceStorage(
        ws,
        deleteSnapshots
      );
      await this.db
   ...
   * @param wsAndOwner
   */
  public async garbageCollectPrebuild(
    ctx: TraceContext,
    ws: WorkspaceAndOwner
  ): Promise<boolean> {
  ...
  protected async deleteWorkspaceStorage(
    ...
    await this.storageClient.deleteWorkspaceBackups(
      ws.ownerId,
      ws.id,
      includeSnapshots
    );
