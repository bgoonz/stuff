...
export class ConfigProvider {
  ...
  @inject(ConfigInferenceProvider)
  protected readonly inferingConfigProvider: ConfigInferenceProvider;
  ...
  public async fetchConfig(
    ...
    try {
      ...
      if (!config.image) {
        ...
        let dockerfilePath = [configBasePath, config.image.file]
          .filter((s) => !!s)
          .join("/");
        ...
        if (
          !(
            AdditionalContentContext.is(commit) &&
            commit.additionalFiles[dockerfilePath]
          )
        ) {
  ...
  protected async fetchCustomConfig(
    ...
    { customConfig: WorkspaceConfig; configBasePath: string } | undefined
  > {
    ...
    try {
      ...
      if (
        AdditionalContentContext.is(commit) &&
        commit.additionalFiles[".gitpod.yml"]
      ) {
      ...
      if (!customConfig) {
        ...
        const contextRepoConfig = services.fileProvider.getGitpodFileContent(
          commit,
          user
        );
        ...
        const definitelyGpConfig = this.fetchExternalGitpodFileContent(
          { span },
          commit.repository
        );
        ...
        if (customConfigString) {
          ...
          customConfig._origin = fromProjectDB
            ? "project-db"
            : fromDefinitelyGp
            ? "definitely-gp"
            : "repo";
  ...
  protected async fetchWorkspaceImageSourceDocker(
    ...
    try {
      ...
      if (!hostContext || !hostContext.services) {
        throw new Error(
          `Cannot fetch workspace image source for host: ${host}`
        );
      }
      const repoHost = hostContext.services;
      const lastDockerFileSha =
        await repoHost.fileProvider.getLastChangeRevision(
          repository,
          revisionOrTagOrBranch,
          user,
          dockerFilePath
      ...
      const possibleConfigs = [
        [
          this.fetchDefinitelyGpContent(
            { span },
            `${ownerConfigBasePath}/.gitpod.yml`
          ),
          ownerConfigBasePath,
        ],
        [
  ...
  protected async fetchDefinitelyGpContent(
    ctx: TraceContext,
    filePath: string
  ) {
  ...
  protected async validateConfig(
    ...
    if (checkoutLocation) {
      const normalizedPath = path.join(
        POD_PATH_WORKSPACE_BASE,
        checkoutLocation
      );
      if (this.leavesWorkspaceBase(normalizedPath)) {
        log.error(
          { userId: user.id },
          `Invalid checkout location. Would end up at ${normalizedPath}`
        );
        throw new Error(
    ...
    if (workspaceLocation) {
      ...
      if (this.leavesWorkspaceBase(normalizedPath)) {
        log.error(
          { userId: user.id },
          `Invalid workspace location. Would end up at ${normalizedPath}`
        );
        throw new Error(
  ...
  protected leavesWorkspaceBase(normalizedPath: string) {
    ...
    return (
      ...
      pathSegments.slice(0, 2).join("/") != POD_PATH_WORKSPACE_BASE
    );
  }
}
...
export namespace InvalidGitpodYMLError {
  export function is(obj: object): obj is InvalidGitpodYMLError {
    return (
      "errorType" in obj &&
      (obj as any).errorType === "invalidGitpodYML" &&
