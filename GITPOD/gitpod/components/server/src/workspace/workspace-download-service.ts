    ...
    router.get("/get/:id", async (req, res, next) => {
      ...
      try {
        const wsi = await this.workspaceDB
          .trace({})
          .findWorkspaceAndInstance(workspaceId);
        ...
        if (
          ...
          log.warn(
            { workspaceId, userId },
            "user attempted to download someone else's workspace"
          );
          res.sendStatus(500);
          return;
        }
        ...
        const signedUrl =
          await this.storageClient.createWorkspaceContentDownloadUrl(
            userId,
            workspaceId
          );
        ...
        log.info(
          { workspaceId, userId },
          "user is downloading workspace content"
        );
        res.send(signedUrl);
      } catch (err) {
