...
class TestEnvvarPrefixParser {
  ...
  public testFindPrefixGood() {
    ...
    expect(this.findPrefix("foo1=bar1,foo2=bar2/http...")).to.be.equal(
      "foo1=bar1,foo2=bar2/"
    );
  }
  ...
  public testFindPrefixSomewhatOk() {
    expect(this.findPrefix("foo==bar,,foo2=bar2/http...")).to.be.equal(
      "foo==bar,,foo2=bar2/"
    );
    expect(this.findPrefix("foo1=bar1,foo2=bar2/http...")).to.be.equal(
      "foo1=bar1,foo2=bar2/"
  ...
  public async testHandleGood() {
    ...
    expect(await this.parseAndFormat("foo1=bar1,foo2=bar2/")).to.deep.equal({
      foo1: "bar1",
      foo2: "bar2",
    });
    expect(
      await this.parseAndFormat("foo1=bar1,foo2=bar2,foo3=bar3/")
    ).to.deep.equal({ foo1: "bar1", foo2: "bar2", foo3: "bar3" });
    expect(
      await this.parseAndFormat("foo1=bar1,foo2=bar2,foo3=bar3,foo4=bar4/")
    ).to.deep.equal({ foo1: "bar1", foo2: "bar2", foo3: "bar3", foo4: "bar4" });
  }

  @test
  public async testHandleDuplicate() {
    expect(
      await this.parseAndFormat("foo1=bar1,foo2=bar21,foo2=bar22/")
    ).to.deep.equal({ foo1: "bar1", foo2: "bar22" });
    expect(
      await this.parseAndFormat("foo1=bar11,foo1=bar12,foo3=bar31,foo3=bar32/")
    ).to.deep.equal({ foo1: "bar12", foo3: "bar32" });
  }
  ...
  public async testHandleBad() {
    ...
    expect(await this.parseAndFormat("foo==bar,,foo2=bar2/")).to.deep.equal({
      foo2: "bar2",
    });
    expect(await this.parseAndFormat("fo% 1=bar1,foo2=bar2/")).to.deep.equal({
      foo2: "bar2",
    });
  }
