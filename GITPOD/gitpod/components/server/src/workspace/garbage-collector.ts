...
export class WorkspaceGarbageCollector {
  @inject(ConsensusLeaderQorum)
  protected readonly leaderQuorum: ConsensusLeaderQorum;
  @inject(WorkspaceDeletionService)
  protected readonly deletionService: WorkspaceDeletionService;
  ...
  public async garbageCollectWorkspacesIfLeader() {
    if (await this.leaderQuorum.areWeLeader()) {
      ...
      this.softDeleteOldWorkspaces().catch((err) =>
      ...
      this.deleteOldPrebuilds().catch((err) =>
  ...
  protected async softDeleteOldWorkspaces() {
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("softDeleteOldWorkspaces");
    try {
      const workspaces = await this.workspaceDB
        .trace({ span })
        .findWorkspacesForGarbageCollection(
          this.env.daysBeforeGarbageCollection,
          this.env.garbageCollectionLimit
        );
      const deletes = await Promise.all(
        workspaces.map((ws) =>
          this.deletionService.softDeleteWorkspace({ span }, ws, "gc")
        )
      );
  ...
  protected async deleteWorkspaceContentAfterRetentionPeriod() {
    const span = opentracing
      .globalTracer()
      .startSpan("deleteWorkspaceContentAfterRetentionPeriod");
    try {
      const workspaces = await this.workspaceDB
        .trace({ span })
        .findWorkspacesForContentDeletion(
          this.env.workspaceDeletionRetentionPeriodDays,
          this.env.workspaceDeletionLimit
        );
      const deletes = await Promise.all(
        workspaces.map((ws) =>
      ...
      log.info(
        `wsgc: successfully deleted the content of ${deletes.length} workspaces`
      );
      span.addTags({ nrOfCollectedWorkspaces: deletes.length });
    } catch (err) {
      TraceContext.logError({ span }, err);
    ...
    try {
      const workspaces = await this.workspaceDB
        ...
        .findPrebuiltWorkspacesForGC(
          this.env.daysBeforeGarbageCollectingPrebuilds,
          this.env.workspaceDeletionLimit
        );
      const deletes = await Promise.all(
        workspaces.map((ws) =>
          this.deletionService.garbageCollectPrebuild({ span }, ws)
        )
      );
