...
> implements GitpodServer, Disposable
{
  ...
  protected readonly workspaceStarter: WorkspaceStarter;
  @inject(WorkspaceManagerClientProvider)
  protected readonly workspaceManagerClientProvider: WorkspaceManagerClientProvider;
  @inject(ImageBuilderClientProvider)
  protected imageBuilderClientProvider: ImageBuilderClientProvider;

  @inject(UserDB) protected readonly userDB: UserDB;
  @inject(TokenProvider) protected readonly tokenProvider: TokenProvider;
  @inject(UserService) protected readonly userService: UserService;
  @inject(UserMessageViewsDB)
  protected readonly userMessageViewsDB: UserMessageViewsDB;
  @inject(UserStorageResourcesDB)
  protected readonly userStorageResourcesDB: UserStorageResourcesDB;
  @inject(UserDeletionService)
  protected readonly userDeletionService: UserDeletionService;
  @inject(IAnalyticsWriter) protected readonly analytics: IAnalyticsWriter;
  @inject(AuthorizationService)
  protected readonly authorizationService: AuthorizationService;
  @inject(TeamDB) protected readonly teamDB: TeamDB;

  @inject(AppInstallationDB)
  protected readonly appInstallationDB: AppInstallationDB;

  @inject(IClientDataPrometheusAdapter)
  protected readonly clientDataPrometheusAdapter: IClientDataPrometheusAdapter;

  @inject(TheiaPluginService)
  protected readonly pluginService: TheiaPluginService;
  ...
  @inject(GitTokenScopeGuesser)
  protected readonly gitTokenScopeGuesser: GitTokenScopeGuesser;

  @inject(HeadlessLogService)
  protected readonly headlessLogService: HeadlessLogService;
  ...
  protected checkAndBlockUser(methodName?: string, logPayload?: {}): User {
    ...
    if (user.blocked) {
      ...
      if (logPayload) {
        log.info(
          userContext,
          `${methodName || "checkAndBlockUser"}: blocked`,
          logPayload
        );
      } else {
  ...
  protected async doUpdateUser(): Promise<void> {
    ...
    if (!this.showSetupCondition || this.showSetupCondition.value === true) {
      const hasAnyStaticProviders = this.hostContextProvider
        .getAll()
        .some((hc) => hc.authProvider.config.builtin === true);
  ...
  protected async checkTermsAcceptance() {
    if (!this.termsAccepted) {
      if (this.user) {
        this.termsAccepted = await this.userService.checkTermsAccepted(
          this.user
        );
      }
    }
    if (!this.termsAccepted) {
      throw new ResponseError(
        ErrorCodes.USER_TERMS_ACCEPTANCE_REQUIRED,
  ...
  public async getAuthProviders(): Promise<AuthProviderInfo[]> {
    ...
    for (const info of authProviders) {
      const identity = this.user.identities.find(
        (i) => i.authProviderId === info.authProviderId
      );
      if (identity) {
  ...
  public async getToken(
    ...
    try {
      ...
      await this.guardAccess(
        { kind: "token", subject: token, tokenOwnerID: user.id },
        "get"
      );
  ...
  public async getPortAuthenticationToken(workspaceId: string): Promise<Token> {
    const user = this.checkAndBlockUser("getPortAuthenticationToken", {
      workspaceId,
    });
    const span = opentracing
      .globalTracer()
      .startSpan("getPortAuthenticationToken");
    span.setTag("workspaceId", workspaceId);

    const workspace = await this.workspaceDb
      .trace({ span })
      .findById(workspaceId);
    await this.guardAccess({ kind: "workspace", subject: workspace! }, "get");

    try {
      const token = await this.tokenProvider.getFreshPortAuthenticationToken(
        user,
        workspaceId
      );
      await this.guardAccess(
  ...
  public async getWorkspace(id: string): Promise<WorkspaceInfo> {
    ...
    try {
      const workspace = await this.internalGetWorkspace(
        id,
        this.workspaceDb.trace({ span })
      );
      await this.guardAccess({ kind: "workspace", subject: workspace }, "get");

      const latestInstance = await this.workspaceDb
        .trace({})
        .findCurrentInstance(id);
      if (!!latestInstance) {
        await this.guardAccess(
          {
  ...
  public async startWorkspace(
    ...
    try {
      ...
      const mayStartPromise = this.mayStartWorkspace(
        { span },
        user,
        this.workspaceDb.trace({ span }).findRegularRunningInstances(user.id)
      );
      const workspace = await this.internalGetWorkspace(
        workspaceId,
        this.workspaceDb.trace({ span })
      ...
      const runningInstance = await this.workspaceDb
        .trace({ span })
        .findRunningInstance(workspace.id);
      if (runningInstance) {
      ...
      if (workspace.type !== "regular") {
        throw new ResponseError(
          ErrorCodes.PERMISSION_DENIED,
          "Cannot (re-)start irregular workspace."
        );
      }
      ...
      if (workspace.deleted) {
        throw new ResponseError(
          ErrorCodes.PERMISSION_DENIED,
          "Cannot (re-)start a deleted workspace."
        );
      }
      ...
      return await this.workspaceStarter.startWorkspace(
        { span },
        workspace,
        user,
        await envVars,
  ...
  protected async internalStopWorkspace(
    ...
    const instance = await this.workspaceDb
      .trace(ctx)
      .findRunningInstance(workspaceId);
    if (!instance) {
    ...
    await this.internalStopWorkspaceInstance(
      ctx,
      instance.id,
      instance.region,
      policy
  ...
  public async deleteWorkspace(id: string): Promise<void> {
    ...
    try {
      const ws = await this.internalGetWorkspace(
        id,
        this.workspaceDb.trace({})
      );
      ...
      await this.workspaceDeletionService.softDeleteWorkspace(
        { span },
        ws,
        "user"
      );
  ...
  public async setWorkspaceDescription(
    id: string,
    description: string
  ): Promise<void> {
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("setWorkspaceDescription");
    span.setTag("workspaceId", id);
    span.setTag("userId", user.id);
    ...
    try {
      ...
      await this.guardAccess(
        { kind: "workspace", subject: workspace },
        "update"
      );
  ...
  public async getWorkspaces(
    ...
    await Promise.all(
      res.map((ws) =>
        this.guardAccess({ kind: "workspace", subject: ws.workspace }, "get")
      )
    );
    await Promise.all(
      res.map((ws) =>
        this.guardAccess(
          {
            kind: "workspaceInstance",
            subject: ws.latestInstance,
            workspaceOwnerID: ws.workspace.ownerId,
            workspaceIsShared: ws.workspace.shareable || false,
          },
          "get"
        )
      )
  ...
  public async sendHeartBeat(
    ...
    try {
      ...
      const wsi = await this.workspaceDb
        .trace({ span })
      ...
      const ws = await this.workspaceDb
        .trace({ span })
        .findById(wsi.workspaceId);
      if (!ws) {
        throw new ResponseError(
          ErrorCodes.NOT_FOUND,
          "workspace does not exist"
      ...
      await this.guardAccess(
        {
          ...
          workspaceIsShared: ws.shareable || false,
        },
        "update"
      );
      ...
      await this.workspaceDb
        .trace({ span })
        .updateLastHeartbeat(instanceId, user.id, new Date(), wasClosed);
      ...
      if (
        ...
        Number.isFinite(options.roundTripTime)
      ) {
      ...
      if (
        ...
        (e.message as String).endsWith("does not exist")
      ) {
  ...
  public async getWorkspaceUsers(
    ...
    return await this.workspaceDb
      .trace({})
      .getWorkspaceUsers(workspaceId, this.env.workspaceUserTimeout);
  }

  protected async internalGetWorkspace(
    id: string,
    db: WorkspaceDB
  ): Promise<Workspace> {
    ...
    try {
      ...
      const runningInfos = await Promise.all(
        runningInstances.map(async (workspaceInstance) => {
          const workspace = await this.workspaceDb
            .trace({ span })
            .findById(workspaceInstance.workspaceId);
          if (!workspace) {
            return;
          }
      ...
      return runningInfos
        .filter(
          (info) =>
            info && info.workspace.type === "regular" && sameContext(info)
        )
        .map((info) => info!);
    } catch (e) {
      TraceContext.logError({ span }, e);
  ...
  public async isPrebuildDone(pwsid: string): Promise<boolean> {
    throw new ResponseError(
      ErrorCodes.EE_FEATURE,
      "Prebuilds are implemented in Gitpod's enterprise edition"
    );
  }
  ...
  public async createWorkspace(
    ...
    try {
      ...
      const runningInstancesPromise = this.workspaceDb
        .trace({ span })
        .findRegularRunningInstances(user.id);
      ...
      const contextPromise = this.contextParser.handle(
        ctx,
        user,
      ...
      if (
        ...
        if (!hostContext || !services) {
          ...
          if (
            await services.repositoryService.canInstallAutomatedPrebuilds(
              user,
              cloneUrl
            )
          ) {
            console.log("Installing automated prebuilds for " + cloneUrl);
            services.repositoryService.installAutomatedPrebuilds(
              user,
              cloneUrl
            );
          }
      ...
      if (
        ...
        context.forceCreateNewWorkspace !== true
      ) {
      ...
      try {
        ...
        await this.workspaceDb
          .trace({ span })
          .hardDeleteWorkspace(workspace.id);
        throw err;
      }
  ...
  protected async pollDatabaseUntilPrebuildIsAvailable(
    ...
    const pollPrebuildAvailable = new Cancelable(async (cancel) => {
      const prebuild = await this.workspaceDb
        .trace({})
        .findPrebuildByID(prebuildID);
   ...
   * @param context
   */
  protected async mayOpenContext(
    user: User,
    context: WorkspaceContext
  ): Promise<void> {}
  ...
  public async getFeaturedRepositories(): Promise<WhitelistedRepository[]> {
    ...
    const repositories = await this.workspaceDb
      .trace({})
          ...
          .map(async (whitelistedRepo) => {
            ...
            try {
              ...
              return {
                ...
                description:
                  whitelistedRepo.description || repository.description,
  ...
  public async getOpenPorts(
    ...
    try {
      ...
      if (!instance || !workspace) {
        throw new ResponseError(
          ErrorCodes.NOT_FOUND,
          `Workspace ${workspaceId} has no running instance`
        );
      }
    ...
    try {
      ...
      if (!runningInstance) {
        log.warn(
          ...
          "Cannot open port for workspace with no running instance",
          { port }
        );
        return;
      }
  ...
  protected portVisibilityFromProto(
    visibility: ProtoPortVisibility
  ): PortVisibility {
    switch (visibility) {
  ...
  protected portVisibilityToProto(
    visibility: PortVisibility | undefined
  ): ProtoPortVisibility {
    switch (visibility) {
  ...
  public async closePort(workspaceId: string, port: number) {
    ...
    try {
      const { workspace, instance } =
        await this.internGetCurrentWorkspaceInstance(user.id, workspaceId);
  ...
  async watchWorkspaceImageBuildLogs(workspaceId: string): Promise<void> {
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("watchWorkspaceImageBuildLogs");
    ...
    if (!workspace.imageNameResolved) {
      log.warn(
        `No imageNameResolved set for workspaceId ${workspaceId}, cannot watch logs.`
      );
      return;
    }
    ...
    try {
      ...
      imgbuilder.logs({ span }, req, (data) => {
        ...
        lineCount += data.split(
          WorkspaceImageBuild.LogLine.DELIMITER_REGEX
        ).length;
  ...
  async getHeadlessLog(instanceId: string): Promise<HeadlessLogUrls> {
    ...
    if (!ws) {
      throw new ResponseError(
        ErrorCodes.NOT_FOUND,
        `Workspace ${instanceId} not found`
      );
    }
    ...
    if (!wsi) {
      throw new ResponseError(
        ErrorCodes.NOT_FOUND,
        `Workspace instance for ${instanceId} not found`
      );
    }
    ...
    if (
      !urls ||
      (typeof urls.streams === "object" &&
        Object.keys(urls.streams).length === 0)
    ) {
      throw new ResponseError(
        ErrorCodes.NOT_FOUND,
        `Headless logs for ${instanceId} not found`
      );
    }
    return urls;
  }
  ...
  async getUserStorageResource(
    ...
    const userId = this.checkUser("getUserStorageResource", {
      uri: options.uri,
    }).id;
  ...
  async updateUserStorageResource(
    ...
    const userId = this.checkAndBlockUser("updateUserStorageResource", {
      uri: options.uri,
    }).id;
    const uri = options.uri;
    ...
    await this.guardAccess(
      { kind: "userStorage", uri, userID: userId },
      "update"
    );
  ...
  async sendFeedback(feedback: string): Promise<string | undefined> {
    throw new ResponseError(
      ErrorCodes.EE_FEATURE,
      "Sending feedback is not implemented"
    );
  }
  ...
  async takeSnapshot(
    options: GitpodServer.TakeSnapshotOptions
  ): Promise<string> {
    throw new ResponseError(
      ErrorCodes.EE_FEATURE,
  ...
  async getLayout(workspaceId: string): Promise<string | undefined> {
    ...
    const layoutData = await this.workspaceDb
      .trace({})
      .findLayoutDataByWorkspaceId(workspaceId);
    if (!layoutData) {
      return;
    }
  ...
  async getEnvVars(): Promise<UserEnvVarValue[]> {
    ...
    for (const value of await this.userDB.getEnvVars(user.id)) {
      if (
        !(await this.resourceAccessGuard.canAccess(
          { kind: "envVar", subject: value },
          "get"
        ))
      ) {
        continue;
    ...
    return [...result.values()].map(
      ({ value: { id, name, value, repositoryPattern } }) => ({
        id,
        name,
        value,
        repositoryPattern,
  ...
  async setEnvVar(variable: UserEnvVarValue): Promise<void> {
    ...
    variable.repositoryPattern = UserEnvVar.normalizeRepoPattern(
      variable.repositoryPattern
    );
    const existingVars = (await this.userDB.getEnvVars(user.id)).filter(
      (v) => !v.deleted
    );
    ...
    const existingVar = existingVars.find(
      (v) =>
        ...
        v.repositoryPattern == variable.repositoryPattern
    );
    if (!!existingVar) {
    ...
    if (!variable.id) {
      ...
      if (varCount > this.env.maxUserEnvvarCount) {
        throw new ResponseError(
          ErrorCodes.PERMISSION_DENIED,
          `cannot have more than ${this.env.maxUserEnvvarCount} environment variables`
        );
      }
    }
  ...
  public async removeTeamMember(teamId: string, userId: string): Promise<void> {
    ...
    await this.guardTeamOperation(
      teamId,
      user.id === userId ? "get" : "update"
    );
  ...
  public async resetGenericInvite(
    teamId: string
  ): Promise<TeamMembershipInvite> {
    this.checkAndBlockUser("resetGenericInvite");
    await this.guardTeamOperation(teamId, "update");
    return this.teamDB.resetGenericInvite(teamId);
  }

  protected async guardProjectOperation(
    user: User,
    projectId: string,
    op: ResourceAccessOp
  ): Promise<void> {
  ...
  public async getProviderRepositoriesForUser(params: {
    provider: string;
  ...
  public async findPrebuilds(
    params: FindPrebuildsParams
  ): Promise<PrebuildInfo[]> {
  ...
  public async getProjectOverview(
    projectId: string
  ): Promise<Project.Overview | undefined> {
  ...
  public async setProjectConfiguration(
    projectId: string,
    configString: string
  ): Promise<void> {
    ...
    if (parseResult.validationErrors) {
      throw new Error(
        `This configuration could not be parsed: ${parseResult.validationErrors.join(
          ", "
        )}`
      );
    }
    await this.projectsService.setProjectConfiguration(projectId, {
      ".gitpod.yml": configString,
    });
  }
  ...
  public async fetchProjectRepositoryConfiguration(
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("fetchProjectRepositoryConfiguration");
    ...
    const normalizedContextUrl = this.contextParser.normalizeContextURL(
      project.cloneUrl
    );
    ...
    if (!hostContext || !hostContext.services) {
      throw new Error(
        `Cannot fetch repository configuration for host: ${host}`
      );
    }
    const repoHost = hostContext.services;
    const configString = await repoHost.fileProvider.getGitpodFileContent(
      context,
      user
    );
    return configString;
  ...
  public async getContentBlobUploadUrl(name: string): Promise<string> {
    ...
    await this.guardAccess(
      { kind: "contentBlob", name: name, userID: user.id },
      "create"
    );
    ...
    const uploadUrlPromise = new Promise<UploadUrlResponse>(
      (resolve, reject) => {
        this.blobServiceClient.uploadUrl(
          uploadUrlRequest,
          (err: any, resp: UploadUrlResponse) => {
            if (err) {
              reject(err);
            } else {
              resolve(resp);
  ...
  public async getContentBlobDownloadUrl(name: string): Promise<string> {
    ...
    const downloadUrlPromise = new Promise<DownloadUrlResponse>(
      (resolve, reject) => {
        this.blobServiceClient.downloadUrl(
          downloadUrlRequest,
          (err: any, resp: DownloadUrlResponse) => {
            if (err) {
              reject(err);
            } else {
              resolve(resp);
  ...
  public async getGitpodTokens(): Promise<GitpodToken[]> {
    ...
    const res = (await this.userDB.findAllGitpodTokensOfUser(user.id)).filter(
      (v) => !v.deleted
    );
    await Promise.all(
      res.map((tkn) =>
        this.guardAccess({ kind: "gitpodToken", subject: tkn }, "get")
      )
    );
    return res;
  }

  public async generateNewGitpodToken(options: {
    name?: string;
  ...
  public async deleteGitpodToken(tokenHash: string): Promise<void> {
    ...
    if (!tkn) {
      throw new Error(
        `User ${user.id} tries to delete a token ${tokenHash} that does not exist.`
      );
    }
  ...
  async resolvePlugins(
    ...
    const result = await this.pluginService.resolvePlugins(
      workspace.ownerId,
      params
    );
  ...
  async adminGetUsers(
    ...
    throw new ResponseError(
      ErrorCodes.EE_FEATURE,
      `Admin support is implemented in Gitpod's Enterprise Edition`
    );
  }
  ...
  async adminModifyRoleOrPermission(
  ...
  async getLicenseInfo(): Promise<GetLicenseInfoResult> {
    throw new ResponseError(
      ErrorCodes.EE_FEATURE,
      `Licensing is implemented in Gitpod's Enterprise Edition`
    );
  }
  ...
  async getOwnAuthProviders(): Promise<AuthProviderEntry[]> {
    const redacted = (entry: AuthProviderEntry) =>
      AuthProviderEntry.redact(entry);
    let userId: string;
    try {
    ...
    const ownAuthProviders =
      await this.authProviderService.getAuthProvidersOfUser(userId);
  ...
  async updateOwnAuthProvider({
    entry,
  }: GitpodServer.UpdateOwnAuthProviderParams): Promise<AuthProviderEntry> {
    let userId: string;
    try {
    ...
    if (userId !== entry.ownerId) {
      throw new ResponseError(
        ErrorCodes.PERMISSION_DENIED,
        "Not allowed to modify this resource."
      );
    }
    ...
    try {
      if ("host" in safeProvider) {
        ...
        if (hostContext) {
          const builtInExists =
            hostContext.authProvider.config.ownerId === undefined;
          log.debug(`Attempt to override existing auth provider.`, {
            entry,
            safeProvider,
            builtInExists,
          });
      ...
      const result = await this.authProviderService.updateAuthProvider(
        safeProvider
      );
      ...
      const message =
        error && error.message
          ? error.message
          : "Failed to update the provider.";
      throw new ResponseError(ErrorCodes.CONFLICT, message);
    }
  }
  protected redactUpdateOwnAuthProviderParams({
    entry,
  }: GitpodServer.UpdateOwnAuthProviderParams) {
    const safeEntry =
      "id" in entry
  ...
  async deleteOwnAuthProvider(
    ...
    const ownProviders = await this.authProviderService.getAuthProvidersOfUser(
      userId
    );
  ...
  async adminGetAccountStatement(userId: string): Promise<AccountStatement> {
    throw new ResponseError(
      ErrorCodes.SAAS_FEATURE,
      `Not implemented in this version`
    );
  }
  async adminSetProfessionalOpenSource(
    userId: string,
  ...
  async adminAddStudentEmailDomain(
    userId: string,
    domain: string
  ): Promise<void> {
    throw new ResponseError(
