...
export class ContextParser {
  @multiInject(IPrefixContextParser)
  protected readonly prefixParser: IPrefixContextParser[];
  @multiInject(IContextParser)
  protected readonly contextParsers: IContextParser[];
  ...
  public async handle(
    ...
    try {
      ...
      if (prefixResult) {
        contextURL = this.normalizeContextURL(
          contextURL.substring(prefixResult.prefix.length)
        );
      }
      ...
      if (prefixResult) {
        result = await prefixResult.parser.handle(
          user,
          prefixResult.prefix,
          result
        );
  ...
  protected findPrefix(
    user: User,
    context: string
  ): { prefix: string; parser: IPrefixContextParser } | undefined {
