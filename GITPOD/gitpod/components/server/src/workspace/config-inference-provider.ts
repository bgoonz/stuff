...
export type LanguageConfigProvider = (
  language: string,
  user: User,
...
export class ConfigInferenceProvider {
  ...
  protected readonly languageConfigProvider = new Map<
    string,
  ...
  public async infer(
    ...
    const languages = await repoHost.languagesProvider.getLanguages(
      commit.repository,
      user
    );
  ...
  protected async computeGoConfig(
    lang: String,
    user: User,
    commit: CommitContext
  ): Promise<WorkspaceConfig | undefined> {
