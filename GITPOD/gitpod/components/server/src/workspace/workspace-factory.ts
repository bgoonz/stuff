...
export class WorkspaceFactory {
  ...
  protected imageSourceProvider: ImageSourceProvider;
  ...
  public async createForContext(
    ...
    if (SnapshotContext.is(context)) {
      ...
      return this.createForWorkspaceProbe(
        ctx,
        user,
        context,
    ...
    log.error(
      { userId: user.id },
      "Couldn't create workspace for context",
      context
    );
  ...
  protected async createForSnapshot(
    ...
    try {
      const snapshot = await this.db
        .trace({ span })
        .findSnapshotById(context.snapshotId);
      if (!snapshot) {
      ...
      const workspace = await this.db
        .trace({ span })
        .findById(snapshot.originalWorkspaceId);
      if (!workspace) {
        throw new Error(
      ...
      if (
        ...
        throw new Error(
          `The original workspace has been deleted - cannot open this snapshot.`
        );
      }
  ...
  protected async createForCommit(
    ...
    try {
      ...
      if (config._origin === "project-db") {
        ...
        if (project?.config) {
          (context as any as AdditionalContentContext).additionalFiles = {
            ...project.config,
          };
        }
      }
  ...
  protected async isRepositoryOrSourceWhitelisted(
    repository: Repository
  ): Promise<boolean> {
    const repoIsWhiteListed = await this.db
      .trace({})
      .isWhitelisted(repository.cloneUrl);
