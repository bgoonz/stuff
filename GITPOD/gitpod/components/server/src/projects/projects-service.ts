...
export class ProjectsService {
  ...
  protected getRepositoryProvider(project: Project) {
    ...
    const repositoryProvider =
      parsedUrl &&
      this.hostContextProvider.get(parsedUrl.host)?.services
        ?.repositoryProvider;
  ...
  async getBranchDetails(
    ...
    if (branchName) {
      const details = await repositoryProvider.getBranch(
        user,
        owner,
        repo,
        branchName
      ...
      branches.push(
        ...(await repositoryProvider.getBranches(user, owner, repo))
      );
    }
  ...
  protected async getLastPrebuild(
    project: Project,
    branch: Branch
  ): Promise<PrebuildInfo | undefined> {
    const prebuilds = await this.workspaceDb
      .trace({})
      .findPrebuiltWorkspacesByProject(project.id, branch?.name);
  ...
  async findPrebuilds(
    ...
    const repositoryProvider =
      this.hostContextProvider.get(host)?.services?.repositoryProvider;
    if (!repositoryProvider) {
      return [];
    }
    ...
    if (prebuildId) {
      const pbws = await this.workspaceDb
        .trace({})
        .findPrebuiltWorkspacesById(prebuildId);
      if (pbws) {
        prebuilds.push(pbws);
      }
    } else {
      ...
      prebuilds = await this.workspaceDb
        .trace({})
    ...
    for (const prebuild of prebuilds) {
      try {
        const commit = await repositoryProvider.getCommitInfo(
          user,
          owner,
          repo,
          prebuild.commit
        ...
        log.debug(`Could not fetch commit info.`, error, {
          owner,
          repo,
          prebuildCommit: prebuild.commit,
        });
      }
    }
    return result;
  ...
  protected async toPrebuildInfo(
    project: Project,
