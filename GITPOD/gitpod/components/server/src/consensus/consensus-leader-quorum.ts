...
export class ConsensusLeaderQorum implements Disposable {
  @inject(ConsensusLeaderMessenger)
  protected readonly messenger: ConsensusLeaderMessenger;
  ...
  public async start() {
    ...
    this.clock = setInterval(
      () =>
        this.beatClock().catch((err) => log.error("consensus beatClock", err)),
      this.clockPeriod
    );
    this.disposables.push({ dispose: () => clearTimeout(this.clock!) });
    this.disposables.push(
      this.messenger.on("heartbeat", (msg) => this.messages.push(msg))
    );
    this.disposables.push(
      this.messenger.on("requestVote", (msg) => this.messages.push(msg))
    );
    this.disposables.push(
      this.messenger.on("castVote", (msg) => this.messages.push(msg))
    );
  }
  ...
  protected async beatClock() {
    ...
    if (this.role === "leader") {
      if (
        !this.lastHeartbeatSend ||
        Date.now() - this.lastHeartbeatSend > this.heartbeatPeriod
      ) {
    ...
    if (this.role === "follower") {
      ...
      if (
        Date.now() - this.lastHeartbeatFromLeader! >=
          this.termTimeoutMilliseconds &&
        !this.votedFor
      ) {
    ...
    if (this.role === "candidate") {
      if (!this.electionDeadline) {
        ...
        this.electionDeadline =
          Date.now() +
          (Math.random() * this.electionTimeoutVariation +
  ...
  protected async startElection() {
    const electionTimeout =
      Math.random() * this.electionTimeoutVariation + this.electionTimeout;
