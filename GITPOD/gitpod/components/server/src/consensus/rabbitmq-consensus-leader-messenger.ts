  ...
  implements ConsensusLeaderMessenger
{
  ...
  protected async setupExchangeAndQueue() {
    ...
    await channel.assertExchange(exchangeConsensusLeader, "fanout", {
      durable: false,
    });
  ...
  protected async doRegister(uid: string) {
    ...
    await this.channel.consume(queueConsensusPeers, (message) => {}, {
      noAck: true,
      consumerTag: uid,
    });
  }
  ...
  on(event: ConsensusLeaderMessageType, cb: (msg: any) => void): Disposable {
    ...
    this.listen(
      new EventListener(exchangeConsensusLeader, forwarder),
