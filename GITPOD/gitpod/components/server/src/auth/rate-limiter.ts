...
export class UserRateLimiter {
  ...
  async consume(user: string, method: string): Promise<RateLimiterRes> {
    const group =
      this.config.functions[method as GitpodServerMethodType]?.group ||
      "default";
    if (
      group !== this.config.functions[method as GitpodServerMethodType]?.group
    ) {
      log.warn(
        `method '${method}' is not configured for a rate limiter, using 'default'`
      );
    }
    ...
    const points =
      this.config.functions[method as GitpodServerMethodType]?.points || 1;
