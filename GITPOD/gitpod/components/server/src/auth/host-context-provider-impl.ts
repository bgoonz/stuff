...
export class HostContextProviderImpl implements HostContextProvider {
  ...
  protected async updateDynamicHosts() {
    ...
    for (const config of all) {
      ...
      const existingConfig =
        existingContext && existingContext.authProvider.config;
      if (existingConfig && config.id === existingConfig.id) {
        if (existingConfig.host !== config.host) {
          log.warn("Ignoring host update for dynamic Auth Provider: " + host, {
            config,
            existingConfig,
          });
          continue;
        ...
        if (
          JSON.stringify(existingConfig.oauth) ===
            JSON.stringify(config.oauth) &&
          existingConfig.status === config.status
        ) {
          continue;
        }
        log.debug("Updating existing dynamic Auth Provider: " + host, {
          config,
          existingConfig,
        });
      } else {
    ...
    const tobeRemoved = [...this.dynamicHosts.keys()].filter(
      (h) => !currentHosts.has(h)
    );
  ...
  get(hostname: string): HostContext | undefined {
    ...
    const hostContext =
      this.fixedHosts.get(hostname) || this.dynamicHosts.get(hostname);
    if (!hostContext) {
