...
export class GenericAuthProvider implements AuthProvider {
  ...
  @inject(LoginCompletionHandler)
  protected readonly loginCompletionHandler: LoginCompletionHandler;

  @postConstruct()
  init() {
    this.initAuthUserSetup();
    log.info(`(${this.strategyName}) Initialized.`, {
      defaultStrategyOptions: this.defaultStrategyOptions,
    });
  }
  ...
  async refreshToken(user: User) {
    log.info(`(${this.strategyName}) Token to be refreshed.`, {
      userId: user.id,
    });
    ...
    try {
      ...
      log.error(`(${this.strategyName}) Failed to refresh token!`, {
        error,
        token,
      });
      throw error;
    ...
    if (configFn) {
      this.readAuthUserSetup = async (
        ...
        try {
          ...
          log.error(
            `(${this.strategyName}) Failed to call "fetchAuthUserSetup"`,
            { error, configFn, accessToken }
          );
        ...
        try {
          ...
          log.error(`(${this.strategyName}) Failed to run "configFn"`, {
            error,
            configFn,
            accessToken,
          });
  ...
  async callback(
    ...
    if (response.headersSent) {
      log.warn(cxt, `(${strategyName}) Callback called repeatedly.`, {
        request,
        clientInfo,
      });
      return;
    ...
    const isAlreadyLoggedIn =
      request.isAuthenticated() && User.is(request.user);
    ...
    if (!authFlow) {
      ...
      log.error(
        cxt,
        `(${strategyName}) No session found during auth callback.`,
        { request, clientInfo }
      );
      response.redirect(
        this.getSorryUrl(
    ...
    if (authFlow.host !== this.host) {
      ...
      log.error(cxt, `(${strategyName}) Host does not match.`, {
        request,
        clientInfo,
      });
    ...
    // check OAuth2 errors
    const callbackParams = new URL(`https://anyhost${request.originalUrl}`)
      .searchParams;
    const callbackError = callbackParams.get("error");
    const callbackErrorDescription: string | null =
    ...
    const context = LogContext.from({
      user: User.is(user) ? { userId: user.id } : undefined,
      request,
    });
    ...
    if (err) {
      ...
      log.error(
        context,
        `(${strategyName}) Redirect to /sorry from verify callback`,
        err,
        { ...defaultLogPayload, err }
      );
    ...
    if (flowContext) {
      if (
        ...
        (TosFlow.WithUser.is(flowContext) &&
          flowContext.termsAcceptanceRequired)
      ) {
        ...
        response.redirect(
          this.env.hostUrl
            .withApi({ pathname: "/tos", search: "mode=login" })
            .toString()
        );
        return;
      } else {
  ...
  protected sendCompletionRedirectWithError(
    response: express.Response,
    error: object
  ): void {
    log.info(`(${this.strategyName}) Send completion redirect with error`, {
      error,
    });

    const url = this.env.hostUrl
      .with({
        pathname: "/complete-auth",
        search:
          "message=error:" +
          Buffer.from(JSON.stringify(error), "utf-8").toString("base64"),
      })
      .toString();
  ...
  protected async verify(
    ...
    let currentGitpodUser: User | undefined = User.is(req.user)
      ? req.user
      : undefined;
    ...
    try {
      ...
      const { authUser, currentScopes, envVars } =
        await this.fetchAuthUserSetup(accessToken, tokenResponseObject);
      const { authName, primaryEmail } = authUser;
      candidate = { authProviderId, ...authUser };

      log.info(`(${strategyName}) Verify function called for ${authName}`, {
        ...defaultLogPayload,
        authUser,
      });
      ...
      if (currentGitpodUser) {
        ...
        if (currentIdentity && currentIdentity.authId !== candidate.authId) {
          log.warn(
            `User is trying to connect with another provider identity.`,
            {
              ...defaultLogPayload,
              authUser,
              candidate,
        ...
        currentGitpodUser = await this.userService.findUserForLogin({
          candidate,
        });
        ...
        if (!currentGitpodUser) {
          ...
          const existingUserWithSameEmail = (
            await this.userDb.findUsersByEmail(primaryEmail)
          )[0];
      ...
      if (currentGitpodUser) {
        const termsAcceptanceRequired =
          await this.userService.checkTermsAcceptanceRequired({
            config,
            identity: candidate,
        ...
        const elevateScopes = authFlow.overrideScopes
          ? undefined
          : await this.getMissingScopeForElevation(
              currentGitpodUser,
              currentScopes
            );
        const isBlocked = await this.userService.isBlocked({
          user: currentGitpodUser,
        });

        await this.userService.updateUserOnLogin(
          currentGitpodUser,
          authUser,
          candidate,
          token
        );
        await this.userService.updateUserEnvVarsOnLogin(
          currentGitpodUser,
          envVars
        ...
        const { githubIdentity, githubToken } =
          this.createGhProxyIdentity(candidate);
      ...
      log.error(`(${strategyName}) Exception in verify function`, err, {
        ...defaultLogPayload,
        err,
        authFlow,
      });
  ...
  protected async getMissingScopeForElevation(
    user: User,
  ...
  protected async fetchAuthUserSetup(
    accessToken: string,
    tokenResponse: object
  ): Promise<AuthUserSetup> {
    if (!this.readAuthUserSetup) {
      throw new Error(
        `(${this.strategyName}) is missing configuration for reading of user information.`
      );
    }
  ...
  protected prevScopesAreMissing(
    currentScopes: string[],
    prevScopes: string[]
  ): boolean {
  ...
  protected createGhProxyIdentity(originalIdentity: Identity) {
    const githubTokenValue =
      this.config.params && this.config.params.githubToken;
    if (!githubTokenValue) {
      return {};
    }
  ...
  protected isOAuthError(err: any): boolean {
    if (
      typeof err === "object" &&
      (err.name == "InternalOAuthError" || err.name === "AuthorizationError")
    ) {
      return true;
    }
    return false;
  ...
  protected get defaultStrategyOptions(): StrategyOptionsWithRequest {
    ...
    const augmentedAuthParams = this.env.devBranch
      ? { ...authorizationParams, state: this.env.devBranch }
      : authorizationParams;
    return {
...
export class GenericOAuth2Strategy extends OAuth2Strategy {
  ...
  protected patch_getOAuthAccessToken() {
    ...
    (oauth2 as any).getOAuthAccessToken = (
      code: string,
      params: any,
      callback: oauth2tokenCallback
    ) => {
      const patchedCallback: oauth2tokenCallback = (
        err,
        accessToken,
        refreshToken,
        params
