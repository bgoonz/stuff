...
export class LoginCompletionHandler {
  ...
  async complete(
    ...
    response: express.Response,
    {
      ...
      authHost,
      elevateScopes,
    }: LoginCompletionHandler.CompleteParams
  ) {
    ...
    try {
      ...
      log.error(logContext, `Redirect to /sorry on login`, err, {
        err,
        session: request.session,
      });
      response.redirect(
        this.env.hostUrl
          .asSorry("Oops! Something went wrong during login.")
          .toString()
      );
      return;
    }
    ...
    if (elevateScopes) {
      const elevateScopesUrl = this.env.hostUrl
        .withApi({
          pathname: "/authorize",
          search: `returnTo=${encodeURIComponent(
            returnTo
          )}&host=${authHost}&scopes=${elevateScopes.join(",")}`,
        })
        .toString();
    ...
    log.info(
      logContext,
      `User is logged in successfully. Redirect to: ${returnTo}`,
      { session: request.session }
    );
    ...
    if (hostCtx) {
      ...
      if (!builtin && !verified) {
        try {
          ...
          log.error(
            LogContext.from({ user }),
            `Failed to mark AuthProvider as verified!`,
            { error }
          );
        }
      }
