...
export class AuthProviderService {
  ...
  async getAllAuthProviders(): Promise<AuthProviderParams[]> {
    ...
    const unique = transformed.reduce((prev, current) => {
      ...
      if (duplicate) {
        log.warn(`Duplicate dynamic Auth Provider detected.`, {
          rawResult: all,
  ...
  async getAuthProvidersOfUser(
    user: User | string
  ): Promise<AuthProviderEntry[]> {
    const result = await this.authProviderDB.findByUserId(
  ...
  async updateAuthProvider(
    ...
    if ("id" in entry) {
      ...
      const existing = (await this.authProviderDB.findByUserId(ownerId)).find(
        (p) => p.id === id
      );
      if (!existing) {
      ...
      const changed =
        ...
        (entry.clientSecret &&
          entry.clientSecret !== existing.oauth.clientSecret);
    ...
    return await this.authProviderDB.storeAuthProvider(
      authProvider as AuthProviderEntry
    );
  }
  protected initializeNewProvider(
    newEntry: AuthProviderEntry.NewEntry
  ): AuthProviderEntry {
  ...
  async markAsVerified(params: { ownerId: string; id: string }) {
    ...
    try {
      ...
      if (ap) {
        ...
        log.warn("Failed to find the AuthProviderEntry to be activated.", {
          params,
          id,
          ap,
        });
