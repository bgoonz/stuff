...
function createBearerAuthError(message: string): BearerAuthError {
  return Object.assign(new Error(message), { code: bearerAuthCode } as {
    code: typeof bearerAuthCode;
  });
}
...
export class BearerAuth {
  ...
  private async doAuth(req: express.Request): Promise<void> {
    ...
    const userAndToken = await this.userDB.findUserByGitpodToken(
      token,
      GitpodTokenType.API_AUTH_TOKEN
    );
    if (!userAndToken) {
    ...
    const resourceGuard = new TokenResourceGuard(
      userAndToken.user.id,
      userAndToken.token.scopes
    );
    ...
    if (functionScopes.length === 1 && functionScopes[0] === "*") {
      (req as WithFunctionAccessGuard).functionGuard =
      ...
      (req as WithFunctionAccessGuard).functionGuard =
        new ExplicitFunctionAccessGuard(functionScopes);
    }
