...
export class Authenticator {
  ...
  protected hostContextProvider: HostContextProvider;
  ...
  protected async authCallbackHandler(
    req: express.Request,
    res: express.Response,
    next: express.NextFunction
  ) {
  ...
  protected async getAuthProviderForHost(
    host: string
  ...
  async authenticate(
    ...
    if (
      this.env.disableDynamicAuthProviderLogin &&
      !authProvider.config.builtin
    ) {
      log.info({ sessionId: req.sessionID }, `Auth Provider is not allowed.`, {
        req,
        ap: authProvider.info,
      });
      res.redirect(
        this.getSorryUrl(
          `Login with ${authProvider.config.host} is not allowed.`
        )
      );
      return;
    }
    if (!req.session) {
      ...
      res.redirect(
        this.getSorryUrl(`No session found. Please refresh the browser.`)
      );
      return;
    }
    ...
    if (!authProvider.info.verified && !(await this.isInSetupMode())) {
      ...
      log.info(
        { sessionId: req.sessionID },
        `Login with "${host}" is not permitted.`,
        { req, "login-flow": true, ap: authProvider.info }
      );
  ...
  async deauthorize(
    ...
    const returnTo: string =
      req.query.returnTo || this.env.hostUrl.asDashboard().toString();
    ...
    try {
      ...
      log.error(
        { sessionId: req.sessionID },
        `Failed to disconnect a provider.`,
        error,
        { req, host, userId: user.id }
      );
      res.redirect(
        this.getSorryUrl(
          `Failed to disconnect a provider: ${
  ...
  async authorize(
    ...
    if (!authProvider.info.verified && user.id !== authProvider.info.ownerId) {
      log.info(
        { sessionId: req.sessionID },
        `Authorization with "${host}" is not permitted.`,
        { req, "authorize-flow": true, ap: authProvider.info }
      );
      res.redirect(
        this.getSorryUrl(`Authorization with "${host}" is not permitted.`)
      );
      return;
    }
    ...
    log.info(
      { sessionId: req.sessionID },
      `(doAuthorize) wanted scopes (${
        override ? "overriding" : "merging"
      }): ${wantedScopes.join(",")}`
    );
  ...
  protected async getCurrentScopes(user: any, authProvider: AuthProvider) {
    if (User.is(user)) {
      try {
        const token = await this.tokenProvider.getTokenForHost(
          user,
          authProvider.config.host
        );
        return token.scopes;
      } catch {
        // no token
