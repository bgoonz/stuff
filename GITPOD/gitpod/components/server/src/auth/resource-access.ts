...
export function isGuardedResourceKind(kind: any): kind is GuardedResourceKind {
  return (
    typeof kind === "string" &&
    ALL_GUARDED_RESOURCE_KINDS.has(kind as GuardedResourceKind)
  );
}
...
export class CompositeResourceAccessGuard implements ResourceAccessGuard {
  ...
  async canAccess(
    ...
    return (
      await Promise.all(
        this.children.map((c) => c.canAccess(resource, operation))
      )
    ).some((x) => x);
  }
}
        ...
        switch (operation) {
          ...
          case "delete":
            ...
            return resource.members.some(
              (m) => m.userId === this.userId && m.role === "owner"
            );
        }
      case "workspaceLog":
        return resource.subject.ownerId === this.userId;
...
export class ScopedResourceGuard<
  K extends GuardedResourceKind = GuardedResourceKind
> implements ResourceAccessGuard
{
  private readonly scopes = new Map<string, Set<ResourceAccessOp>>();

  constructor(
    scopes: ScopedResourceGuard.ResourceScope<K>[],
  ...
  protected pushScope(scope: ScopedResourceGuard.ResourceScope<K>): void {
    this.scopes.set(
      `${scope.kind}::${scope.subjectID}`,
      new Set(scope.operations)
    );
  }
}
...
export class WorkspaceEnvVarAccessGuard extends ScopedResourceGuard<"envVar"> {
  ...
  async canAccess(
    ...
    if (
      operation === "get" &&
      this.readAccessWildcardPatterns?.has(resource.subject.repositoryPattern)
    ) {
      return true;
    }
  ...
  protected pushScope(
    scope: ScopedResourceGuard.ResourceScope<"envVar">
  ): void {
    ...
    this.readAccessWildcardPatterns =
      this.readAccessWildcardPatterns || new Set<string>();
...
export namespace ScopedResourceGuard {
  ...
  export function ofKind<K extends GuardedResourceKind>(
    scope: ResourceScope,
    kind: K
  ): scope is ResourceScope<K> {
    return scope.kind === kind;
  }

  export function isAllowedUnder(
  ...
  export function marshalResourceScopeFromResource(
    resource: GuardedResource,
    ops: ResourceAccessOp[]
  ): string {
    const subjectID = ScopedResourceGuard.subjectID(resource);
    if (!subjectID) {
...
export class TokenResourceGuard implements ResourceAccessGuard {
  ...
  constructor(userID: string, protected readonly allTokenScopes: string[]) {
    const hasDefaultResourceScope = allTokenScopes.some(
      (s) => s === TokenResourceGuard.DefaultResourceScope
    );
    ...
    if (hasDefaultResourceScope) {
      ...
      const resourceScopes =
        TokenResourceGuard.getResourceScopes(allTokenScopes);
  ...
  async canAccess(
    ...
    if (resource.kind === "gitpodToken" && operation === "create") {
      return TokenResourceGuard.areScopesSubsetOf(
        this.allTokenScopes,
        resource.subject.scopes
      );
    }
...
export namespace TokenResourceGuard {
  ...
  export function getResourceScopes(
    s: string[]
  ): ScopedResourceGuard.ResourceScope[] {
    return s
      .filter((s) => s.startsWith("resource:") && s !== DefaultResourceScope)
      .map((s) =>
        ScopedResourceGuard.unmarshalResourceScope(
          s.substring("resource:".length)
        )
      );
  }

  export function areScopesSubsetOf(
    upperScopes: string[],
    lowerScopes: string[]
  ) {
    /*
    ...
    if (
      lowerScopes.includes(DefaultResourceScope) &&
      !upperScopes.includes(DefaultResourceScope)
    ) {
      return false;
    }

    const upperResourceScopes =
      TokenResourceGuard.getResourceScopes(upperScopes);
    const lowerResourceScopes =
      TokenResourceGuard.getResourceScopes(lowerScopes);

    const allNewScopesAllowed = lowerResourceScopes.every((lrs) =>
      upperResourceScopes.some((urs) =>
        ScopedResourceGuard.isAllowedUnder(urs, lrs)
      )
    );
...
export class WorkspaceLogAccessGuard implements ResourceAccessGuard {
  ...
  async canAccess(
    ...
    if (!hostContext) {
      throw new Error(
        `no HostContext found for hostname: ${contextURL.hostname}`
      );
    }
    ...
    if (!svcs) {
      throw new Error(
        `no services found in HostContext for hostname: ${contextURL.hostname}`
      );
    }
