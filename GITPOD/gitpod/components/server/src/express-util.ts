...
export const query = (...tuples: [string, string][]) => {
  ...
  return (
    "?" + tuples.map((t) => `${t[0]}=${encodeURIComponent(t[1])}`).join("&")
  );
};
...
export const isAllowedWebsocketDomain = (
  originHeader: any,
...
const looksLikeWorkspaceHostname = (
  originHostname: URL,
  gitpodHostName: string
): boolean => {
...
export function saveSession(
  reqOrSession: express.Request | Express.Session
): Promise<void> {
