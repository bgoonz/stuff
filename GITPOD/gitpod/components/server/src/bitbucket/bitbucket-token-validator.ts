...
import {
  CheckWriteAccessResult,
  IGitTokenValidator,
  IGitTokenValidatorParams,
} from "../workspace/git-token-validator";

@injectable()
export class BitbucketTokenValidator implements IGitTokenValidator {
  async checkWriteAccess(
    params: IGitTokenValidatorParams
  ): Promise<CheckWriteAccessResult> {
    ...
    const repos = (
      await api.user.listPermissionsForRepos({
        q: `repository.full_name="${repoFullName}"`,
      })
    ).data.values;
    ...
    if (repos && repos.length > 0) {
      if (repos.length > 1) {
        console.warn(
          `checkWriteAccessForBitbucketRepo: Found more than one repo for ${repoFullName}.`
        );
      }
      result.found = true;
      result.isPrivateRepo = repos[0].repository!.is_private;
      const matchingRepo = repos.find(
        (r) => r.permission == "write" || r.permission == "admin"
      );
      if (matchingRepo) {
