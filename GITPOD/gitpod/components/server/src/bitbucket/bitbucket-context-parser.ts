...
{
  @inject(BitbucketTokenHelper)
  protected readonly tokenHelper: BitbucketTokenHelper;
  @inject(BitbucketApiFactory)
  protected readonly apiFactory: BitbucketApiFactory;
  ...
  public async handle(
    ...
    try {
      const { host, owner, repoName, moreSegments, searchParams } =
        await this.parseURL(user, contextUrl);
      if (moreSegments.length > 0) {
        switch (moreSegments[0]) {
          case "src": {
            const more: Partial<NavigatorContext> = {};
            const branchTagOrHash =
              moreSegments.length > 1 ? moreSegments[1] : "";
            const isHash = await this.isValidCommitHash(
              user,
              owner,
              repoName,
              branchTagOrHash
            ...
            const pathSegments =
              moreSegments.length > 2 ? moreSegments.slice(2) : [];
            more.path = pathSegments.join("/");
            return this.handleNavigatorContext(
              ctx,
              user,
              host,
              owner,
      ...
      return await this.handleNavigatorContext(
        ctx,
        user,
        host,
        owner,
  ...
  protected async isValidCommitHash(
    user: User,
    owner: string,
    repoName: string,
    potentialCommitHash: string
  ) {
  ...
  protected async isTag(
    ...
    try {
      const api = await this.api(user);
      const result = await api.repositories.getTag({
        workspace: owner,
        repo_slug: repoName,
        name: potentialTag,
      });
  ...
  protected async handleNavigatorContext(
    ...
    const span = TraceContext.startSpan(
      "BitbucketContextParser.handleNavigatorContext",
      ctx
    );
    try {
      const api = await this.api(user);
      const repo =
        givenRepo ||
        (await api.repositories.get({ workspace: owner, repo_slug: repoName }))
          .data;
      ...
      if (!more.revision) {
        ...
        if (
          ...
          more.ref === repository.defaultBranch
        ) {
          // empty repo
          more.ref = undefined;
          more.revision = "";
      ...
      return {
        ...more,
        title: `${owner}/${repoName} - ${more.ref || more.revision}${
          more.path ? ":" + more.path : ""
        }`,
        repository,
      } as NavigatorContext;
      ...
      log.error(
        { userId: user.id },
        "Error parsing Bitbucket navigator request context",
        e
      );
      throw e;
    } finally {
  ...
  protected async handlePullRequestContext(
    ...
    const span = TraceContext.startSpan(
      "BitbucketContextParser.handleIssueContext",
      ctx
    );
    try {
      const api = await this.api(user);
      ...
      log.error(
        { userId: user.id },
        "Error parsing Bitbucket pull request context",
        e
      );
      throw e;
    } finally {
  ...
  protected async handleIssueContext(
    ...
    try {
      const api = await this.api(user);
      const issue = (
        await api.repositories.getIssue({
          workspace: owner,
          repo_slug: repoName,
          issue_id: `${more.nr}`,
        })
      ...
      const navigatorContext = await this.handleNavigatorContext(
        ctx,
        user,
        host,
        owner,
      ...
      log.error(
        { userId: user.id },
        "Error parsing Bitbucket issue context",
        e
      );
      throw e;
    } finally {
  ...
  public async fetchCommitHistory(
    ...
    const span = TraceContext.startSpan(
      "BitbucketContextParser.fetchCommitHistory",
      ctx
    );
    try {
      ...
      log.error(
        { userId: user.id },
        "Error fetching Bitbucket commit history",
        e
      );
      throw e;
    } finally {
