...
export class BitbucketAuthProvider extends GenericAuthProvider {
  ...
  protected get oauthConfig() {
    ...
    return <typeof oauth>{
      ...
      authorizationUrl:
        ...
        `https://${this.config.host}/site/oauth2/authorize`,
      tokenUrl:
        oauth.tokenUrl ||
        `https://${this.config.host}/site/oauth2/access_token`,
      settingsUrl:
        oauth.settingsUrl ||
        `https://${this.config.host}/account/settings/app-authorizations/`,
  ...
  authorize(
    ...
    super.authorize(
      ...
      res,
      next,
      scope ? scope : BitbucketOAuthScopes.Requirements.DEFAULT
    );
  }
  ...
  protected readAuthUserSetup = async (
    accessToken: string,
    ...
    try {
      ...
      const primaryEmail = emails.values.find(
        (x: { is_primary: boolean; email: string }) => x.is_primary
      ).email;
      ...
      log.error(
        `(${this.strategyName}) Reading current user info failed`,
        error,
        { accessToken, error }
      );
      throw error;
