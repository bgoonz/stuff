...
@suite(
  timeout(10000),
  retries(2),
  skipIfEnvVarNotSet("GITPOD_TEST_TOKEN_BITBUCKET")
)
class TestBitbucketFileProvider {
    ...
    container.load(
      new ContainerModule((bind, unbind, isBound, rebind) => {
        ...
        bind(AuthProviderParams).toConstantValue(
          TestBitbucketFileProvider.AUTH_HOST_CONFIG
        );
  ...
  @test public async testGetFileContents() {
    const result = await this.fileProvider.getFileContent(
      {
        repository: { owner: "gitpod", name: "sample-repository" },
        revision: "5a24a0c",
      } as Commit,
      this.user,
      "README.md"
...
This is the readme of the second branch.`);
  ...
  @test public async testGetLastChangeRevision() {
    const result = await this.fileProvider.getLastChangeRevision(
      { owner: "gitpod", name: "sample-repository" } as Repository,
      "second-branch",
      this.user,
      "README.md"
    );
