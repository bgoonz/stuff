...
export class ServerEE<
  C extends GitpodClient,
  S extends GitpodServer
> extends Server<C, S> {
  @inject(GraphQLController)
  protected readonly adminGraphQLController: GraphQLController;
  ...
  protected async registerRoutes(app: express.Application): Promise<void> {
    ...
    if (this.env.githubAppEnabled && this.githubApp.server) {
      ...
      log.debug(
        `GitHub app ready under ${this.githubApp.server.expressApp.path()}`
      );
    } else {
