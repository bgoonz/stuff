...
export class GraphQLResolvers {
  ...
  get(): Resolvers {
    return {
      Query: {
        me: this.authenticated((_, _args, ctx) => ctx.user!),
        user: this.admin((_, { userId }, _ctx) =>
        ...
        users: this.admin(
          async (
            ...
            const { total, rows } = await this.userDb.findAllUsers(
              offset,
              limit,
              orderBy,
              orderDir,
              searchTerm,
              parseMaybeDate(minCreationDate),
              parseMaybeDate(maxCreationDate),
              true
            );
            return { items: rows, total, hasMore: offset + limit < total };
          }
        ),
        userCount: this.admin((_, _args, _ctx) =>
          this.userDb.getUserCount(true)
        ),
        workspaces: this.admin(
          async (
            ...
            const { total, rows } = await this.workspaceDb.findAllWorkspaces(
              offset,
              limit,
              orderBy,
              orderDir,
              ownerId,
              searchTerm,
              parseMaybeDate(minCreationTime),
              parseMaybeDate(maxCreationTime),
              type
            );
            return { items: rows, total, hasMore: offset + limit < total };
          }
        ...
        workspaceInstances: this.admin(
          async (
            ...
            const { total, rows } =
              await this.workspaceDb.findAllWorkspaceInstances(
                offset,
                limit,
                orderBy,
                orderDir,
      ...
      User: {
        ...
        workspaces: (user, _args, _ctx) =>
          this.workspaceDb.findWorkspacesByUser(user.id),
        deleted: (user, _args, _ctx) => user.markedDeleted,
      },
      Workspace: {
        owner: (workspace, _args, _ctx) =>
          this.userDb.findUserById(workspace.ownerId).then((owner) => owner!),
        instances: (workspace, _args, _ctx) =>
          this.workspaceDb.findInstances(workspace.id),
      },
      WorkspaceInstance: {
  ...
  protected authenticated<T1, T2, T3 extends Context, T4, T5>(
    next: (root: T1, args: T2, ctx: T3, info: T4) => T5
  ): (root: T1, args: T2, ctx: T3, info: T4) => T5 {
  ...
  protected authorized<T1, T2, T3 extends Context, T4, T5>(
  ...
  protected admin<T1, T2, T3 extends Context, T4, T5>(
    next: (root: T1, args: T2, ctx: T3, info: T4) => T5
  ): (root: T1, args: T2, ctx: T3, info: T4) => T5 {
