...
import {
  ...
  HostContextProvider,
} from "../../../src/auth/host-context-provider";
import { HostContextProviderImpl } from "../../../src/auth/host-context-provider-impl";
const expect = chai.expect;

const userServiceTestContainerModule = new ContainerModule(
  (bind, unbind, isBound, rebind) => {
    ...
    bind(ConsensusLeaderMessenger)
      .to(InMemoryConsensusLeaderMessenger)
      .inSingletonScope();
...
export class EligibilityServiceSpec {
  ...
  get IDENTITY1() {
    return Object.assign({}, this._IDENTITY1);
  ...
  @test public async canOpenPrivateRepo_userCreationDate() {
    ...
    const user = await this.setupUser1({
      creationDate: userCreationDate.toISOString(),
    });
    const svc =
      this.userServiceTestContainer.get<EligibilityService>(EligibilityService);
    ...
    actual = await svc.mayOpenPrivateRepo(
      user,
      new Date(
        userCreationDate.getTime() +
