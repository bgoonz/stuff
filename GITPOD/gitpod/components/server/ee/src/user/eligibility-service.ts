...
import {
  ...
  CachedAccountStatement,
} from "./account-statement-provider";
...
export class EligibilityService {
  ...
  @inject(EMailDomainService)
  protected readonly domainService: EMailDomainService;
  ...
  async isStudent(user: User | string): Promise<boolean> {
    ...
    for (const identity of user.identities) {
      ...
      const emailSuffixMatches =
        await this.domainService.hasEducationalInstitutionSuffix(
  ...
  async getGitHubEducationPack(user: User): Promise<GitHubEducationPack> {
    let token: string;
    try {
      token = (await this.tokenProvider.getTokenForHost(user, "github.com"))
        .value;
    } catch (err) {
    ...
    try {
      ...
      log.warn(
        { userId: user.id },
        "error while checking student pack status",
        err
      );
    }
    return { student: false, faculty: false };
  ...
  async mayStartWorkspace(
    ...
    const hasHitParallelWorkspaceLimit = async (): Promise<
      HitParallelWorkspaceLimit | undefined
    > => {
      const max = await this.getMaxParallelWorkspaces(user);
      const instances = (await runningInstances).filter(
        (i) => i.status.phase !== "unknown"
      );
   ...
   */
  protected async getMaxParallelWorkspaces(
    user: User,
    date: Date = new Date()
  ): Promise<number> {
    ...
    return subscriptions
      .map((s) => Plans.getParallelWorkspacesById(s.planId))
  ...
  protected async checkEnoughCreditForWorkspaceStart(
    ...
    const cachedAccountStatement =
      this.accountStatementProvider.getCachedStatement();
    const lowerBound = this.getRemainingUsageHoursLowerBound(
      cachedAccountStatement,
      date.toISOString()
    );
    if (
      lowerBound &&
      (lowerBound === "unlimited" ||
        lowerBound > Accounting.MINIMUM_CREDIT_FOR_OPEN_IN_HOURS)
    ) {
      return true;
    }

    const remainingUsageHours =
      await this.accountStatementProvider.getRemainingUsageHours(
        userId,
        date.toISOString(),
        runningInstances
      );
  ...
  protected getRemainingUsageHoursLowerBound(
    ...
    const diffInMillis =
      new Date(cachedStatement.endDate).getTime() - new Date(date).getTime();
    const maxPossibleUsage =
      millisecondsToHours(diffInMillis) * MAX_PARALLEL_WORKSPACES;
  ...
  async mayOpenPrivateRepo(
    ...
    if (
      EligibilityService.DURATION_30_DAYS_MILLIS +
        freeTrialTimeStart.getTime() -
        date.getTime() >
      0
    ) {
   ...
   * @param now
   */
  protected async ensureFreePrivateRepoTrialStarted(
    user: User,
    now: string
  ): Promise<void> {
  ...
  protected getPrivateRepoTrialStart(user: User): Date | undefined {
    const freeTrialStartDate =
      user.featureFlags && user.featureFlags.privateRepoTrialStartDate;
   ...
   */
  async getPrivateRepoTrialEndDate(
    user: User,
    date: Date = new Date()
    ...
    if (
      await this.subscriptionService.hasActivePaidSubscription(user.id, date)
    ) {
      return undefined;
    }
    return new Date(
      EligibilityService.DURATION_30_DAYS_MILLIS + start.getTime()
    );
  }
  ...
  async maySetTimeout(user: User, date: Date = new Date()): Promise<boolean> {
    ...
    return (
      subscriptions.filter((s) => eligblePlans.includes(s.planId!)).length > 0
    );
  }
   ...
   */
  async hasFixedWorkspaceResources(
    user: User,
    date: Date = new Date()
  ): Promise<boolean> {
    const subscriptions =
