...
export class ChargebeeCouponComputer {
  ...
  public async getAvailableCouponIds(user: User): Promise<string[]> {
    ...
    const { student, faculty } =
      await this.eligibilityService.getGitHubEducationPack(user);
    if (student || faculty) {
      // `student` and `faculty` are mutually exclusive
      const coupon = student
        ? COUPON_GITHUB_STUDENT_PACK
        : COUPON_GITHUB_TEACHER_PACK;
  ...
  protected async hasUsedCouponBefore(
    user: User,
    couponId: string
  ): Promise<boolean> {
    /*
     ...
     * is eligible.
     */
    return this.accountingDB.hadSubscriptionCreatedWithCoupon(
      user.id,
      couponId
    );
  }
  ...
  public async getAppliedCouponIds(user: User, date: Date): Promise<string[]> {
    ...
    const subscriptions =
      await this.accountingDB.findActiveSubscriptionsForUser(
        user.id,
        date.toISOString()
      );
    for (const subscription of subscriptions) {
      ...
      const additionalData =
        await this.accountingDB.findSubscriptionAdditionalData(
