...
> {
  ...
  protected readonly licenseKeySource: LicenseKeySource;

  @inject(MessageBusIntegration)
  protected readonly messageBusIntegration: MessageBusIntegrationEE;

  // per-user state
  @inject(EligibilityService)
  protected readonly eligibilityService: EligibilityService;
  @inject(AccountStatementProvider)
  protected readonly accountStatementProvider: AccountStatementProvider;
  ...
  @inject(TeamSubscriptionDB)
  protected readonly teamSubscriptionDB: TeamSubscriptionDB;
  @inject(TeamSubscriptionService)
  protected readonly teamSubscriptionService: TeamSubscriptionService;

  @inject(ChargebeeProvider)
  protected readonly chargebeeProvider: ChargebeeProvider;
  @inject(UpgradeHelper) protected readonly upgradeHelper: UpgradeHelper;
  @inject(ChargebeeCouponComputer)
  protected readonly couponComputer: ChargebeeCouponComputer;
  @inject(ChargebeeService)
  protected readonly chargebeeService: ChargebeeService;

  @inject(GitHubAppSupport)
  protected readonly githubAppSupport: GitHubAppSupport;
  ...
  protected listenToCreditAlerts(): void {
    ...
    this.disposables.push(
      this.messageBusIntegration.listenToCreditAlerts(
        this.user.id,
        async (ctx: TraceContext, creditAlert: CreditAlert) => {
          ...
          if (creditAlert.remainingUsageHours < 1e-6) {
            ...
            runningInstances.forEach(
              async (instance) => await this.stopWorkspace(instance.workspaceId)
            );
          }
        }
      )
  ...
  protected async mayStartWorkspace(
    ...
    try {
      const result = await this.eligibilityService.mayStartWorkspace(
        user,
        new Date(),
        runningInstances
      );
  ...
  protected async mayOpenContext(
    ...
    const mayOpenContext = await this.eligibilityService.mayOpenContext(
      user,
      context,
      new Date()
    );
    if (!mayOpenContext) {
      throw new ResponseError(
        ErrorCodes.PLAN_DOES_NOT_ALLOW_PRIVATE_REPOS,
        `You do not have a plan that allows for opening private repositories.`
      );
    }
  }
  ...
  protected requireEELicense(feature: Feature) {
    if (!this.licenseEvaluator.isEnabled(feature)) {
      throw new ResponseError(
        ErrorCodes.EE_LICENSE_REQUIRED,
  ...
  async validateLicense(): Promise<LicenseValidationResult> {
    ...
    const canAnotherUserSignUp = this.licenseEvaluator.hasEnoughSeats(
      userCount + 1
    );
  ...
  public async setWorkspaceTimeout(
    ...
    try {
      ...
      if (!(await this.maySetTimeout(user))) {
        throw new ResponseError(
          ErrorCodes.PLAN_PROFESSIONAL_REQUIRED,
      ...
      const runningInstance = runningInstances.find(
        (i) => i.workspaceId === workspaceId
      );
      if (!runningInstance) {
        throw new ResponseError(
          ErrorCodes.NOT_FOUND,
          "Can only set keep-alive for running workspaces"
        );
      }
      ...
      const defaultTimeout = await this.userService.getDefaultWorkspaceTimeout(
        user
      );
    ...
    try {
      ...
      if (!runningInstance) {
        log.warn(
          ...
          "Can only get keep-alive for running workspaces"
        );
      ...
      const duration = desc
        .getStatus()!
        .getSpec()!
        .getTimeout() as WorkspaceTimeoutDuration;
  ...
  async getSnapshots(workspaceId: string): Promise<string[]> {
    ...
    try {
      ...
      const snapshots = await this.workspaceDb
        .trace({ span })
        .findSnapshotsByWorkspaceId(workspaceId);
      await Promise.all(
        snapshots.map((s) =>
          this.guardAccess(
            {
      ...
      (
        await Promise.all(
          workspaces.map((workspace) =>
            workspaceDb.findRunningInstance(workspace.id)
          )
        )
      )
        .filter(isDefined)
        .forEach((instance) =>
          this.internalStopWorkspaceInstance(
            { span },
            instance.id,
            instance.region,
  ...
  async adminModifyRoleOrPermission(
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("adminModifyRoleOrPermission");
    span.log(req);
    try {
      const target = await this.userDB.findUserById(req.id);
      if (!target) {
      ...
      const rolesOrPermissions = new Set(
        (target.rolesOrPermissions || []) as string[]
      );
      req.rpp.forEach((e) => {
        if (e.add) {
      ...
      target.rolesOrPermissions = Array.from(
        rolesOrPermissions.values()
      ) as RoleOrPermission[];
  ...
  async adminModifyPermanentWorkspaceFeatureFlag(
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("adminModifyPermanentWorkspaceFeatureFlag");
    span.log(req);
    try {
      const target = await this.userDB.findUserById(req.id);
      if (!target) {
      ...
      const featureFlags = new Set(
        featureSettings.permanentWSFeatureFlags || []
      );
  ...
  async adminGetWorkspace(id: string): Promise<WorkspaceAndInstance> {
    ...
    try {
      result = await this.workspaceDb
        .trace({ span })
        .findWorkspaceAndInstance(id);
    } catch (e) {
      TraceContext.logError({ span }, e);
      throw new ResponseError(500, e.toString());
    } finally {
  ...
  async adminForceStopWorkspace(id: string): Promise<void> {
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("adminForceStopWorkspace");
  ...
  async adminRestoreSoftDeletedWorkspace(id: string): Promise<void> {
    ...
    const span = opentracing
      .globalTracer()
      .startSpan("adminRestoreSoftDeletedWorkspace");
    await this.workspaceDb.trace({ span }).transaction(async (db) => {
      const ws = await db.findById(id);
      if (!ws) {
        throw new ResponseError(
          ErrorCodes.NOT_FOUND,
          `No workspace with id '${id}' found.`
        );
      }
      if (!ws.softDeleted) {
        return;
  ...
  protected async guardAdminAccess(
    method: string,
    params: any,
    ...
    if (!this.authorizationService.hasPermission(user, requiredPermission)) {
      log.warn({ userId: this.user?.id }, "unauthorised admin access", {
        authorised: false,
        method,
        params,
      });
    ...
    log.info({ userId: this.user?.id }, "admin access", {
      authorised: true,
      method,
      params,
    });
  ...
  protected async findPrebuiltWorkspace(
    ...
    try {
      ...
      if (prebuiltWorkspace.state === "available") {
        log.info(
          logCtx,
          `Found prebuilt workspace for ${cloneUrl}:${context.revision}`,
          logPayload
        );
        ...
        prebuiltWorkspace.state === "queued" ||
        prebuiltWorkspace.state === "building"
      ) {
        ...
        if (!inSameCluster) {
          if (mode === CreateWorkspaceMode.UsePrebuild) {
            ...
            const finishedPrebuiltWorkspace =
              await this.pollDatabaseUntilPrebuildIsAvailable(
        ...
        log.info(
          ...
          }) workspace for ${cloneUrl}:${context.revision}`,
          logPayload
        );
        return result;
      }
  ...
  async getLicenseInfo(): Promise<GetLicenseInfoResult> {
    ...
    const isAdmin = this.authorizationService.hasPermission(
      user,
      Permission.ADMIN_API
    );
  ...
  async getAppliedCoupons(): Promise<PlanCoupon[]> {
    ...
    const couponIds = await this.couponComputer.getAppliedCouponIds(
      user,
      new Date()
    );
    return this.getChargebeePlanCoupons(couponIds);
  ...
  public async getPrivateRepoTrialEndDate(): Promise<string | undefined> {
    ...
    const endDate = await this.eligibilityService.getPrivateRepoTrialEndDate(
      user
    );
    if (!endDate) {
      return undefined;
  ...
  protected async getChargebeePlanCoupons(couponIds: string[]) {
        ...
        (c) =>
          new Promise<chargebee.Coupon | undefined>((resolve, reject) =>
            this.chargebeeProvider.coupon.retrieve(c).request((err, res) => {
              if (!!err) {
                log.error({}, "could not retrieve coupon: " + err.message, {
                  coupon: c,
                });
                resolve(undefined);
              } else if (!!res) {
    ...
    for (const coupon of chargebeeCoupons) {
      ...
      for (const planID of coupon.plan_ids) {
        ...
        let newPrice =
          (plan.pricePerMonth * 100) / (100 - coupon.discount_percentage);
        result.push({
  ...
  protected async findAvailableCouponForPlan(
    user: User,
    planId: string
    ...
    const applicableCoupon = chargbeeCoupons
      .filter(
        (c) =>
          ...
          c.plan_ids &&
          c.plan_ids.indexOf(planId) != -1
      )
      .sort((a, b) =>
        a!.discount_percentage! < b!.discount_percentage! ? -1 : 1
      )[0];
  ...
  async subscriptionUpgradeTo(
    ...
    await this.doUpdateUserPaidSubscription(
      user.id,
      subscriptionId,
      chargebeePlanId,
      false
  ...
  protected async ensureIsEligibleForPlan(
    user: User,
    chargebeePlanId: string
  ): Promise<void> {
    const p = Plans.getById(chargebeePlanId);
    if (!p) {
      log.error({ userId: user.id }, "Invalid plan", {
  ...
  async subscriptionCancel(subscriptionId: string): Promise<void> {
    ...
    const chargebeeSubscriptionId = await this.doGetUserPaidSubscription(
      user.id,
      subscriptionId
    );
      ...
      this.chargebeeProvider.subscription
        ...
        .request((error: any, result: any) => {
          if (error) {
            log.error(
              logContext,
              "Chargebee Subscription update error",
              error,
              logPayload
            );
            reject(error);
  ...
  protected async doCancelDowngradeUserPaidSubscription(
    userId: string,
    subscriptionId: string
  ) {
    const chargebeeSubscriptionId = await this.doGetUserPaidSubscription(
      ...
      this.chargebeeProvider.subscription
        ...
        .request((error: any, result: any) => {
          if (error) {
            ...
            log.debug(
              logContext,
              "Chargebee scheduled change to Subscription removed",
              logPayload
            );
            resolve();
          }
  ...
  protected async doGetUserPaidSubscription(
    userId: string,
    subscriptionId: string
  ) {
    ...
    if (!chargebeeSubscriptionId) {
      const err = new Error(
        `SubscriptionId ${subscriptionId} has no paymentReference!`
      );
  ...
  async tsGet(): Promise<TeamSubscription[]> {
    ...
    return this.teamSubscriptionDB.findTeamSubscriptionsForUser(
      user.id,
      new Date().toISOString()
    );
  }
  ...
  async tsGetSlots(): Promise<TeamSubscriptionSlotResolved[]> {
    ...
    return this.teamSubscriptionService.findTeamSubscriptionSlotsBy(
      user.id,
      new Date()
    );
  }
  ...
  async tsGetUnassignedSlot(
    ...
    const slots = await this.teamSubscriptionService.findUnassignedSlots(
      teamSubscriptionId
    );
    return slots[0];
  }
  ...
  protected async tsGetActiveSlotQuantity(
    teamSubscriptionId: string
  ): Promise<number> {
    const slots = await this.teamSubscriptionDB.findSlotsByTeamSubscriptionId(
      teamSubscriptionId
    );
  ...
  async tsAddSlots(
    ...
    const ts = await this.internalGetTeamSubscription(
      teamSubscriptionId,
      user.id
    );
    ...
    try {
      ...
      await this.doChargeForTeamSubscriptionUpgrade(
        ts,
        oldQuantity,
        newQuantity,
        now.toISOString()
      ...
      if (chargebee.ApiError.is(err) && err.type === "payment") {
        throw new ResponseError(
          ...
          `${err.api_error_code}: ${err.message}`
        );
      }
  ...
  async tsAssignSlot(
    ...
    try {
      ...
      const assigneeInfo: FindUserByIdentityStrResult = identityStr
        ...
        : await this.getAssigneeInfo(user);
      const {
        user: assignee,
  ...
  protected async getAssigneeInfo(user: User) {
    ...
    for (const identity of user.identities) {
      const provider = authProviders.find(
        (p) => p.authProviderId === identity.authProviderId
      );
  ...
  protected async ensureMayGetAssignedToTS(
    ts: TeamSubscription,
    user: User,
    assignee: User
  ) {
    ...
    const slots = await this.teamSubscriptionDB.findSlotsByAssignee(
      assignee.id
    );
    const now = new Date().toISOString();
    const assignedSlots = slots.filter(
      (slot) => TeamSubscriptionSlot.status(slot, now) === "assigned"
    );
  ...
  protected readonly findAssigneeThrottled: ThrottledFunction<
    any[],
    FindUserByIdentityStrResult
  > = pThrottle(
    async (
      logCtx: LogContext,
      identityStr: string
    ): Promise<FindUserByIdentityStrResult> => {
      let assigneeInfo = undefined;
      try {
        assigneeInfo = await this.userService.findUserByIdentityStr(
          identityStr
        );
      } catch (err) {
  ...
  async tsDeactivateSlot(
    ...
    this.updateTeamSubscriptionQueue.enqueue(async () => {
      ...
      const newQuantity =
        (await this.tsGetActiveSlotQuantity(teamSubscriptionId)) - 1;
      try {
        const now = new Date();
        // Downgrade by 1 unit
        await this.doUpdateTeamSubscription(user.id, ts.id, newQuantity, false);
  ...
  async tsReactivateSlot(
    ...
    this.updateTeamSubscriptionQueue.enqueue(async () => {
      ...
      try {
        ...
        await this.doUpdateTeamSubscription(user.id, ts.id, newQuantity, false);
        await this.teamSubscriptionService.reactivateSlot(
          ts,
          teamSubscriptionSlotId,
          now
        );
  ...
  async getGithubUpgradeUrls(): Promise<GithubUpgradeURL[]> {
    ...
    const ghidentity = user.identities.find(
      (i) => i.authProviderId == "Public-GitHub"
    );
    if (!ghidentity) {
      log.debug(
        { userId: user.id },
        "user has no GitHub identity - cannot provide plan upgrade URLs"
      );
      return [];
    }
    const produceUpgradeURL = (planID: number) =>
      `https://www.github.com/marketplace/${this.env.githubAppMarketplaceName}/upgrade/${planID}/${ghidentity.authId}`;
    ...
    return Plans.getAvailablePlans("USD")
      ...
      .map(
        (p) =>
          <GithubUpgradeURL>{
            url: produceUpgradeURL(p.githubPlanNumber!),
            planID: p.githubId!,
          }
      );
  }
  ...
  protected async doChargeForTeamSubscriptionUpgrade(
    ...
    if (oldQuantity < newQuantity) {
      ...
      const pricePerUnitInCents = await this.getPricePerUnitInCents(
        ts,
        chargebeeSubscriptionId
      );
  ...
  protected async getPricePerUnitInCents(
    ts: TeamSubscription,
    chargebeeSubscriptionId: string
  ): Promise<number> {
    ...
    const subscriptionPlanUnitPriceInCents =
      chargebeeSubscription.plan_unit_price;
  ...
  protected async getChargebeeSubscription(
    ...
    const retrieveResult =
      await new Promise<chargebee.SubscriptionRetrieveResult>(
        (resolve, reject) => {
          this.chargebeeProvider.subscription
            ...
            .request(function (error: any, result: any) {
              if (error) {
                ...
                log.debug(
                  logCtx,
                  "Retrieve subscription: successful",
                  logPayload
                );
                resolve(result);
              }
  ...
  protected async doUpdateTeamSubscription(
    ...
    const logContext = { userId };
    const teamSubscription = await this.internalGetTeamSubscription(
      teamSubscriptionId,
      userId
    );
    ...
    if (!chargebeeSubscriptionId) {
      const err = new Error(
        `TeamSubscriptionId ${teamSubscriptionId} has no paymentReference!`
      );
   ...
   * @param userId
   */
  protected async internalGetTeamSubscription(
    id: string,
    userId?: string
  ): Promise<TeamSubscription> {
    const teamSubscription =
      await this.teamSubscriptionDB.findTeamSubscriptionById(id);
    if (!teamSubscription) {
    ...
    if (userId && userId !== teamSubscription.userId) {
      throw new ResponseError(
        ErrorCodes.PERMISSION_DENIED,
        "Only the owner may access a team subscription!"
      );
    }
    return teamSubscription;
  }

  protected async internalGetSubscription(
    id: string,
    userId: string
    ...
    if (userId !== subscription.userId) {
      const err = new ResponseError(
        ErrorCodes.PERMISSION_DENIED,
        "Only the owner may access a subscription!"
      );
  ...
  async adminGetAccountStatement(userId: string): Promise<AccountStatement> {
    ...
    if (
      !this.authorizationService.hasPermission(user, Permission.ADMIN_USERS)
    ) {
      throw new ResponseError(ErrorCodes.PERMISSION_DENIED, "not allowed");
    }

    return await this.accountService.getAccountStatement(
      userId,
      new Date().toISOString()
    );
  }
  ...
  async adminGrantExtraHours(
    ...
    await this.subscriptionService.addCredit(
      userId,
      extraHours,
      new Date().toISOString()
    );
  ...
  async sendFeedback(feedback: string): Promise<string | undefined> {
    ...
    const stillEnoughCredits =
      remainingUsageHours >
      Math.max(...Accounting.LOW_CREDIT_WARNINGS_IN_HOURS);
    log.info({ userId: user.id }, `Feedback: "${feedback}"`, {
      feedback,
      stillEnoughCredits,
    });
  ...
  async getProviderRepositoriesForUser(params: {
    ...
    const repositories =
      await this.githubAppSupport.getProviderRepositoriesForUser({
        user,
        ...params,
      });
    ...
    repositories.forEach((r) => {
      r.inUse = cloneUrlsInUse.has(r.cloneUrl);
    });
  ...
  async triggerPrebuild(projectId: string, branchName: string): Promise<void> {
    ...
    const branchDetails = await this.projectsService.getBranchDetails(
      user,
      project,
      branchName
    );
    if (branchDetails.length !== 1) {
      log.debug({ userId: user.id }, "Cannot find branch details.", {
        project,
        branchName,
      });
    }
    ...
    await this.prebuildManager.startPrebuild(
      { span },
      {
        contextURL,
