...
export class WorkspaceHealthMonitoring {
  ...
  async startWorkspaceProbe(
    ...
    try {
      const user = await this.userDB
        .trace({ span })
        .findUserByName(BUILTIN_WORKSPACE_PROBE_USER_NAME);
      if (!user) {
        throw new Error(
          "cannot find workspace probe user. DB not set up properly?"
        );
      }
  ...
  async probeAllRunningWorkspaces(ctx: TraceContext): Promise<ProbeResult[]> {
    ...
    try {
      const workspaces = await this.workspaceDb
        .trace({ span })
        .findRunningInstancesWithWorkspaces(this.env.installationShortname);
      const workspacesFilter = (ws: RunningWorkspaceInfo) =>
        !!ws.latestInstance.ideUrl &&
        ws.latestInstance.status.phase === "running";
      const resultMapper = async (ws: RunningWorkspaceInfo) => {
        const result = await this.probeWorkspaceOnDifferentProbeURLs(
          { span },
          ws.latestInstance
        );
        if (!result.ok) {
          ...
          const wsi = await this.workspaceDb
            .trace({ span })
            .findInstanceById(result.instanceID);
          if (wsi && wsi.status.phase === "running") {
            return result;
          } else {
            log.info(
              {
                instanceId: ws.latestInstance.id,
  ...
  protected async probeWorkspaceOnDifferentProbeURLs(
    ...
    for (const probeURL of probeURLs) {
      result = {
        ...(await this.probeWorkspace(ctx, wsi, probeURL)),
  ...
  protected async probeWorkspace(
    ctx: TraceContext,
    wsi: WorkspaceInstance,
    probeURL: URL
  ): Promise<ProbeResult> {
    ...
    try {
      ...
      log.error(
        { instanceId: wsi.id, workspaceId: wsi.workspaceId },
