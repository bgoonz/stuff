...
export class WorkspaceFactoryEE extends WorkspaceFactory {
  ...
  public async createForContext(
    ...
    if (StartPrebuildContext.is(context)) {
      return this.createForStartPrebuild(
        ctx,
        user,
        context,
      ...
      return this.createForPrebuiltWorkspace(
        ctx,
        user,
        context,
        normalizedContextURL
    ...
    try {
      ...
      const existingPWS = await this.db
        ...
        .findPrebuiltWorkspaceByCommit(
          ...
          commitContext.revision
        );
      if (existingPWS) {
        const wsInstance = await this.db
          .trace({ span })
          .findRunningInstance(existingPWS.buildWorkspaceId);
        if (wsInstance) {
      ...
      for (const parent of context.commitHistory || []) {
        ...
        log.debug(
          `Considering parent prebuild for ${commitContext.revision}`,
          parentPrebuild
        );
        const buildWorkspace = await this.db
          .trace({ span })
          .findById(parentPrebuild.buildWorkspaceId);
        if (!buildWorkspace) {
          continue;
        }
        ...
        const parentPrebuildTasks = filterPrebuildTasks(
          buildWorkspace.config.tasks
        );
        if (
          JSON.stringify(prebuildTasks) !== JSON.stringify(parentPrebuildTasks)
        ) {
  ...
  protected async createForPrebuiltWorkspace(
    ...
    const fallback = await this.fallbackIfOutPrebuildTime(
      ctx,
      user,
      context,
      normalizedContextURL
  ...
  protected async fallbackIfOutPrebuildTime(
    ctx: TraceContext,
    user: User,
    context: PrebuiltWorkspaceContext,
    normalizedContextURL: string
    ...
    if (!this.licenseEvaluator.canUsePrebuild(prebuildTime || 0)) {
      ...
      log.warn(
        {},
        "cannot use prebuild because enterprise license prevents it",
        { prebuildTime }
      );
