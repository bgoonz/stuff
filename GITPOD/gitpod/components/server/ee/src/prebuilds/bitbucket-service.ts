...
export class BitbucketService extends RepositoryService {
  ...
  @inject(BitbucketContextParser)
  protected bitbucketContextParser: BitbucketContextParser;
  ...
  async installAutomatedPrebuilds(user: User, cloneUrl: string): Promise<void> {
    try {
      ...
      const { owner, repoName } = await this.bitbucketContextParser.parseURL(
        user,
        cloneUrl
      );
      ...
      if (
        ...
        existing.data.values.some(
          (hook) => hook.url && hook.url.indexOf(hookUrl) !== -1
        )
      ) {
        console.log(
          `bitbucket webhook already installed on ${owner}/${repoName}`
        );
        return;
      }
