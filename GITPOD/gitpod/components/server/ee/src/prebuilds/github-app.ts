...
export class GithubApp {
  ...
  constructor(
    ...
    @inject(PrebuildStatusMaintainer)
    protected readonly statusMaintainer: PrebuildStatusMaintainer
  ) {
  ...
  protected async buildApp(app: Probot, options: ApplicationFunctionOptions) {
    ...
    app.on("installation.created", async (ctx) => {
      ...
      const user = await this.userDB.findUserByIdentity({
        authProviderId: this.env.githubAppAuthProviderId,
        authId,
      });
      ...
      log.debug({ userId }, "New installation recorded", {
        userId,
        authId,
        targetAccountName,
      });
    ...
    app.on("installation.deleted", async (ctx) => {
      const installationId = `${ctx.payload.installation.id}`;
      await this.appInstallationDB.recordUninstallation(
        "github",
        "platform",
        installationId
      );
            ...
            if (state) {
              const url = this.env.hostUrl
                .with({
                  ...
                  search:
                    "message=payload:" +
                    Buffer.from(JSON.stringify(payload), "utf-8").toString(
                      "base64"
                    ),
                })
                .toString();
              ...
              const url = this.env.hostUrl
                .with({
                  pathname: "install-github-app",
                  search: `installation_id=${installationId}`,
                })
                .toString();
              res.redirect(url);
            }
  ...
  protected async handlePushEvent(
    ctx: WebhookEvent<EventPayloads.WebhookPayloadPush> &
    ...
    try {
      ...
      const owner =
        installationId &&
        ((await this.findProjectOwner(cloneURL)) ||
          (await this.findInstallationOwner(installationId)));
      if (!owner) {
        log.info(`No installation or associated user found.`, {
      ...
      if (!!owner.user.blocked) {
        log.info(logCtx, `Blocked user tried to start prebuild`, {
          repo: ctx.payload.repository,
        });
        return;
      }
      ...
      let config = await this.prebuildManager.fetchConfig(
        { span },
        owner.user,
        contextURL
      );
      const runPrebuild = this.appRules.shouldRunPrebuild(
        config,
      ...
      this.prebuildManager
        ...
        .catch((err) =>
          log.error(logCtx, "Error while starting prebuild", err, {
            contextURL,
          })
        );
    } catch (e) {
  ...
  protected async handlePullRequest(
    ctx: WebhookEvent<EventPayloads.WebhookPayloadPullRequest> &
    ...
    try {
      ...
      if (!owner) {
        log.warn(
          "Did not find user for installation. Someone's Gitpod experience may be broken.",
          { repo: ctx.payload.repository, installationId }
        );
        return;
      }
      ...
      const config = await this.prebuildManager.fetchConfig(
        { span },
        owner.user,
        contextURL
      );

      const prebuildStartPromise = this.onPrStartPrebuild(
        { span },
        config,
        owner,
        ctx
  ...
  protected async onPrAddCheck(
    ...
    try {
      ...
      const pws = await this.workspaceDB
        .trace({ span })
        .findPrebuildByWorkspaceID(spr.wsid);
      if (!pws) {
        return;
      }
      ...
      await this.statusMaintainer.registerCheckRun(
        ...
        pws,
        {
          ...
          details_url: this.env.hostUrl
            .withContext(ctx.payload.pull_request.html_url)
            .toString(),
        }
      );
    } catch (err) {
  ...
  protected onPrStartPrebuild(
    ...
    if (runPrebuild) {
      ...
      prebuildStartPromise.catch((err) =>
        log.error(err, "Error while starting prebuild", { contextURL })
      );
      return prebuildStartPromise;
    } else {
      log.debug(
        { userId: owner.user.id },
        `Not running prebuild, the user did not enable it for this context`,
        { contextURL, owner }
      );
      return;
    }
  ...
  protected onPrAddBadge(config: WorkspaceConfig | undefined, ctx: Context) {
    ...
    const button = `<a href="${this.env.hostUrl.withContext(
      contextURL
    ...
    const newBody = body + `\n\n${button}\n\n`;
    const updatePrPromise = ctx.octokit.pulls.update({
      ...ctx.repo(),
      pull_number: pr.number,
      body: newBody,
    });
    updatePrPromise.catch((err) =>
      log.error(err, "Error while updating PR body", { contextURL })
    );
  }
  ...
  protected async onPrAddComment(
    ...
    newCommentPromise.catch((err) =>
      log.error(err, "Error while adding new PR comment", { contextURL })
    );
  }

  protected getBadgeImageURL(): string {
    return this.env.hostUrl
      .with({ pathname: "/button/open-in-gitpod.svg" })
      .toString();
  }

  protected async findProjectOwner(
    cloneURL: string
    ...
    if (project) {
      const owner = !!project.userId
        ...
        : (await this.teamDB.findMembersByTeam(project.teamId || "")).filter(
            (m) => m.role === "owner"
          )[0];
      if (owner) {
  ...
  protected async findInstallationOwner(
    ...
    // Legacy mode
    //
    const installation = await this.appInstallationDB.findInstallation(
      "github",
