...
export class PrebuildManager {
  ...
  async hasAutomatedPrebuilds(
    ...
    try {
      const existingPBs = await this.workspaceDB
        .trace({ span })
        .findPrebuildsWithWorkpace(cloneURL);
    ...
    try {
      ...
      if (!!existingPB) {
        return {
          ...
          didFinish: existingPB.state === "available",
        };
      }
      ...
      const actual = (await contextParser.handle(
        { span },
        user,
        contextURL
      )) as CommitContext;
  ...
  async retriggerPrebuild(
    ...
    try {
      ...
      if (runningInstance !== undefined) {
        throw new WorkspaceRunningError(
          "Workspace is still runnning",
          runningInstance
        );
      }
      span.setTag("starting", true);
  ...
  shouldPrebuild(config: WorkspaceConfig | undefined): boolean {
    ...
    const hasPrebuildTask =
      !!config.tasks && config.tasks.find((t) => !!t.init || !!t.prebuild);
    if (!hasPrebuildTask) {
      return false;
    }
  ...
  protected shouldPrebuildIncrementally(cloneUrl: string): boolean {
    const trimRepoUrl = (url: string) =>
      url.replace(/\/$/, "").replace(/\.git$/, "");
    const repoUrl = trimRepoUrl(cloneUrl);
    return this.env.incrementalPrebuildsRepositoryPassList.some(
      (url) => trimRepoUrl(url) === repoUrl
    );
  }
  ...
  async fetchConfig(
    ...
    try {
      ...
      return await this.configProvider.fetchConfig(
        { span },
        user,
