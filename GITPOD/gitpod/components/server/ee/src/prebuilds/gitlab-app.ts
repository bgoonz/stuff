...
export class GitLabApp {
  ...
  protected async findUser(
    ...
    try {
      ...
      if (!identity) {
        throw new Error(
          `User ${user.id} has no identity for '${TokenService.GITPOD_AUTH_PROVIDER_ID}'.`
        );
      }
  ...
  protected async handlePushHook(
    ...
    try {
      ...
      log.debug({ userId: user.id }, "GitLab push hook: Context URL", {
        context: body,
        contextURL,
      });
      ...
      if (!this.prebuildManager.shouldPrebuild(config)) {
        log.debug(
          { userId: user.id },
          "GitLab push hook: There is no prebuild config.",
          { context: body, contextURL }
        );
        return undefined;
      }

      log.debug({ userId: user.id }, "GitLab push hook: Starting prebuild", {
        body,
        contextURL,
      });
  ...
  protected createContextUrl(body: GitLabPushHook) {
    ...
    const contextURL = `${repoUrl.substr(
      0,
      repoUrl.length - 4
    )}/-/tree${body.ref.substr("refs/head/".length)}`;
    return contextURL;
  }
