...
export class GithubAppRules {
  protected mergeWithDefaultConfig(
  ...
  public shouldRunPrebuild(
    ...
    const hasPrebuildTask =
      !!config.tasks &&
      config.tasks.find((t) => !!t.before || !!t.init || !!t.prebuild);
    if (!hasPrebuildTask) {
      return false;
    }
  ...
  public shouldDo(
    ...
    if (action === "addCheck") {
      ...
      log.warn(
        "In GitHub app we asked whether we should do an unknown action. This is a bug!",
        { action }
      );
      return false;
    }
