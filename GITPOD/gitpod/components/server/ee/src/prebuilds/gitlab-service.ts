...
export class GitlabService extends RepositoryService {
  ...
  @inject(GitlabContextParser)
  protected gitlabContextParser: GitlabContextParser;
  ...
  async installAutomatedPrebuilds(user: User, cloneUrl: string): Promise<void> {
    ...
    const { owner, repoName } = await this.gitlabContextParser.parseURL(
      user,
      cloneUrl
    );
    const response = (await api.Projects.show(
      `${owner}/${repoName}`
    )) as unknown as GitLab.Project;
    if (GitLab.ApiError.is(response)) {
      throw response;
    }
    const hooks = (await api.ProjectHooks.all(
      response.id
    ...
    await api.ProjectHooks.add(response.id, this.getHookUrl(), <
      Partial<GitLab.ProjectHook>
    >{
      ...existingProps,
      push_events: true,
