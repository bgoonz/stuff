...
export class PrebuildStatusMaintainer implements Disposable {
  ...
  start(githubApiProvider: AuthenticatedGithubProvider): void {
    ...
    this.periodicChecker = setInterval(
      this.periodicUpdatableCheck.bind(this),
      60 * 1000
    ...
    try {
      ...
      if (pws.state == "queued" || pws.state == "building") {
        await this.workspaceDB
          .trace({ span })
          .attachUpdatableToPrebuild(pws.id, {
            id: uuidv4(),
            owner: cri.owner,
            repo: cri.repo,
            isResolved: false,
        ...
        await githubApi.repos.createCommitStatus({
          ...
          description:
            conclusion == "success"
              ...
              : NON_PREBUILT_STATUS_DESCRIPTION,
  ...
  protected getConclusionFromPrebuildState(
    pws: PrebuiltWorkspace
    ...
    if (pws.state === "aborted") {
      ...
      log.warn(
        "Should have updated prebuilt workspace updatable but prebuild still seems to be running. Resorting to error conclusion.",
        { pws }
      );
      return "error";
    } else {
  ...
  protected async handlePrebuildFinished(
    ...
    const span = TraceContext.startSpan(
      "PrebuildStatusMaintainer.handlePrebuildFinished",
      ctx
    );
    ...
    try {
      ...
      if (!prebuild) {
        log.warn(
          "received headless log message without associated prebuild",
          msg
        );
        return;
      }

      const updatatables = await this.workspaceDB
        .trace({ span })
        .findUpdatablesForPrebuild(prebuild.id);
      await Promise.all(
        updatatables
          .filter((u) => !u.isResolved)
          .map((u) => this.doUpdate({ span }, u, prebuild))
      );
    } catch (err) {
      TraceContext.logError({ span }, err);
      throw err;
  ...
  protected async doUpdate(
    ...
    try {
      ...
      if (!github) {
        log.error(
          "unable to authenticate GitHub app - this leaves user-facing checks dangling."
        );
        return;
      }
      ...
      if (!!updatatable.contextUrl) {
        ...
        await this.workspaceDB
          .trace({ span })
          .markUpdatableResolved(updatatable.id);
        log.info(
          `Resolved updatable. Marked check on ${updatatable.contextUrl} as ${conclusion}`
        );
  ...
  protected async periodicUpdatableCheck() {
    const unresolvedUpdatables = await this.workspaceDB
      .trace({})
      .getUnresolvedUpdatables();

    for (const updatable of unresolvedUpdatables) {
      if (
        Date.now() - Date.parse(updatable.workspace.creationTime) >
        MAX_UPDATABLE_AGE
      ) {
        log.info(
