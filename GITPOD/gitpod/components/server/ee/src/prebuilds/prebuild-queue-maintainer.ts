...
} from "@gitpod/gitpod-protocol/lib/headless-workspace-log";
...
export class PrebuildQueueMaintainer implements Disposable {
  ...
  @inject(PrebuildRateLimiter)
  protected readonly prebuildRateLimiter: PrebuildRateLimiter;

  @inject(ConsensusLeaderQorum) protected readonly qorum: ConsensusLeaderQorum;

  @inject(IClientDataPrometheusAdapter)
  protected readonly prometheusAdapter: IClientDataPrometheusAdapter;

  protected messagebusListener?: Disposable;
  protected periodicMaintainer?: NodeJS.Timer;

  start(): void {
    this.messagebusListener = this.messagebus.listenForPrebuildUpdatableQueue(
      (ctx, msg) => this.handlePrebuildFinished(ctx, msg)
    );
    this.periodicMaintainer = setInterval(
      this.periodicQueueMaintainance.bind(this),
      60 * 1000
    ) as any as NodeJS.Timer;
    log.debug("prebuild queue maintainer started");
  }

  protected async handlePrebuildFinished(
    ctx: TraceContext,
    msg: HeadlessWorkspaceEvent
  ) {
    const span = TraceContext.startSpan(
      "PrebuildQueueMaintainer.handlePrebuildFinished",
      ctx
    );
    ...
    try {
      const prebuild = await this.workspaceDB
        .trace({ span })
        .findPrebuildByWorkspaceID(msg.workspaceID);
      if (!prebuild) {
        return;
      }
      const workspace = await this.workspaceDB
        .trace({ span })
        .findById(prebuild.buildWorkspaceId);
      if (!workspace) {
        return;
      }
      const user = await this.userDB
        .trace({ span })
        .findUserById(workspace.ownerId);
      if (!user) {
        return;
      }
      const queuedPrebuilds = await this.workspaceDB
        .trace({ span })
        .findQueuedPrebuilds(prebuild.cloneURL);
      if (queuedPrebuilds.length === 0) {
        return;
      }
      const canBuildNow = await this.prebuildRateLimiter.canBuildNow(
        { span },
        user,
        prebuild.cloneURL
      );
      ...
      for (let i = 0; i < queuedPrebuilds.length; i++) {
        const { prebuild: nextPrebuild, workspace: nextWorkspace } =
          queuedPrebuilds[i];
        if (
          !(await this.prebuildRateLimiter.shouldStillBuild(
            { span },
            nextPrebuild
          ))
        ) {
  ...
  protected async periodicQueueMaintainance() {
    ...
    const span = TraceContext.startSpan(
      "PrebuildQueueMaintainer.periodicQueueMaintainance",
      {}
    );
    ...
    try {
      ...
      for (const { prebuild, workspace } of queuedPrebuilds) {
        if (
          !(await this.prebuildRateLimiter.shouldStillBuild({ span }, prebuild))
        ) {
      ...
      const t = new Date();
      Array.from(queueLengths.keys()).forEach((q) =>
        this.prometheusAdapter.storePrebuildQueueLength(
          t,
          q,
          queueLengths.get(q) || 0
        )
      );
    } catch (err) {
      TraceContext.logError({ span }, err);
  ...
  protected async discardPrebuild(
    ctx: TraceContext,
    prebuild: PrebuiltWorkspace
  ) {
    const span = TraceContext.startSpan(
      "PrebuildQueueMaintainer.discardPrebuild",
      {}
    );
