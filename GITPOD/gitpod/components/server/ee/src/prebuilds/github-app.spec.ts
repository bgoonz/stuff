...
describe("GitHub app", () => {
  it("should not run prebuilds without explicit configuration", async () => {
    ...
    chai.assert.isFalse(
      rules.shouldRunPrebuild(undefined, false, false, false)
    );
  ...
  it("should not run prebuilds without tasks to execute", async () => {
    ...
    const rules = container.get(GithubAppRules) as GithubAppRules;
    chai.assert.isFalse(
      rules.shouldRunPrebuild(noTaskConfig, true, false, false)
    );
    chai.assert.isFalse(
      rules.shouldRunPrebuild(noTaskConfig, false, true, false)
    );
    chai.assert.isFalse(
      rules.shouldRunPrebuild(noTaskConfig, false, false, false)
    );
    chai.assert.isFalse(
      rules.shouldRunPrebuild(noTaskConfig, false, true, true)
    );
  });
  ...
  it("should behave well with individual configuration", async () => {
    ...
    checkConfig({ pullRequests: false, master: false }, [
      false,
      false,
      false,
      false,
    ]);
    checkConfig({ pullRequests: true, pullRequestsFromForks: true }, [
      true,
      true,
      false,
      true,
