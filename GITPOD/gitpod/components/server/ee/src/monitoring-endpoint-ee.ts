...
export class MonitoringEndpointsAppEE extends WorkspaceHealthMonitoring {
  @inject(WorkspaceHealthMonitoring)
  protected readonly workspaceHealthMonitoring: WorkspaceHealthMonitoring;
    ...
    monApp.get("/workspace-health", async (req, res) => {
      try {
        const result = await checkWorkspaceHealth(
          {},
          this.workspaceHealthMonitoring,
          !!req.query.extra
        );
...
async function checkWorkspaceHealth(
  ...
  const result = await workspaceHealthMonitoring.probeAllRunningWorkspaces({
    span,
  });
