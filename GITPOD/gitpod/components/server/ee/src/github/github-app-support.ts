...
export class GitHubAppSupport {
  ...
  async getProviderRepositoriesForUser(params: {
    ...
    const listReposForInstallation = async (
      ...
      try {
        const accessibleRepos =
          await sub.apps.listReposAccessibleToInstallation({ per_page: 100 });
    ...
    const ownRepos = await listReposAccessibleToInstallation(
      usersGitHubAccount
    );
    if (ownRepos) {
    ...
    if (!isNaN(installationId)) {
      if (!result.some((r) => r.installationId === installationId)) {
        const installation = await appApi.apps.getInstallation({
        ...
        if (installation) {
          ...
          if (additional) {
            for (const repo of additional) {
              if (
                result.some(
                  (r) => r.account === repo.account && r.name === repo.name
                )
              ) {
              ...
              user.additionalData.knownGitHubOrgs =
                user.additionalData.knownGitHubOrgs || [];
          ...
          log.debug(`Provided installationId appears to be invalid.`, {
            installationId,
          });
        }
      }
