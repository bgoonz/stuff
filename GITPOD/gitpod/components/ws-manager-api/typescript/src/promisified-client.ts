...
export function withTracing(ctx: TraceContext) {
  ...
  if (ctx.span) {
    ...
    opentracing
      .globalTracer()
      .inject(ctx.span, opentracing.FORMAT_HTTP_HEADERS, carrier);
    Object.keys(carrier)
...
type RetryStrategy = <Res>(
  run: (attempt: number) => Promise<Res>
) => Promise<Res>;
export const noRetry: RetryStrategy = <Res>(
  run: (attempt: number) => Promise<Res>
) => run(0);
...
async function linearBackoffRetry<Res>(
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      const isUnavailableErr =
        "code" in err && err.code === grpc.status.UNAVAILABLE;
      const isDeadlineExceeded =
        "code" in err && err.code === grpc.status.DEADLINE_EXCEEDED;
...
export class PromisifiedWorkspaceManagerClient implements Disposable {
  ...
  public markActive(
    ctx: TraceContext,
    request: MarkActiveRequest
  ): Promise<MarkActiveResponse> {
    return this.retryIfUnavailable(
      (attempt: number) =>
  ...
  public controlPort(
    ctx: TraceContext,
    request: ControlPortRequest
  ): Promise<ControlPortResponse> {
    return this.retryIfUnavailable(
      (attempt: number) =>
  ...
  public controlAdmission(
    ctx: TraceContext,
    request: ControlAdmissionRequest
  ): Promise<ControlAdmissionResponse> {
    // we do not use the default options here as takeSnapshot can take a very long time - much longer than the default deadline allows
    return this.retryIfUnavailable(
      (attempt: number) =>
        new Promise<ControlAdmissionResponse>((resolve, reject) => {
          const span = TraceContext.startSpan(
            `/ws-manager/controlAdmission`,
            ctx
          );
          span.log({ attempt });
  ...
  public subscribe(
    ...
    return new Promise<grpc.ClientReadableStream<SubscribeResponse>>(
      (resolve, reject) => {
