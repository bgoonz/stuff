...
export const WorkspaceManagerClientProviderSource = Symbol(
...
@injectable()
export class WorkspaceManagerClientProviderEnvSource
...
@injectable()
export class WorkspaceManagerClientProviderDBSource
  implements WorkspaceManagerClientProviderSource
{
      ...
      for (const cluster of clusters) {
        if (allClusters.has(cluster.name)) {
          log.warn(
            `${
              cluster.name
            } is specified multiple times, overriding with: \n${JSON.stringify(
              cluster
            )}`
          );
        }
