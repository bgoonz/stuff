...
export class WorkspaceManagerClientProvider implements Disposable {
  ...
  protected readonly connectionCache = new Map<
    string,
  ...
  public async getStartManager(
    ...
    const availableCluster = await this.getAvailableStartCluster(
      user,
      workspace,
      instance
    );
  ...
  public async get(
    ...
    if (!client) {
      ...
      console.warn(
        `Lost connection to workspace manager \"${name}\" - attempting to reestablish`
      );
      const info = await getConnectionInfo();
      client = this.createClient(info, grpcOptions);
      this.connectionCache.set(name, client);
...
function chooseCluster(
  availableCluster: WorkspaceClusterWoTLS[]
): WorkspaceClusterWoTLS {
