...
import {
  ...
  getCancelledAt,
  getUpdatedAt,
} from "./chargebee-subscription-helper";
...
export class SubscriptionMapper {
   ...
   */
  protected handleSubscriptionCreated(
   ...
   * @param chargebeeSubscription
   */
  protected handleSubscriptionChanged(
    chargebeeSubscription: chargebee.Subscription
  ) {
    // Find old Gitpod subscription for the incoming, updated Chargebee Subscription
    const oldSubscription = this.model.findSubscriptionByPaymentReference(
    ...
    if (!oldPlan) {
      throw new Error(
        `Unknown plan id (data inconsistency): '${oldSubscription.planId}'`
      );
    }

    const subscriptionChange = Plans.subscriptionChange(
      oldPlan.type,
      chargebeePlan.type
    );
    if (subscriptionChange === "downgrade") {
      ...
      if (
        ...
        oldSubscription.paymentData.downgradeDate
      ) {
        ...
        dateInLastPeriod = new Date(
          secondsBefore(oldSubscription.paymentData.downgradeDate, 1)
        );
      } else {
   ...
   * @param chargebeeSubscription
   */
  protected handleSubscriptionCancelled(
    chargebeeSubscription: chargebee.Subscription
  ) {
    const gitpodSubscription = this.model.findSubscriptionByPaymentReference(
      chargebeeSubscription.id
    );
    const cancellationDate = getCancelledAt(chargebeeSubscription);

    const endDate = calculateCurrentTermEnd(
   ...
   * @param chargebeeSubscription
   */
  protected handleSubscriptionReactivated(
    chargebeeSubscription: chargebee.Subscription
  ) {
    this.model.add(
      Subscription.create({
  ...
  protected handleSubscriptionChangesScheduled(
    ...
    const { endDate: downgradeEffectiveDate } =
      Subscription.calculateCurrentPeriod(
        gitpodSubscription.startDate,
    ...
    if (gitpodSubscription.paymentData) {
      ...
      gitpodSubscription.paymentData = {
        downgradeDate: downgradeEffectiveDate,
      };
    }
...
const calculateCurrentTermEnd = (startDate: string, upTo: string): string => {
  ...
  while (potentialCurrentTermEnd.getTime() < upToDate.getTime()) {
    potentialCurrentTermEnd = new Date(
      oneMonthLater(potentialCurrentTermEnd.toISOString(), dayOfMonth)
    );
  }
...
const getPlan = (chargebeeSubscription: chargebee.Subscription) => {
  ...
  if (!plan) {
    throw new Error(
      `Cannot find Gitpod plan for id: ${chargebeeSubscription.plan_id}`
    );
  }
  return plan;
};
