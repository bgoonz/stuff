...
export function getStartDate(
  ...
  const chargebeeStart =
    chargebeeSubscription.started_at || chargebeeSubscription.start_date;
  if (!chargebeeStart) {
    throw new Error(
...
export function getCancelledAt(
  chargebeeSubscription: chargebee.Subscription
): string {
  if (!chargebeeSubscription.cancelled_at) {
    throw new Error(
      "subscription.cancelled_at must be set on subscription cancellation changed event."
    );
  } else {
...
export function getCurrentTermEnd(
  chargebeeSubscription: chargebee.Subscription
): string {
