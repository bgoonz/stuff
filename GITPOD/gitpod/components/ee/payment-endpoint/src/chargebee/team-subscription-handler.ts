...
@injectable()
export class TeamSubscriptionHandler
  implements EventHandler<chargebee.SubscriptionEventV2>
{
  ...
  protected readonly service: TeamSubscriptionService;
  ...
  async handleSingleEvent(
    ...
    log.info(logContext, `Start TeamSubscriptionHandler.handleSingleEvent`, {
      eventType,
    });
    try {
      await this.mapToTeamSubscription(
        userId,
        eventType,
      ...
      log.error(
        logContext,
        "Error in TeamSubscriptionHandler.handleSingleEvent",
        error
      );
      throw error;
    }
    log.info(logContext, "Finished TeamSubscriptionHandler.handleSingleEvent", {
      eventType,
    });
    return true;
  }
    ...
    await this.db.transaction(async (db) => {
      ...
      if (subs.length === 0) {
        const ts = TeamSubscription.create({
          ...
          endDate: chargebeeSubscription.cancelled_at
            ? getCancelledAt(chargebeeSubscription)
            : undefined,
        ...
        const oldSubscription = subs.find(
          (s) => s.paymentReference === chargebeeSubscription.id
        );
        if (!oldSubscription) {
          throw new Error(
            `Cannot find TeamSubscription for paymentReference ${chargebeeSubscription.id}!`
          );
        }
        ...
        if (eventType === "subscription_cancelled") {
          ...
          await this.service.deactivateAllSlots(
            oldSubscription,
