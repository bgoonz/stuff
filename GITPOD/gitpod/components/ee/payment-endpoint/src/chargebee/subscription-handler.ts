...
export class SubscriptionHandler
  implements EventHandler<chargebee.SubscriptionEventV2>
{
  @inject(SubscriptionService)
  protected readonly subscriptionService: SubscriptionService;
  @inject(AccountingDB) protected readonly db: AccountingDB;
  @inject(SubscriptionMapperFactory)
  protected readonly mapperFactory: SubscriptionMapperFactory;
  ...
  async handleSingleEvent(
    event: chargebee.Event<chargebee.SubscriptionEventV2>
  ): Promise<boolean> {
    ...
    log.info(logContext, `Start SubscriptionHandler.handleSingleEvent`, {
      eventType,
    });
    try {
      if (!event.content.subscription) {
        log.error(
          logContext,
          "Ignoring event, because it does not contain a subscription",
          event
        );
      } else {
        try {
          await this.storeAdditionalData(
          ...
          log.error(
            logContext,
            "Failed to store additional subscription data",
            event
          );
        }
      }
      ...
      log.error(
        logContext,
        "Error in SubscriptionHandler.handleSingleEvent",
        error
      );
      throw error;
    }
    log.info(logContext, "Finished SubscriptionHandler.handleSingleEvent", {
      eventType,
    });
    return true;
  }

  async storeAdditionalData(
    subscription: chargebee.Subscription,
    invoice?: chargebee.Invoice
  ): Promise<void> {
    ...
    if (invoice) {
      lastInvoice =
        (invoice.date && new Date(invoice.date * 1000).toISOString()) || "";
   ...
   */
  protected async checkAndChargeForUpgrade(
    userId: string,
    chargebeeSubscription: chargebee.Subscription
  ) {
    const gitpodSubscriptions =
      await this.db.findSubscriptionForUserByPaymentRef(
        userId,
    ...
    if (newPlan.pricePerMonth > oldPlan.pricePerMonth) {
      ...
      const diffInCents =
        newPlan.pricePerMonth * 100 - oldPlan.pricePerMonth * 100;
      const upgradeTimestamp = getUpdatedAt(chargebeeSubscription);
      const description = `Difference on Upgrade from '${oldPlan.name}' to '${
        newPlan.name
      }' (${formatDate(upgradeTimestamp)})`;
   ...
   * @param event
   */
  protected async mapToGitpodSubscription(
    userId: string,
    event: chargebee.Event<chargebee.SubscriptionEventV2>
  ) {
    await this.db.transaction(async (db) => {
      const subscriptions = await db.findAllSubscriptionsForUser(userId);
      const userPaidSubscriptions = subscriptions.filter((s) =>
  ...
  // TODO
  handleEvents(
    events: chargebee.Event<chargebee.SubscriptionEventV2>[],
    startDate: Date
  ) {
