...
export class EndpointController {
  ...
  @inject(CompositeEventHandler)
  protected readonly eventHandler: CompositeEventHandler;
      ...
      (
        ...
        passport.authenticate(
          ...
          (err: any, user: any, info: any) => {
            if (err) {
              log.error(
                `Login error on chargebee payment update route!`,
                err,
                req
              );
              return; // Drop request
            ...
            if (!user) {
              log.error(
                `Unauthorized user on chargebee payment update route!`,
                req
              );
              return; // Drop request
            }
   ...
   * @param res
   */
  private async handleUpdateGitpodSubscription(
    req: express.Request,
    res: express.Response
  ) {
    ...
    try {
      ...
      if (!handled) {
        const payload = {
          chargebeeEventType: req.body.event_type,
          action: "ignored",
        };
        log.debug(
          `Faithfully ignoring chargebee event of type: ${req.body.event_type}`,
          payload
        );
      }
      res.status(200).send();
