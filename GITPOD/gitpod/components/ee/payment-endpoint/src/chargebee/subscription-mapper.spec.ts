...
class SubscriptionMapperSpec {
  mapperFactory = testContainer.get<SubscriptionMapperFactory>(
    SubscriptionMapperFactory
  );
  ...
  @test async full_cycle() {
    ...
    expect(
      filterIrrelevantFields(subscriptions),
      "basic-created"
    ).to.deep.equal([
      {
  ...
  @test async downgrade_unlimited_to_professional_new() {
    const userCreatedDate = new Date("2018-11-20T15:25:48.000Z").toISOString();
    const events = loadFromFolder<chargebee.SubscriptionEventV2>(
      "downgrade_unlimited_to_professional_new"
    );
 ...
 */
const loadFromFolder = <T>(
  testname: string
): Map<string, chargebee.Event<T>> => {
