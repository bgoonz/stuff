...
@injectable()
export class PaymentSourceHandler
  implements EventHandler<chargebee.PaymentSourceEventV2>
{
  ...
  async handleSingleEvent(
    event: chargebee.Event<chargebee.PaymentSourceEventV2>
  ): Promise<boolean> {
    const userId = event.content.payment_source.customer_id;
    const eventType = event.event_type;

    const logContext: LogContext = { userId };
    log.info(logContext, `Start PaymentSourceHandler.handleSingleEvent`, {
      eventType,
    });
    try {
      ...
      log.error(
        logContext,
        "Error in PaymentSourceHandler.handleSingleEvent",
        error
      );
      throw error;
    }
    log.info(logContext, "Finished PaymentSourceHandler.handleSingleEvent", {
      eventType,
    });
    return true;
  }
  ...
  async storePaymentSourceInfo(
    userId: string,
    paymentSource: chargebee.PaymentSource
  ): Promise<void> {
    ...
    if (!resourceVersion) {
      log.error({ userId }, "PaymentSourceHandler: no resource_version set!", {
        id: paymentSource.id,
      });
      return;
    }
