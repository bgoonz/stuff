...
export class SubscriptionService {
  ...
  async getSubscriptionHistoryForUserInPeriod(
    ...
    const subscriptions =
      await this.accountingDB.findSubscriptionsForUserInPeriod(
        user.id,
        startDate,
        endDate
      );
   ...
   */
  async getNotYetCancelledSubscriptions(
    user: UserCreated,
    date: string
  ): Promise<Subscription[]> {
    const subscriptions =
      await this.accountingDB.findNotYetCancelledSubscriptions(user.id, date);
    const model = new SubscriptionModel(user.id, subscriptions);
    return model
      .mergedWithFreeSubscriptions(user.creationDate)
      .sort(orderByStartDateAscEndDateAsc);
  ...
  async subscribe(
    ...
    return this.accountingDB.transaction(async (db) => {
      ...
      log.info({ userId }, "Creating subscription", {
   ...
   */
  async hasActivePaidSubscription(
    userId: string,
    date: Date
  ): Promise<boolean> {
    const subscriptions =
    ...
    return (
      subscriptions.filter((s) => Subscription.isActive(s, date.toISOString()))
        .length > 0
    );
  }
