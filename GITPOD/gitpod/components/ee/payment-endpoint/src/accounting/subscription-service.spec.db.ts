...
class SubscriptionServiceSpec {
  ...
  subscriptionService =
    localTestContainer.get<SubscriptionService>(SubscriptionService);
  ...
  @test.skip async subscriptions() {
    ...
    expectExactlyOne(
      await this.acocuntingDB.findActiveSubscriptionsForUser("Jan", start),
      {
        amount: 20,
        planId: "plan20",
        paymentReference: "02",
    ...
    expectExactlyOne(
      await this.acocuntingDB.findActiveSubscriptionsForUser(
        "Jan",
        secondMonth
      ),
      {
    ...
    const subscriptionsThirdMonth =
      await this.acocuntingDB.findActiveSubscriptionsForUser("Jan", thirdMonth);
    ...
    const allSubscriptions =
      await this.acocuntingDB.findAllSubscriptionsForUser("Jan");
...
const expectExactlyOne = <T>(
  actualSubscriptions: T[],
  expected: Partial<T>
) => {
