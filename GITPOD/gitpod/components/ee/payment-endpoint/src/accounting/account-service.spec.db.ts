...
class AccountServiceSpec {
  ...
  async testRemainingHoursUnlimited() {
    ...
    const remainingUsageHours = this.accountService.getRemainingUsageHours(
      statement,
      1
    );
  ...
  @test async testRemainingHours() {
    ...
    const statement = await this.accountService.getAccountStatement(
      "Sven",
      hoursLater(subscriptionSwitchDate, 5)
    );
  ...
  @test async noSessions() {
    ...
    expect(await this.invoice(rightAfter(start))).to.be.equal(
      "2000-01-01T00:00:00.000Z 100 credit 100"
    );
...
2000-02-11T00:00:00.000Z 200 credit 200`
  ...
  @test async multiSubscriptionOverlappingSession_2() {
    ...
    Subscription.cancelSubscription(
      subscription2,
      subscriptionSwitchDate2,
      oneMonthLater(subscription2.startDate)
    );
...
2000-02-11T05:00:00.000Z 300 credit 300`
  ...
  @test async remainingHoursNoSession() {
    ...
    expect(
      await this.remainingHours(hoursLater(secondMonth, -10), 1, true)
    ).to.be.equal(110);
    expect(
      await this.remainingHours(hoursLater(secondMonth, -10), 1, false)
    ).to.be.equal(100);
    expect(
      await this.remainingHours(hoursLater(secondMonth, -100), 1, true)
    ).to.be.equal(200);
    expect(
      await this.remainingHours(hoursLater(secondMonth, -100), 1, false)
    ).to.be.equal(100);
    expect(
      await this.remainingHours(hoursLater(secondMonth, -101), 1, true)
    ).to.be.equal(100);

    expect(await this.remainingHours(rightAfter(start), 2)).to.be.equal(50);
    expect(
      await this.remainingHours(hoursLater(secondMonth, -51), 2)
    ).to.be.equal(50);
  }

  @test async remainingHoursOneSession() {
    await this.createSession(start, 100);
    expect(await this.remainingHours(rightAfter(start))).to.be.equal(
      99.99999972222221
    );
    expect(await this.remainingHours(rightAfter(secondMonth))).to.be.equal(100);
  ...
  @test async creditsAreBookedAgainstSessionsByAge() {
    ...
    await this.accountingDb.storeSubscription(
      await this.accountingDb.newSubscription(subscriptionWith40Hours)
    );
...
2000-02-29T23:59:59.999Z -40 expiry`
  ...
  @test async testPaidPlanWhileProOpenSource() {
    ...
    let statement = await this.accountService.getAccountStatement(
      "Sven",
      hoursLater(start, 5)
    );
  ...
  @test async testInvalidSessionDates() {
    ...
    statement = await this.accountService.getAccountStatement(
      "Sven",
      hoursLater(start, 12)
    );
    expect(statement!.remainingHours).to.be.equal(100);
  ...
  private stringifyStatement(statement: AccountStatement): string {
    const result = [...statement.credits, ...statement.debits]
      ...
      .map(
        (e) =>
          `${e.date} ${e.amount} ${e.kind}${
            e.remainingAmount !== undefined ? " " + e.remainingAmount : ""
          }`
      )
      .join("\n");
    return result;
  ...
  private async remainingHours(
    ...
    const result = this.accountService.getRemainingUsageHours(
      statement,
      numInstances,
      includeNext
    );
