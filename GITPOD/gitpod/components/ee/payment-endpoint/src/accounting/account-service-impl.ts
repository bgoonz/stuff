...
import {
  orderByExpiryDateDesc,
  SortedArray,
  orderCreditFirst,
  within,
} from "./accounting-util";
...
export class AccountServiceImpl implements AccountService {
  ...
  public getRemainingUsageHours(
    ...
    const projectedCreditChanges = this.projectCreditChanges(
      statement,
      considerNextPeriod
    );
    const creditChangesSorted = new SortedArray(
      projectedCreditChanges,
      (c1, c2) => orderByDateAscPosFirst(c1, c2)
    );
    ...
    const creditsNow = statement.credits
      ...
      .reduce(
        (v, c) =>
          v + (c.remainingAmount !== undefined ? c.remainingAmount : c.amount),
        0
      );
    ...
    let hitZeroDate: string | undefined =
      credits === 0 ? lastCreditChange.date : undefined;
    while (creditChange) {
      const duration = durationInMillis(
        creditChange.date,
        lastCreditChange.date
      );
      ...
      if (credits - debits <= 0) {
        ...
        if (
          hitZeroDate &&
          durationInHours(creditChange.date, hitZeroDate) >
            Accounting.GOODWILL_IN_HOURS
        ) {
          // No cure in sight.
          break;
        }
   ...
   * @param considerNextPeriod
   */
  protected projectCreditChanges(
    statement: AccountStatement,
    ...
    if (considerNextPeriod) {
      ...
      return creditChanges.filter(
        (c) => c.date > statement.endDate && c.date < lastDateOfInterest
      );
    }
  }
  ...
  public async getAccountStatement(
    ...
    const unorderedOpenCredits = await this.projectCreditSources(
      userId,
      userCreationDate,
      startDate,
      endDate
    );
    const openCredits = new SortedArray<OpenCredit>(
      unorderedOpenCredits,
      orderByExpiryDateDesc
    );
    ...
    const { credits, debits } = this.enterDebits(
      userId,
      openDebits,
      openCredits,
      endDate
    ...
    return {
      userId,
      subscriptions: await this.getNotYetCancelledSubscriptions(
        userId,
        userCreationDate,
        endDate
      ),
  ...
  protected async projectCreditSources(
    ...
    // Subscriptions
    const subscriptions =
      await this.subscriptionService.getSubscriptionHistoryForUserInPeriod(
  ...
  protected projectSubscriptionsCredits(
    ...
    for (const subscription of subscriptions) {
      ...
      while (
        ...
        let billingPeriodEnd = oneMonthLater(
          billingPeriodStart,
        ...
        billingPeriodEnd = earliest(
          billingPeriodEnd,
          subscription.endDate || billingPeriodEnd
        );
        const amount =
   ...
   * @param endDate
   */
  protected async projectDebits(
    userId: string,
    startDate: string,
    endDate: string
  ): Promise<OpenDebit[]> {
    const sessions = await this.workspaceDB.findSessionsInPeriod(
      userId,
      startDate,
      endDate
    );
    return sessions
      ...
      .map<OpenDebit>((s) => {
        ...
        const sessionEndDate = earliest(
          wsi.stoppingTime || wsi.stoppedTime || endDate,
          endDate
        );
        return {
          userId,
  ...
  protected shouldGetBilled(s: WorkspaceInstanceSessionWithWorkspace) {
    ...
    log.warn(
      "unknown workspace type - cannot decide if this workspace ought to be billed or not",
      s
    );
    return false;
  }
  ...
  protected isPrivateRepoContext(ctx: WorkspaceContext): boolean {
    return (
      (CommitContext.is(ctx) && ctx.repository.private === true) ||
      (PrebuiltWorkspaceContext.is(ctx) &&
        this.isPrivateRepoContext(ctx.originalContext))
    );
  }
  ...
  protected enterDebits(
    ...
    while (openDebit) {
      ...
      if (openDebit.date < openCredit.date) {
        const { before, after } = this.truncateDebitLeft(
          openDebit,
          openCredit.date
        );
        if (before) {
      ...
      const { before, after } = this.truncateDebitRight(
        openDebit,
        openCredit.expiryDate
      );
      if (after) {
        openDebits.push(after);
      }
  ...
  protected enterDebit(
    ...
    const debitEntry = this.toDebitEntry(
      openDebit,
      openCredit.uid,
      -accountableAmount,
      endDate
    ...
    const remainingDebitAmount = millisecondsToHours(
      debitAmountPos - accountableAmount
    );
   ...
   * @param date
   */
  protected truncateDebitLeft(
    debit: OpenDebit,
    date: string
  ): { before?: OpenDebit; after: OpenDebit } {
    if (debit.date >= date) {
      return { after: { ...debit } };
    } else {
      return {
  ...
  protected toDebitEntry(
    ...
    const debitEntry = AccountEntry.create<Debit>({
      ...
      date:
        debit.endDate < endDate ? debit.endDate : rightBefore(debit.endDate),
      creditId,
    });
    return debitEntry;
  }
  ...
  protected getRemainingHours(
    credits: AccountEntryFixedPeriod[],
    date: string
  ): number | "unlimited" {
  ...
  protected async getNotYetCancelledSubscriptions(
    ...
    const subscriptions =
      await this.subscriptionService.getNotYetCancelledSubscriptions(
        { id: userId, creationDate: userCreationDate },
        date
      );
