...
const toInt = (dateStr?: string) =>
  dateStr ? new Date(dateStr).getTime() : Number.MAX_SAFE_INTEGER;
export const orderByEndDateDescThenStartDateDesc = (
  s1: Subscription,
  s2: Subscription
) => {
  return (
    toInt(s2.endDate) - toInt(s1.endDate) ||
    toInt(s2.startDate) - toInt(s1.startDate)
  );
};
...
export const orderByStartDateAscEndDateAsc = (
  s1: Subscription,
  s2: Subscription
) => {
  return (
    toInt(s1.startDate) - toInt(s2.startDate) ||
    toInt(s1.endDate) - toInt(s2.endDate)
  );
};

export const within = (date: string, period: AccountEntryFixedPeriod) => {
  return (
    date >= period.date && (!period.expiryDate || date < period.expiryDate)
  );
};
