...
export class SubscriptionModel {
  protected readonly result: SubscriptionModel.Result =
    SubscriptionModel.Result.create();
  ...
  findOpenSubscriptions(planId?: string): Subscription[] {
    let subscriptionsForPaymentRef = this.subscriptions.filter(
      (s) => !s.endDate
    );
    if (planId) {
      subscriptionsForPaymentRef = subscriptionsForPaymentRef.filter(
        (s) => s.planId == planId
      );
    }
  ...
  findSubscriptionByPaymentReference(paymentReference: string): Subscription {
    const subscriptionsForPaymentRef = this.subscriptions.filter(
      (s) => s.paymentReference === paymentReference
    );
    ...
    return subscriptionsForPaymentRef.sort(
      orderByEndDateDescThenStartDateDesc
    )[0];
  }

  findSubscriptionByTeamSubscriptionSlotId(
    slotId: string
  ): Subscription | undefined {
    const subscriptionsForSlot = this.subscriptions.filter(
      (s) => s.teamSubscriptionSlotId === slotId
    );
  ...
  mergedWithFreeSubscriptions(userCreationDate: string): Subscription[] {
    ...
    return this.insertFreeSubscriptions(subscriptions, userCreationDate).sort(
      orderByEndDateDescThenStartDateDesc
    );
  }
   ...
   */
  protected insertFreeSubscriptions(
...
namespace Period {
  ...
  export const from = (p: Period): Period => ({
