...
export class TeamSubscriptionService {
  ...
  async addSlots(teamSubscription: TS, addQuantity: number) {
    ...
    for (let i = 0; i < addQuantity; i++) {
      promises.push(
        this.db.storeSlot(
          TeamSubscriptionSlot.create({ teamSubscriptionId: ts.id })
        )
      );
    }
    await Promise.all(promises);
    log.info({ userId: ts.userId }, "Added slots to team subscription", {
      teamSubscription: ts,
      addQuantity,
    });
  }
  ...
  async deactivateAllSlots(teamSubscription: TS, date: Date) {
    ...
    log.info({ userId: ts.userId }, "Deactivated all team subscription slots", {
      teamSubscription: ts,
      date,
    });
  }
  ...
  async assignSlot(
    ...
    return this.accountingDb.transaction(async (db) => {
      ...
      const ts = await this.getTeamSubscription(teamSubscription);
      const { startDate } = Subscription.calculateCurrentPeriod(
        ts.startDate,
        date
      );
      const { slot } = await this.loadSlot(
        slotId,
        ts.id,
        now,
        "unassigned",
      ...
      switch (state) {
        ...
        case "deactivated":
          ...
          const remainingHoursThisPeriod =
            await this.calculateRemainingHoursThisPeriod(
              db,
              slot,
              startDate,
              now,
  ...
  async deactivateSlot(teamSubscription: TS, slotId: string, date: Date) {
    return this.accountingDb.transaction(async (db) => {
      const now = date.toISOString();
      const ts = await this.getTeamSubscription(teamSubscription);
      const { endDate } = Subscription.calculateCurrentPeriod(
        ts.startDate,
        date
      );
      const { slot, state } = await this.loadSlot(
        slotId,
        ts.id,
        now,
        "assigned",
  ...
  async reassignSlot(
    ...
    return this.accountingDb.transaction(async (db) => {
      ...
      const slot = (await this.loadSlot(slotId, ts.id, now, "assigned"))
        .slot as TeamSubscriptionSlotAssigned;
  ...
  protected async calculateRemainingHoursThisPeriod(
        ...
        statement.credits
          ...
          .filter(
            (c) =>
              ...
              subscriptions.some(
                (s) =>
                  s.uid === (c.description as CreditDescription).subscriptionId
              )
          )
          // Cumulate usage (total - remaining)
          .reduce(
            (cum, e) =>
              !e.remainingAmount ? cum : cum + (e.amount - e.remainingAmount),
            0
          )
      );
    };
    ...
    const totalUsage = (await Promise.all(totalUsagesPerUser)).reduce(
      (prev, cur) => prev + cur,
      0
    );
   ...
   * @param date
   */
  async findTeamSubscriptionSlotsBy(
    assignerUserId: string,
    date: Date
  ): Promise<TeamSubscriptionSlotResolved[]> {
    const tss = await this.db.findTeamSubscriptionsForUser(
      assignerUserId,
      date.toISOString()
    );
    const mapToSubscriptions = tss.map((ts) =>
      this.findAssignedSubscriptionsBy(ts, date)
    );
   ...
   */
  async findUnassignedSlots(
    teamSubscriptionId: string
  ): Promise<TeamSubscriptionSlot[]> {
    ...
    return (
      await this.db.findSlotsByTeamSubscriptionId(teamSubscriptionId)
    ).filter((s) => TeamSubscriptionSlot.status(s, now) === "unassigned");
  }

  protected async findAssignedSubscriptionsBy(
    teamSubscription: TeamSubscription,
    date: Date
  ) {
    const dateStr = date.toISOString();
    const slots = await this.db.findSlotsByTeamSubscriptionId(
      teamSubscription.id
    );
    const { startDate } = Subscription.calculateCurrentPeriod(
      teamSubscription.startDate,
      date
    );
  ...
  protected async loadSlot(
    ...
    if (slot.teamSubscriptionId !== tsId) {
      throw new Error(
        `Team Subscription id ${tsId} does not match slot id: ${id}!`
      );
    }
  ...
  protected async cancelSubscription(
    ...
    if (!subscription) {
      throw new Error(
        `Cannot find subscription for Team Subscription Slot ${slotId}!`
      );
    }
  ...
  protected async loadSubscriptionModel(db: AccountingDB, userId: string) {
    ...
    const subscriptionsFromTS = subscriptions.filter((s) =>
      AssignedTeamSubscription.is(s)
    );
    return new SubscriptionModel(userId, subscriptionsFromTS);
  }

  async findTeamSubscriptionSlotsByAssignee(
    assigneeId: string
  ): Promise<TeamSubscriptionSlot[]> {
