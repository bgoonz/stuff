...
export type MarketplacePurchaseEvent =
  Webhooks.WebhookEvent<Webhooks.EventPayloads.WebhookPayloadMarketplacePurchase>;
export type WebhookPayloadMarketplacePurchaseChanged =
  Webhooks.EventPayloads.WebhookPayloadMarketplacePurchase & {
    previous_marketplace_purchase: Webhooks.EventPayloads.WebhookPayloadMarketplacePurchaseMarketplacePurchase;
  };
...
export class GithubSubscriptionMapper {
  ...
  public async map(
    ...
    if (!user) {
      log.error(
        "Received purchase event for an unknown user. Not acting on it.",
        { evt }
      );
      return false;
    }
    ...
    switch (evt.payload.action) {
      ...
      default:
        log.error(
          { userId: user.id },
          "Unknown event type. Not acting on it.",
          { evt }
        );
        return false;
    }
  ...
  public mapSubscriptionPurchase(
    ...
    log.debug({ userId: user.id }, "adding newly purchased subscription", {
      plan,
    });
  }

  protected mapPurchasedEvent(
    user: User,
    evt: MarketplacePurchaseEvent,
    model: SubscriptionModel
  ) {
    ...
    if (!plan) {
      log.error(
        { userId: user.id },
        "cannot map GitHub plan to our own. Not acting on it.",
        { evt }
      );
      return;
    }

    this.mapSubscriptionPurchase(
      user,
      purchase.account.id,
      evt.payload.effective_date,
      plan,
      model
    );
  }

  public mapSubscriptionCancel(
    userId: string,
    effectiveDate: string,
    model: SubscriptionModel
  ) {
    ...
    for (const subscription of subscriptions) {
      ...
      log.debug({ userId }, "cancelling subscription", {
        subscriptionId: subscription.uid,
      });
    }
  }

  protected mapCancelledEvent(
    user: User,
    evt: MarketplacePurchaseEvent,
    model: SubscriptionModel
  ) {
  ...
  public mapSubscriptionChange(
    user: User,
    context: ChangeContext,
    model: SubscriptionModel
  ) {
    const { prevPlan, oldSubscription, newAmount, newStartDate, newPlan } =
      context;
  ...
  protected mapChangedEvent(
    ...
    const context = this.getChangeContext(
      evt.payload as WebhookPayloadMarketplacePurchaseChanged,
      user.id,
      model
    );
    if (!context) {
  ...
  protected mapPendingChangeEvent(
    user: User,
    evt: MarketplacePurchaseEvent,
    model: SubscriptionModel
  ) {
    ...
    if (!oldSubscription) {
      log.error(
        { userId: user.id },
        "received pending change event but can't find previous subscription",
        evt
      );
      return;
    }
    ...
    model.update(oldSubscription);
    log.debug({ userId: user.id }, "marked subscription as pending", {
      subscription: oldSubscription.uid,
    });
  }
  ...
  protected mapGithubToGitpodPlan(
    plan: Webhooks.EventPayloads.WebhookPayloadMarketplacePurchaseMarketplacePurchasePlan
  ): Plan | undefined {
    // GitHub plans are all in USD
    const thisPlan = Plans.getAvailablePlans("USD").filter(
      (p) => p.githubId == plan.id
    );
  ...
  protected getChangeContext(
    payload: WebhookPayloadMarketplacePurchaseChanged,
    userId: string,
    model: SubscriptionModel
    ...
    if (!prevPlan) {
      log.error(
        { userId },
        "Cannot map the previous GitHub plan to our own. Not acting on it.",
        { payload }
      );
      return;
    }
    ...
    if (!newPlan) {
      log.error(
        { userId },
        "Cannot map new GitHub plan to our own. Not acting on it.",
        { payload }
      );
      return;
    }
