...
export class GithubSubscriptionReconciler {
  @inject(GithubSubscriptionMapper)
  protected readonly subscriptionMapper: GithubSubscriptionMapper;
  @inject(PendingGithubEventDB)
  protected readonly pendingEventsDB: PendingGithubEventDB;
  ...
  public async handleIncomingEvent(evt: MarketplacePurchaseEvent) {
    const authId: string =
      evt.payload.marketplace_purchase.account.id.toString();
    ...
    if (user) {
      ...
      await this.reconciliationTasks.enqueue(() =>
        this.reconcileEvent(user, evt)
      );
    } else {
      log.error(
        "Received GitHub marketplace purchase event for an unknown user. Storing for later use.",
        { evt }
      );
  ...
  protected async reconcilePendingEvents() {
    const pendingPurchaseEvents = await this.pendingEventsDB.findWithUser(
    ...
    for (const evt of pendingPurchaseEvents) {
      try {
        ...
        log.debug(
          ...
          "followed up on pending purchasing event",
          { evt }
        );
      } catch (err) {
  ...
  protected async reconcilePlan(plan: Plan, token: string) {
    ...
    const matchingSubscriptions =
      await this.accountingDB.findActiveSubscriptionsByIdentity(
        accounts.map((k) => k.toString()),
        "Public-GitHub"
      );
    for (const account of accounts) {
      const subscriptions = (
        matchingSubscriptions[account.toString()] || []
      ).filter(
      ...
      if (!subscriptions || subscriptions.length == 0) {
        ...
        this.subscriptionMapper.mapSubscriptionPurchase(
          user,
          account,
        ...
        subscriptions[0].planId != plan.chargebeeId
      ) {
        ...
        // We have an active subscription, but not for this plan. Let's change to new plan.
        model = new SubscriptionModel(user.id, subscriptions);
        this.subscriptionMapper.mapSubscriptionCancel(
          user.id,
          new Date().toISOString(),
          model
        );
    ...
    const subscriptionsInDB = (
      await this.accountingDB.findActiveSubscriptionByPlanID(plan.chargebeeId)
    ).filter(
      (s) => !!s.paymentReference && s.paymentReference.startsWith("github:")
    );
  ...
  public start() {
    setInterval(
      () =>
        this.reconciliationTasks.enqueue(() => this.reconcilePendingEvents()),
      1 * 60 * 1000
    ); // every one minute
    setInterval(
      () =>
  ...
  public async reconcileEvent(user: User, evt: MarketplacePurchaseEvent) {
    ...
    await this.accountingDB.transaction(async (db) => {
      ...
      const userPaidSubscriptions = subscriptions.filter(
        (s) =>
          UserPaidSubscription.is(s) && s.paymentReference.startsWith("github:")
      );
