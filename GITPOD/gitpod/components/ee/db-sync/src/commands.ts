...
export class RunCommand implements ICommand {
  ...
  async run(args: any): Promise<void> {
    const config = JSON.parse(
      readFileSync(args.config).toString()
    ) as ReplicationConfig;

    if (config.roundRobin) {
      if (config.source) {
        console.warn(
          "Running in round robin mode. Ignoring source connection configuration!"
        );
      }
      ...
      const replicators = await Promise.all(
        targets.map(async (src, si) => {
          ...
          if (config.replicationLogDir) {
            replicator.enableLogs(
              path.join(config.replicationLogDir, src.name.replace("/", "-"))
            );
          }
      ...
      console.log(
        `Set up ${replicators.length} replicators. Starting initial round.`
      );
      ...
      process.once("SIGINT", () => {
        ...
        if (syncRunning) {
          console.info(
            "SIGINT received ... sync is currently running. Waiting for round to complete."
          );
          requestExit = true;
        } else {
      ...
      return new Promise<void>((resolve, reject) => {
        setInterval(async () => {
          if (syncRunning) {
            console.log(
              "Replication is already running ... skipping this time"
            );
            return;
          }
...
export class ExportCommand implements ICommand {
  ...
  async run(args: any): Promise<void> {
    console.log(
      "Selecting data in range: ",
      args.start_date || "<OPEN>",
      args.end_date || "<OPEN>"
    );
    const conn = await connect(new Config().mysqlConfig);

    const statements =
      await this.tableUpdateProvider.getAllStatementsForAllTables(
        conn,
        args.table_set,
        args.start_date,
        args.end_date
    ...
    await new Promise((resolve, reject) => {
      writeFile(
        "export.sql",
        [...statements.deletions, ...statements.updates].join("\n"),
        (err) => {
          if (err) {
            reject(err);
          } else {
