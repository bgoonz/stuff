...
export class TableUpdate {
  constructor(
    protected table: TableDescription,
  ...
  public async populate(conn: Connection): Promise<void> {
    ...
    if (!timeField) {
      ...
      !(timeField.Type as string).toLowerCase().startsWith("timestamp")
    ) {
      throw new Error(
    ...
    if (this.table.expiryColumn) {
      timeConditions.push(
        `${this.table.expiryColumn} >= UNIX_TIMESTAMP(UTC_TIMESTAMP())`
      );
    }
    let condition = "";
    ...
    const dataQuery = `SELECT ${this.columns.join(", ")} FROM ${
      this.table.name
    } ${condition}`;
    const deletionsAndUpdates = await new Promise<string[][]>(
      (resolve, reject) => {
        ...
        try {
          conn
              ...
              new Transform({
                ...
                transform: (data: any, encoding, cb) => {
                  if (data[this.table.timeColumn] > Date.now()) {
                    const pk = this.table.primaryKeys
                      .map((pk) => data[pk])
                      .join(", ");
                    console.warn(
            ...
            .on("error", (err) =>
              reject(
                new Error(`Error while exporting ${this.table.name}: ${err}`)
              )
            )
            .on("finish", () => {
              console.debug(
  ...
  protected getUpdateStatement(
    ...
    const updates = this.updateColumns
      .map((c, i) => `${c}=${updateValues[i]}`)
      .join(", ");
    const updateConditions = this.getUpdateConditions(row);

    let result = [
      `INSERT${forceInsert ? "" : " IGNORE"} INTO ${
        this.table.name
    ...
    if (!forceInsert) {
      result.push(
        `UPDATE ${this.table.name} SET ${updates} WHERE ${updateConditions};`
      );
    }
    return result;
  }
  ...
  protected getUpdateConditions(row: any): string {
    return this.table.primaryKeys
      ...
      .concat([
        `${this.table.timeColumn}<=${escape(row[this.table.timeColumn])}`,
      ])
      .join(" AND ");
  }
}
...
export class TableUpdateProvider {
  ...
  public async getAllStatementsForAllTables(
    ...
    if (!provider) {
      throw new Error(
        `Unknown table set ${tableSet} or no table description providers registered`
      );
    }
    const tableUpdates = provider
      .getSortedTables()
      .map((t) => new TableUpdate(t, start_date, end_date));
