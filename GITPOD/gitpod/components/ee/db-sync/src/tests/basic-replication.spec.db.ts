...
describe("Basic unidirectional replication", () => {
  ...
  beforeEach(async function () {
    ...
    container
      .bind(TableDescriptionProvider)
      .to(TestTableDescriptionProvider)
      .inSingletonScope();
  ...
  it("Should mark the last replication time", async () => {
    ...
    const lastExport = (await query(
      source,
      'SELECT * FROM gitpod_replication WHERE item="lastExport"'
    )) as any[];
    expect(lastExport).to.be.not.empty;
    expect(lastExport[0].item).to.equal("lastExport");
    expect(
      Date.now() - new Date(lastExport[0].value).getTime()
    ).to.be.greaterThan(0);
  });
  ...
  it("Should pick up from the last replication time", async () => {
    ...
    await query(
      source,
      `INSERT INTO names VALUES ("9fa18735-c43b-4651-81c5-ddfbdee1035d", "Bar", ?, 0)`,
    ...
    expect(await query(target, "SELECT * FROM names")).to.deep.equal([
      ...
      {
        _deleted: 0,
        _lastModified: lastExport[0].value
          .toString()
          .replace("T", " ")
          .replace("Z", ""),
        name: "Bar",
