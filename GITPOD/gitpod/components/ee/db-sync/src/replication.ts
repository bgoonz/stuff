...
export class PeriodicReplicator {
  ...
  public async synchronize(ignoreStartDate: boolean): Promise<void> {
    ...
    try {
      ...
      await Promise.all(
        [this.source, ...this.targets].map((target) =>
          this.update(target, deletions)
        )
      );
      await Promise.all(
        this.targets.map((target) => this.update(target, updates))
      );
  ...
  protected async logStatements(now: Date, updates: string[]) {
    ...
    const dest = path.join(
      logdir,
      `${this.tableSet || "default"}-${now.getTime()}.sql`
    );
    try {
      if (
        !(await new Promise<boolean>((resolve, reject) =>
          fs.exists(logdir, resolve)
        ))
      ) {
  ...
  protected async update(
    ...
    const bar =
      this.showProgressBar && updates.length > batchSize
        ? new ProgressBar(
            "inserting/updating [:bar] :rate/bps :percent :etas",
    ...
    try {
      ...
      if (inTransaction) {
        console.error(
          "Caught an error during modification. Rolling back transaction.",
          err
        );
  ...
  public toString(): string {
    return `${this.source.name} -> [ ${this.targets
      .map((t) => t.name)
      .join(", ")} ]`;
  }
}
