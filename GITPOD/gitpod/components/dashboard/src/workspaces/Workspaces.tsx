    ...
    return (
      <>
        ...
        <div className="lg:px-28 px-10 pt-8 flex">
          ...
          <div className="py-3">
            <DropDown
              ...
              activeEntry={wsModel?.active ? "Active" : "All"}
              entries={[
                {
                  title: "Active",
                  onClick: onActive,
          ...
          <div className="py-3 pl-3">
            <DropDown
              prefix="Limit: "
              contextMenuWidth="w-32"
              activeEntry={wsModel ? wsModel?.limit + "" : undefined}
              entries={[
                {
                  title: "50",
                  onClick: () => {
                    if (wsModel) wsModel.limit = 50;
                  },
                },
                {
                  title: "100",
                  onClick: () => {
                    if (wsModel) wsModel.limit = 100;
                  },
                },
                {
                  title: "200",
                  onClick: () => {
                    if (wsModel) wsModel.limit = 200;
                  },
                },
              ]}
            />
          ...
          {wsModel && this.state?.workspaces.length > 0 ? (
            <button onClick={this.showStartWSModal} className="ml-2">
              New Workspace
            </button>
          ) : null}
        </div>
              ...
              {wsModel.active || wsModel.searchTerm ? null : (
                <Item className="w-full bg-gitpod-kumquat-light py-6 px-6">
                  ...
                  <ItemField className=" flex flex-col">
                    <div className="text-gitpod-red font-semibold">
                      Garbage Collection
                    </div>
                    <p className="text-gray-500">
                      Unpinned workspaces that have been stopped for more than
                      14 days will be automatically deleted.{" "}
                      <a
                        className="gp-link"
              ...
              <div className="px-6 py-3 flex justify-between space-x-2 text-gray-400 border-t border-gray-200 dark:border-gray-800 h-96">
                <div className="flex flex-col items-center w-96 m-auto">
                  ...
                  <div className="text-center pb-6 text-gray-500">
                    Prefix any git repository URL with gitpod.io/# or create a
                  ...
                  <span>
                    <button onClick={this.showStartWSModal}>
                      New Workspace
                    </button>
                    {wsModel.getAllFetchedWorkspaces().size > 0 ? (
                      <button className="secondary ml-2" onClick={onAll}>
                        View All Workspaces
                      </button>
                    ) : null}
                  </span>
        ...
        <StartWorkspaceModal
          ...
          recent={
            wsModel && this.state?.workspaces ? this.getRecentSuggestions() : []
          }
        />
      </>
    );
