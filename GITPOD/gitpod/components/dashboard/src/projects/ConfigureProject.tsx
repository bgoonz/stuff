...
export default function () {
  ...
  const routeMatch = useRouteMatch<{ teamSlug: string; projectSlug: string }>(
    "/:teamSlug/:projectSlug/configure"
  );
  ...
  const [workspaceCreationResult, setWorkspaceCreationResult] = useState<
    WorkspaceCreationResult | undefined
  >();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const project = projects.find(
        (p) => p.name === routeMatch?.params.projectSlug
      );
      if (project) {
        setProject(project);
        const configString =
        ...
        if (configString) {
          ...
          setEditorError(
            <span>
              A Gitpod configuration already exists in the project's{" "}
  ...
  return (
    <div className="flex flex-col mt-24 mx-auto items-center">
      <h1>Configure Project</h1>
      <p className="text-gray-500 text-center text-base">
        Fully-automate your project's dev setup.{" "}
        <a
          className="gp-link"
      ...
      <div className="mt-4 w-full flex">
        <div className="flex-1 m-8">
          ...
          {!isEditorDisabled && (
            <select
              className="w-full"
              defaultValue=""
              onChange={(e) => setGitpodYml(e.target.value)}
            >
              <option value="" disabled={true}>
                …
              </option>
              {Object.entries(TASKS).map(([name, value]) => (
          ...
          <div className="mt-2 flex justify-center space-x-2">
            <button disabled={isEditorDisabled} onClick={buildProject}>
        ...
        <div className="flex-1 m-8">
          ...
          {!!workspaceCreationResult && (
            <PrebuildLogs
              workspaceId={workspaceCreationResult.createdWorkspaceId}
            />
          )}
        </div>
      </div>
