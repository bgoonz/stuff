...
export default function () {
  ...
  const [lastPrebuilds, setLastPrebuilds] = useState<
    Map<string, PrebuildInfo | undefined>
  >(new Map());
        ...
        <ItemsList className="mt-2">
          ...
          {branches
            ...
            .map((branch, index) => {
              ...
              const statusIcon =
                prebuild?.status && prebuildStatusIcon(prebuild.status);
              const status =
                prebuild?.status && prebuildStatusLabel(prebuild.status);
                ...
                >
                  <ItemField className="flex items-center">
                    <div>
                      <div className="text-base text-gray-900 dark:text-gray-50 font-medium mb-1">
                        {branchName}
                        {branch.isDefault && (
                          <span className="ml-2 self-center rounded-xl py-0.5 px-2 text-sm bg-blue-50 text-blue-400">
                            DEFAULT
                          </span>
                        )}
                      </div>
                  ...
                  <ItemField className="flex items-center">
                    <div
                      ...
                      onClick={() => prebuild && openPrebuild(prebuild)}
                    >
                      {prebuild ? (
                        <>
                    ...
                    <ItemFieldContextMenu
                      className="py-0.5"
                      menuEntries={branchContextMenu(branch)}
                    />
                  </ItemField>
                </Item>
              );
