...
export default function () {
  ...
  const match =
    useRouteMatch<{ team: string; resource: string }>("/:team/:resource");
  ...
  const [statusFilter, setStatusFilter] = useState<
    PrebuiltWorkspaceState | undefined
  >();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const project =
        projectName && projects.find((p) => p.name === projectName);
      if (project) {
        setProject(project);
                  ...
                  <p>
                    {p.startedByAvatar && (
                      <img
                        ...
                        src={p.startedByAvatar || ""}
                        alt={p.startedBy}
                      />
                    )}
                    Triggered {formatDate(p.startedAt)}
                  </p>
                </div>
              </ItemField>
              <ItemField className="flex items-center">
                <div>
                  <div className="text-base text-gray-500 dark:text-gray-50 font-medium mb-1">
                    {shortCommitMessage(p.changeTitle)}
                  </div>
                  <p>
                    {p.changeAuthorAvatar && (
                      <img
                        className="rounded-full w-4 h-4 inline-block align-text-bottom mr-2"
                        src={p.changeAuthorAvatar || ""}
                        alt={p.changeAuthor}
                      />
                    )}
                    Authored {formatDate(p.changeDate)} ·{" "}
                    {p.changeHash?.substring(0, 8)}
                  </p>
                </div>
              </ItemField>
              <ItemField className="flex items-center">
...
export function prebuildStatusLabel(status: PrebuiltWorkspaceState) {
  switch (status) {
    ...
    case "available":
      return (
        <span className="font-medium text-green-500 uppercase">ready</span>
      );
    case "building":
      return (
    ...
    case "queued":
      return (
        <span className="font-medium text-orange-500 uppercase">pending</span>
      );
    default:
      break;
  }
      ...
      return (
        ...
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M16 8C16 12.4183 12.4183 16 8 16C3.58172 16 0 12.4183 0 8C0 3.58172 3.58172 0 8 0C12.4183 0 16 3.58172 16 8ZM5 6C5 5.44772 5.44772 5 6 5C6.55228 5 7 5.44772 7 6V10C7 10.5523 6.55228 11 6 11C5.44772 11 5 10.5523 5 10V6ZM10 5C9.44771 5 9 5.44772 9 6V10C9 10.5523 9.44771 11 10 11C10.5523 11 11 10.5523 11 10V6C11 5.44772 10.5523 5 10 5Z"
            fill="#FBBF24"
          />
        </svg>
      );
