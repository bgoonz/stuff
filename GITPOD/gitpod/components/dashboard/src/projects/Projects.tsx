...
export default function () {
  ...
  const [lastPrebuilds, setLastPrebuilds] = useState<Map<string, PrebuildInfo>>(
    new Map()
  );
      ...
      {projects.length > 0 && (
        <div className="lg:px-28 px-10">
          ...
          <div className="mt-4 grid grid-cols-3 gap-4">
            {projects.map((p) => (
              <div key={`project-${p.id}`} className="h-48">
                <div className="h-5/6 border border-gray-200 dark:border-gray-800 rounded-t-xl">
                  <div className="h-3/4 p-6">
                    <div className="flex self-center text-base text-gray-900 dark:text-gray-50 font-medium">
                      <Link
                        to={`/${!!team ? team.slug : "projects"}/${p.name}`}
                      >
                        {p.name}
                      </Link>
                  ...
                  <div className="h-1/4 px-6 py-1">
                    <p>__ Active Branches</p>
                  </div>
                </div>
                <div
                  className="h-1/6 px-6 border rounded-b-xl dark:border-gray-800 bg-gray-200 cursor-pointer"
                ...
                >
                  {lastPrebuilds.get(p.id) ? (
                    <div className="flex flex-row space-x-3 h-full text-sm">
                      <div
                        className={
                          "my-auto rounded-full w-3 h-3 text-sm align-middle " +
                      ...
                      <div className="my-auto text-gray-400">
                        {moment(
                          lastPrebuilds.get(p.id)!.startedAt,
                          "YYYYMMDD"
                        ).fromNow()}
                      </div>
                      <div className="my-auto text-gray-400 flex-grow text-right">
                        View All ⟶
                      </div>
                    </div>
                  ) : (
