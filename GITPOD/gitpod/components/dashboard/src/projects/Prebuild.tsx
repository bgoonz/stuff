...
export default function () {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!project) {
        console.error(
          new Error(
            `Project not found! (teamId: ${team?.id}, projectName: ${projectName})`
          )
        );
        return;
      }
  ...
  const renderTitle = () => {
    ...
    return (
      <h1 className="tracking-tight">
        {prebuild.branch}{" "}
        <span className="text-gray-200">#{prebuild.branchPrebuildNumber}</span>
      </h1>
    );
  };
  ...
  const renderSubtitle = () => {
    ...
    const startedByAvatar = prebuild.startedByAvatar && (
      <img
        className="rounded-full w-4 h-4 inline-block align-text-bottom mr-2"
        src={prebuild.startedByAvatar || ""}
        alt={prebuild.startedBy}
      />
    );
    return (
      <div className="flex">
