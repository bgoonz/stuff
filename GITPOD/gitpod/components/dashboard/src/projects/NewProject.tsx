...
export default function NewProject() {
  ...
  useEffect(() => {
    if (reposInAccounts.length === 0) {
      ...
      const mostRecent = reposInAccounts.reduce((prev, current) =>
        (prev.installationUpdatedAt || 0) > (current.installationUpdatedAt || 0)
          ? prev
          : current
      );
  ...
  const createProject = async (
    ...
    const repo = reposInAccounts.find(
      (r) => r.account === selectedAccount && r.name === selectedRepo
    );
    if (!repo) {
    ...
    history.push(
      `/${User.is(teamOrUser) ? "projects" : teamOrUser.slug}/${
        repo.name
      }/configure`
    );
  };
  ...
  const reposToRender = Array.from(reposInAccounts).filter(
    (r) => r.account === selectedAccount && r.name.includes(repoSearchFilter)
  );
  ...
  const renderSelectRepository = () => {
    ...
    const renderRepos = () => (
      <div className="mt-10 border rounded-t-xl border-gray-100 flex-col">
      ...
      <div className="mt-8 border rounded-xl border-gray-100 dark:border-gray-700 flex-col">
        <div>
          <div className="px-12 py-16 text-center text-gray-500 bg-gray-50 dark:bg-gray-800 rounded-xl">
            ...
            <h3 className="mb-2 text-gray-600 dark:text-gray-400">No Access</h3>
    ...
    return (
      <>
        {loaded && empty ? (
          ...
          <GitProviders onHostSelected={onGitProviderSeleted} />
        ) : (
          renderRepos()
        )}
      </>
      ...
      <>
        ...
        <div className="mt-8 border rounded-xl border-gray-100 flex-col">
          <div
            ...
            className={`w-96 border-b px-8 py-4 flex space-x-2 justify-between dark:hover:bg-gray-800 focus:bg-gitpod-kumquat-light transition ease-in-out group`}
          >
...
function GitProviders(props: { onHostSelected: (host: string) => void }) {
