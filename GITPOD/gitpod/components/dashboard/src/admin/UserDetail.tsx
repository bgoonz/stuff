...
export default function UserDetail(p: { user: User }) {
  ...
  const isProfessionalOpenSource =
    accountStatement &&
    accountStatement.subscriptions.some(
  ...
  const addStudentDomain = async () => {
    await updateUser(async (u) => {
      await getGitpodService().server.adminAddStudentEmailDomain(
        u.id,
        emailDomain
      );
    ...
    <>
      ...
      >
        <div className="flex">
          ...
          <button
            className="secondary danger ml-3"
            disabled={activity}
          ...
          <div className="flex flex-col w-full">
            <div className="flex w-full mt-6">
              <Property name="Sign Up Date">
                {moment(user.creationDate).format("MMM D, YYYY")}
              </Property>
              <Property
                ...
                actions={
                  accountStatement && [
                    ...
                    },
                    {
                      ...
                      onClick: async () => {
                        await getGitpodService().server.adminGrantExtraHours(
                          user.id,
                          20
                        );
                        setAccountStatement(
                          await getGitpodService().server.adminGetAccountStatement(
                            user.id
                          )
                        );
                      },
              ...
              >
                {accountStatement?.remainingHours
                  ? accountStatement?.remainingHours.toString()
                  : "---"}
              </Property>
              <Property
                name="Plan"
                actions={
                  accountStatement && [
                    {
                      label:
                        (isProfessionalOpenSource ? "Disable" : "Enable") +
                        " Professional OSS",
                      onClick: async () => {
      ...
      <Modal
        ...
        buttons={[
          <button
            className="secondary"
            onClick={() => setEditFeatureFlags(false)}
          >
            Done
          </button>,
        ]}
      ...
      <Modal
        visible={viewAccountStatement}
        onClose={() => setViewAccountStatement(false)}
        title="Edit Roles"
        buttons={[
          <button
            className="secondary"
            onClick={() => setViewAccountStatement(false)}
          >
            Done
          </button>,
        ]}
...
function Label(p: { text: string; color: string }) {
  return (
    <div
      className={`ml-3 text-sm text-${p.color}-600 truncate bg-${p.color}-100 px-1.5 py-0.5 rounded-md my-auto`}
    >
      {p.text}
    </div>
  );
...
function getFlags(user: User, updateUser: UpdateUserFunction): Entry[] {
  return Object.entries(WorkspaceFeatureFlags)
    ...
    .map((name) => {
      const checked =
        !!user.featureFlags?.permanentWSFeatureFlags?.includes(name);
      return {
        title: name,
        checked,
        onClick: async () => {
...
function getRopEntries(user: User, updateUser: UpdateUserFunction): Entry[] {
  ...
  return [
    ...Object.entries(Permissions).map((e) =>
      createRopEntry(e[0] as RoleOrPermission)
    ),
    ...Object.entries(Roles).map((e) =>
      createRopEntry(e[0] as RoleOrPermission, true)
    ),
  ];
}
