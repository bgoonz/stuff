...
} from "../workspaces/WorkspaceEntry";
...
export default function WorkspaceDetail(props: {
  ...
  useEffect(() => {
    getGitpodService()
      .server.adminGetUser(props.workspace.ownerId)
      .then(setUser);
  }, [props.workspace]);
  ...
  const stopWorkspace = async () => {
    try {
      setActivity(true);
      await getGitpodService().server.adminForceStopWorkspace(
  ...
  const reload = async () => {
    try {
      setActivity(true);
      const ws = await getGitpodService().server.adminGetWorkspace(
        workspace.workspaceId
      );
      setWorkspace(ws);
    } finally {
        ...
        <div className="flex-1">
          <div className="flex">
            ...
            <span className="my-auto ml-3">
              <WorkspaceStatusIndicator
                instance={WorkspaceAndInstance.toInstance(workspace)}
              />
            </span>
          </div>
          <p>{getProject(WorkspaceAndInstance.toWorkspace(workspace))}</p>
        </div>
        <button className="secondary ml-3" disabled={activity} onClick={reload}>
          Reload Data
        </button>
        <button
      ...
      <div className="flex mt-6">
        <div className="flex flex-col w-full">
          <div className="flex w-full mt-6">
            ...
            <Property name="Last Start">
              {moment(workspace.instanceCreationTime).fromNow()}
            </Property>
            <Property name="Context">
              <a
                className="text-blue-400 dark:text-blue-600 hover:text-blue-600 dark:hover:text-blue-400"
                href={workspace.contextURL}
              >
          ...
          <div className="flex w-full mt-6">
            ...
            >
              {workspace.softDeleted
                ? `'${workspace.softDeleted}' ${moment(
                    workspace.softDeletedTime
                  ).fromNow()}`
                : "No"}
            </Property>
          </div>
          <div className="flex w-full mt-12">
            ...
            <Property name="Stopped">
              {workspace.stoppedTime
                ? moment(workspace.stoppedTime).fromNow()
                : "---"}
            </Property>
          </div>
