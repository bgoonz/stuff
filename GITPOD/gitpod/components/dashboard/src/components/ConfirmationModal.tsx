...
export default function ConfirmationModal(props: {
  ...
  const c: React.ReactChild[] = [
    <p className="mt-1 mb-2 text-base text-gray-500">
      {props.areYouSureText || "Are you sure?"}
    </p>,
  ];

  const isEntity = (x: any): x is Entity =>
    typeof x === "object" && "name" in x;
  if (props.children) {
      ...
      c.push(
        <div className="w-full p-4 mb-2 bg-gray-100 dark:bg-gray-700 rounded-xl group">
          ...
          {props.children.description && (
            <p className="text-gray-500">{props.children.description}</p>
          )}
        </div>
      );
