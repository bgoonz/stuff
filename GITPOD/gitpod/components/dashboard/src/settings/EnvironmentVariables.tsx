      ...
      <div className="border-t border-b border-gray-200 dark:border-gray-800 -mx-6 px-6 py-4 flex flex-col">
        ...
        <div className="mt-1">
          <p className="text-gray-500">
            You can pass a variable for a specific project or use wildcard
            character (<CodeText>*/*</CodeText>) to make it available in more
            projects.
          </p>
        </div>
      </div>
...
export default function EnvVars() {
  ...
  const [isDeleteEnvVarModalVisible, setDeleteEnvVarModalVisible] =
    useState(false);
  const update = async () => {
  ...
  const validate = (variable: UserEnvVarValue) => {
    ...
    if (
      !variable.id &&
      envVars.some((v) => v.name === name && v.repositoryPattern === pattern)
    ) {
        ...
        <div className="bg-gray-100 dark:bg-gray-800 rounded-xl w-full h-96">
          <div className="pt-28 flex flex-col items-center w-96 m-auto">
            ...
            <div className="text-center pb-6 text-gray-500">
              In addition to user-specific environment variables you can also
              pass variables through a workspace creation URL.{" "}
              <a
                className="gp-link"
          ...
          {envVars.map((variable) => {
            return (
              <Item className="whitespace-nowrap">
                <ItemField className="w-5/12 overflow-ellipsis truncate">
                  {variable.name}
                </ItemField>
                <ItemField className="w-5/12 overflow-ellipsis truncate text-sm text-gray-400">
