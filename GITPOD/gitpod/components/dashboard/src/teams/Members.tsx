...
export default function () {
  ...
  const resetInviteLink = async () => {
    ...
    if (genericInvite) {
      ...
      const newInvite = await getGitpodService().server.resetGenericInvite(
        team!.id
      );
  ...
  const removeTeamMember = async (userId: string) => {
    ...
    if (newTeams.some((t) => t.id === team!.id)) {
      ...
      const newMembers = await getGitpodService().server.getTeamMembers(
        team!.id
      );
  ...
  const filteredMembers = members.filter((m) => {
    ...
    const memberSearchText = `${m.fullName || ""}${
      m.primaryEmail || ""
    }`.toLocaleLowerCase();
    ...
    <>
      ...
      <div className="lg:px-28 px-10">
        <div className="flex mt-8">
          ...
          <button onClick={() => setShowInviteModal(true)} className="ml-2">
            Invite Members
          </button>
        </div>
                      ...
                      <DropDown
                        ...
                        entries={[
                          ...
                          },
                          {
                            title: "member",
                            onClick: () =>
                              setTeamMemberRole(m.userId, "member"),
                          },
                        ]}
                      />
                    )}
