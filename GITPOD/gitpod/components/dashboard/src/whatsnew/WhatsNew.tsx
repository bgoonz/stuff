...
export const shouldSeeWhatsNew = (
  ...
  return news.some(
    (x) =>
      user.creationDate <= x.maxUserCreationDate &&
      (!whatsNewSeen || !whatsNewSeen[x.newsKey])
  );
};
...
export function WhatsNew(props: { onClose: () => void }) {
  ...
  const _unseenEntries =
    allEntries.filter((x) => user && shouldSeeWhatsNew(user, [x])) || [];
  ...
  const markAsSeen = async (
    ...
    for (const newsKey of (
      news.filter((x) => x !== undefined) as WhatsNewEntry[]
    ).map((x) => x.newsKey)) {
        ...
        <>
          <div className="flex items-center justify-end mt-6 space-x-2">
            <div className="text-sm mr-auto italic">
              {unseenEntries.length} more update
              {unseenEntries.length > 1 ? "s" : ""}
            </div>
