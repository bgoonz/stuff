...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
  ...
  if [[ "$TRAVIS_JDK_VERSION" == *"jdk8" ]]; then
    if [ "$TRAVIS_TAG" == "" ]; then
      ...
      # snapshot
      ./gradlew -Prelease.travisci=true -PbintrayUser="${bintrayUser}" -PbintrayKey="${bintrayKey}" -PsonatypeUsername="${sonatypeUsername}" -PsonatypePassword="${sonatypePassword}" build snapshot --info --scan
    else
