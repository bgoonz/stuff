...
#   [*rewrite*]            - Specifies rewrite rules for vhost server definition. Hash should contain 'regex' and 'replacement' keys (required) and could contain a 'flag' key (optional)
...
#   [*ssi*]                - Enabled of disables server side includes (on|off)
#
# Actions:
#
# Requires:
...
define nginx::resource::vhost(
  ...
  if $rewrite != {} {
    ...
    file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-010":
      ensure => $ensure ? {
        'absent' => 'absent',
        default  => 'file',
      },
      content => template('nginx/vhost/vhost_rewrite.erb'),
      notify  => Class['nginx::service'],
    }
  }
