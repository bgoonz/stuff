...
class ModulePlatform(object):
	...
	def create_platform(cls, platform, project_dir, config, module):
		...
		platform_dir = os.path.join(this_dir, platform)
...
class ModuleProject(object):
	def __init__(self, platform, project_dir, config, ti_sdk_dir):
	...
	def get_file_dest(self, this_dir, from_path):
		file_dest = os.path.expanduser(from_path.replace(this_dir, self.project_dir, 1))
		return self.platform_delegate.get_file_dest(file_dest)
		
	def copy_template_files(self, this_dir):
		for root, dirs, files in os.walk(this_dir):
			...
			for file in files:
				if file in ignoreFiles:
					continue
				prefix = root[len(this_dir):]
				from_ = os.path.join(root, file)
				to_ = self.get_file_dest(this_dir, from_)
...
class ModuleDetector(object):
	def __init__(self, project_dir, ti_sdk_dir):
		self.project_dir = project_dir
		self.ti_sdk_dir = ti_sdk_dir
	...
	def detect_modules(self):
		system_modules_dir = os.path.abspath(os.path.join(self.ti_sdk_dir, '..', '..', '..', 'modules'))
