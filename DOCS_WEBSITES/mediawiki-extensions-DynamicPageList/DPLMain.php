...
class DPLMain {
	...
	public static function dynamicPageList( $input, $params, Parser $parser, &$bReset, $calledInMode ) {
		...
		/**
		 * If we include the Uncategorized, we need the 'dpl_clview': VIEW of the
		 * categorylinks table where we have cl_to='' (empty string) for all
		 * uncategorized pages. This VIEW must have been created by the
		 * administrator of the MediaWiki DB at installation.
		...
		foreach ( $aOrderMethods as $sOrderMethod ) {
			switch ( $sOrderMethod ) {
				...
				case 'sortkey':
					...
					// sortkey in the usual way (full page name, underscores
		...
		// Do NOT process the SQL command if debug==6; this is useful if the SQL statement contains bad code
